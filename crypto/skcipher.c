...
int skcipher_walk_done(struct skcipher_walk *walk, int err)
{
    ...
    if (likely(!(walk->flags & (SKCIPHER_WALK_PHYS | SKCIPHER_WALK_SLOW | SKCIPHER_WALK_COPY | SKCIPHER_WALK_DIFF))))
    {
    unmap_src:
    ...
    if (nbytes)
    {
        crypto_yield(walk->flags & SKCIPHER_WALK_SLEEP ? CRYPTO_TFM_REQ_MAY_SLEEP : 0);
...
static int skcipher_next_copy(struct skcipher_walk *walk)
{
    ...
    if (offset_in_page(walk->page) + walk->nbytes + walk->stride > PAGE_SIZE)
        walk->page = NULL;
    else
                                  ...
                                  struct skcipher_request *req)
{
    ...
    walk->flags |= req->base.flags & CRYPTO_TFM_REQ_MAY_SLEEP ? SKCIPHER_WALK_SLEEP : 0;
...
int skcipher_prepare_alg_common(struct skcipher_alg_common *alg)
{
    ...
    if (alg->ivsize > PAGE_SIZE / 8 || alg->chunksize > PAGE_SIZE / 8 || alg->statesize > PAGE_SIZE / 2 || (alg->ivsize + alg->statesize) > PAGE_SIZE / 2)
        return -EINVAL;
