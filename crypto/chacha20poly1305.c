...
static int chachapoly_init(struct crypto_aead *tfm)
{
    ...
    crypto_aead_set_reqsize(
        tfm,
        align + offsetof(struct chachapoly_req_ctx, u) + max(offsetof(struct chacha_req, req) + sizeof(struct skcipher_request) + crypto_skcipher_reqsize(chacha), offsetof(struct poly_req, req) + sizeof(struct ahash_request) + crypto_ahash_reqsize(poly)));
                             ...
                             const char *name, unsigned int ivsize)
{
    ...
    if (snprintf(inst->alg.base.cra_driver_name, CRYPTO_MAX_ALG_NAME,
                 ...
                 poly->base.cra_driver_name)
        >= CRYPTO_MAX_ALG_NAME)
        goto err_free_inst;

    inst->alg.base.cra_priority  = (chacha->base.cra_priority + poly->base.cra_priority) / 2;
    inst->alg.base.cra_blocksize = 1;
    inst->alg.base.cra_alignmask = chacha->base.cra_alignmask;
    inst->alg.base.cra_ctxsize   = sizeof(struct chachapoly_ctx) + ctx->saltlen;
