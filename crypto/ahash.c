...
int shash_ahash_finup(struct ahash_request *req, struct shash_desc *desc)
{
    ...
    do
    {
        nbytes = crypto_hash_walk_last(&walk) ? crypto_shash_finup(desc, walk.data, nbytes,
                                                                   req->result)
...
int shash_ahash_digest(struct ahash_request *req, struct shash_desc *desc)
{
    ...
    if (nbytes && (sg = req->src, offset = sg->offset, nbytes <= min(sg->length, ((unsigned int)(PAGE_SIZE)) - offset)))
    {
        void *data;
...
{
    if (alg->setkey != ahash_nosetkey && !(alg->halg.base.cra_flags & CRYPTO_ALG_OPTIONAL_KEY))
    ...
    if (likely(tfm->using_shash))
    {
        ...
        if (unlikely(err))
        {
            crypto_ahash_set_flags(tfm,
                                   crypto_shash_get_flags(shash) & CRYPTO_TFM_NEED_KEY);
            return err;
        }
    }
    else
