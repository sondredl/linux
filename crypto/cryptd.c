...
static int cryptd_skcipher_init_tfm(struct crypto_skcipher *tfm)
{
    ...
    crypto_skcipher_set_reqsize(
        tfm, sizeof(struct cryptd_skcipher_request_ctx) + crypto_skcipher_reqsize(cipher));
    return 0;
}
...
static int cryptd_hash_init_tfm(struct crypto_ahash *tfm)
{
    ...
    crypto_ahash_set_reqsize(tfm,
                             sizeof(struct cryptd_hash_request_ctx) + crypto_shash_descsize(hash));
    return 0;
}
                              ...
                              const u8 *key, unsigned int keylen)
{
    ...
    crypto_shash_set_flags(child, crypto_ahash_get_flags(parent) & CRYPTO_TFM_REQ_MASK);
                              ...
                              struct cryptd_queue     *queue)
{
    ...
    inst->alg.halg.base.cra_flags |= CRYPTO_ALG_ASYNC | (alg->base.cra_flags & (CRYPTO_ALG_INTERNAL | CRYPTO_ALG_OPTIONAL_KEY));
...
static int cryptd_aead_init_tfm(struct crypto_aead *tfm)
{
    ...
    crypto_aead_set_reqsize(
        tfm, sizeof(struct cryptd_aead_request_ctx) + crypto_aead_reqsize(cipher));
    return 0;
}
