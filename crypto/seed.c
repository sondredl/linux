...
#define OP(X1, X2, X3, X4, rbase)                                                   \
    ...
    t1 = X4 ^ ks[rbase + 1];                                                        \
    t1 ^= t0;                                                                       \
    t1 = SS0[byte(t1, 0)] ^ SS1[byte(t1, 1)] ^ SS2[byte(t1, 2)] ^ SS3[byte(t1, 3)]; \
    t0 += t1;                                                                       \
    t0 = SS0[byte(t0, 0)] ^ SS1[byte(t0, 1)] ^ SS2[byte(t0, 2)] ^ SS3[byte(t0, 3)]; \
    t1 += t0;                                                                       \
    t1 = SS0[byte(t1, 0)] ^ SS1[byte(t1, 1)] ^ SS2[byte(t1, 2)] ^ SS3[byte(t1, 3)]; \
    t0 += t1;                                                                       \
    X1 ^= t0;                                                                       \
                        ...
                        unsigned int key_len)
{
    ...
    for (i = 0; i < SEED_NUM_KCONSTANTS; i++)
    {
        t0          = x1 + x3 - KC[i];
        t1          = x2 + KC[i] - x4;
        *(keyout++) = SS0[byte(t0, 0)] ^ SS1[byte(t0, 1)] ^ SS2[byte(t0, 2)] ^ SS3[byte(t0, 3)];
        *(keyout++) = SS0[byte(t1, 0)] ^ SS1[byte(t1, 1)] ^ SS2[byte(t1, 2)] ^ SS3[byte(t1, 3)];
