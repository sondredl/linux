                               ...
                               u32 *boot)
{
    ...
    if (*boot)
    {
        if (((u32)atomic_read(&data->rb_writer)) > JENT_TEST_RINGBUFFER_SIZE)
        {
            *boot = 2;
    ...
    data->jent_testing_rb[((u32)atomic_read(&data->rb_writer)) & JENT_TEST_RINGBUFFER_MASK] = value;
...
{
    return ((((u32)atomic_read(&data->rb_writer)) & JENT_TEST_RINGBUFFER_MASK) != (data->rb_reader & JENT_TEST_RINGBUFFER_MASK));
}
