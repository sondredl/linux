...
static int crypto_check_alg(struct crypto_alg *alg)
{
    ...
    if (!alg->cra_type && (alg->cra_flags & CRYPTO_ALG_TYPE_MASK) == CRYPTO_ALG_TYPE_CIPHER)
    {
                                           ...
                                           struct list_head  *algs_to_put)
{
    ...
    list_for_each_entry(q, &crypto_alg_list, cra_list)
    {
        ...
        if (crypto_is_larval(q))
        {
             ...
             */
            if (strcmp(alg->cra_name, q->cra_name) && strcmp(alg->cra_driver_name, q->cra_name))
                continue;
        ...
        if (strcmp(alg->cra_driver_name, q->cra_driver_name) && q->cra_priority > alg->cra_priority)
            continue;
...
static struct crypto_larval *crypto_alloc_test_larval(struct crypto_alg *alg)
{
    ...
    if (!IS_ENABLED(CONFIG_CRYPTO_MANAGER) || IS_ENABLED(CONFIG_CRYPTO_MANAGER_DISABLE_TESTS) || (alg->cra_flags & CRYPTO_ALG_INTERNAL))
    ...
    __crypto_register_alg(struct crypto_alg *alg, struct list_head *algs_to_put)
{
    ...
    list_for_each_entry(q, &crypto_alg_list, cra_list)
    {
        ...
        if (!strcmp(q->cra_driver_name, alg->cra_name) || !strcmp(q->cra_driver_name, alg->cra_driver_name) || !strcmp(q->cra_name, alg->cra_driver_name))
            goto err;
    }
...
void crypto_inc(u8 *a, unsigned int size)
{
    ...
    if (IS_ENABLED(CONFIG_HAVE_EFFICIENT_UNALIGNED_ACCESS) || IS_ALIGNED((unsigned long)b, __alignof__(*b)))
        for (; size >= 4; size -= 4)
        {
            c  = be32_to_cpu(*--b) + 1;
            *b = cpu_to_be32(c);
