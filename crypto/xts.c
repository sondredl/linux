                         ...
                         bool enc)
{
    ...
    while (w.nbytes)
    {
        ...
        do
        {
            if (unlikely(cts) && w.total - w.nbytes + avail < 2 * XTS_BLOCK_SIZE)
            {
                if (!enc)
                {
                    if (second_pass)
...
static int xts_init_tfm(struct crypto_skcipher *tfm)
{
    ...
    crypto_skcipher_set_reqsize(tfm, crypto_skcipher_reqsize(child) + sizeof(struct xts_request_ctx));
...
static int xts_create(struct crypto_template *tmpl, struct rtattr **tb)
{
    ...
    inst->alg.base.cra_alignmask = alg->base.cra_alignmask | (__alignof__(u64) - 1);
