                                     ...
                                     unsigned int         flags)
{
    ...
    return crypto_ahash_digest(ahreq) ?: crypto_authenc_esn_genicv_tail(req, aead_request_flags(req));
}
...
static int crypto_authenc_esn_init_tfm(struct crypto_aead *tfm)
{
    ...
    crypto_aead_set_reqsize(
        tfm,
        sizeof(struct authenc_esn_request_ctx) + ctx->reqoff + max_t(unsigned int, crypto_ahash_reqsize(auth) + sizeof(struct ahash_request), sizeof(struct skcipher_request) + crypto_skcipher_reqsize(enc)));
                                     ...
                                     struct rtattr         **tb)
{
    ...
    if (snprintf(inst->alg.base.cra_driver_name, CRYPTO_MAX_ALG_NAME,
                 ...
                 enc->base.cra_driver_name)
        >= CRYPTO_MAX_ALG_NAME)
        goto err_free_inst;

    inst->alg.base.cra_priority  = enc->base.cra_priority * 10 + auth_base->cra_priority;
