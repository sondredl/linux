                             ...
                             unsigned int keylen)
{
    ...
    desc->tfm = tctx->hash;
    err       = crypto_shash_init(desc) ?: crypto_shash_update(desc, keys.enckey, keys.enckeylen) ?
...
static int essiv_skcipher_crypt(struct skcipher_request *req, bool enc)
{
    ...
    return enc ? crypto_skcipher_encrypt(subreq) : crypto_skcipher_decrypt(subreq);
}
...
static int essiv_skcipher_init_tfm(struct crypto_skcipher *tfm)
{
    ...
    crypto_skcipher_set_reqsize(tfm, sizeof(struct skcipher_request) + crypto_skcipher_reqsize(skcipher));
...
static int essiv_aead_init_tfm(struct crypto_aead *tfm)
{
    ...
    BUILD_BUG_ON(offsetofend(struct essiv_aead_request_ctx, aead_req) != sizeof(struct essiv_aead_request_ctx));

    aead = crypto_spawn_aead(&ictx->u.aead_spawn);
    if (IS_ERR(aead))
        return PTR_ERR(aead);

    subreq_size = sizeof_field(struct essiv_aead_request_ctx, aead_req) + crypto_aead_reqsize(aead);

    tctx->ivoffset = offsetof(struct essiv_aead_request_ctx, aead_req) + subreq_size;
                                       ...
                                       int               ivsize)
{
    ...
    if (hash_alg->digestsize < alg->cra_cipher.cia_min_keysize || hash_alg->digestsize > alg->cra_cipher.cia_max_keysize)
        goto out;
...
static int essiv_create(struct crypto_template *tmpl, struct rtattr **tb)
{
    ...
    switch (type)
    {
        ...
        case CRYPTO_ALG_TYPE_AEAD:
            aead_inst = kzalloc(sizeof(*aead_inst) + sizeof(*ictx), GFP_KERNEL);
            if (!aead_inst)
                return -ENOMEM;
     ...
     * flags manually.
     */
    base->cra_flags |= (hash_alg->base.cra_flags & CRYPTO_ALG_INHERITED_FLAGS);
