                             ...
                             size_t ignored, int flags)
{
    ...
    areq = af_alg_alloc_areq(sk, sizeof(struct af_alg_async_req) + crypto_skcipher_reqsize(tfm));
    if (IS_ERR(areq))
        return PTR_ERR(areq);
    ...
    if (msg->msg_iocb && !is_sync_kiocb(msg->msg_iocb))
    {
        ...
        skcipher_request_set_callback(&areq->cra_u.skcipher_req,
                                      cflags | CRYPTO_TFM_REQ_MAY_SLEEP,
                                      algif_skcipher_done, areq);
        err = ctx->enc ? crypto_skcipher_encrypt(&areq->cra_u.skcipher_req) : crypto_skcipher_decrypt(&areq->cra_u.skcipher_req);
    ...
    else
    {
        ...
        skcipher_request_set_callback(&areq->cra_u.skcipher_req,
                                      cflags | CRYPTO_TFM_REQ_MAY_SLEEP | CRYPTO_TFM_REQ_MAY_BACKLOG,
                                      crypto_req_done, &ctx->wait);
        err = crypto_wait_req(ctx->enc ? crypto_skcipher_encrypt(&areq->cra_u.skcipher_req) : crypto_skcipher_decrypt(&areq->cra_u.skcipher_req),
                              &ctx->wait);
