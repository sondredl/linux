                                ...
                                unsigned int key_len)
{
    ...
    crypto_skcipher_set_flags(child, crypto_skcipher_get_flags(tfm) & CRYPTO_TFM_REQ_MASK);
...
static int simd_skcipher_encrypt(struct skcipher_request *req)
{
    ...
    *subreq = *req;
    ...
    if (!crypto_simd_usable() || (in_atomic() && cryptd_skcipher_queued(ctx->cryptd_tfm)))
                                               ...
                                               const char *basename)
{
    ...
    if (snprintf(drvname, CRYPTO_MAX_ALG_NAME, "simd-%s", basename) >= CRYPTO_MAX_ALG_NAME)
...
static int simd_aead_encrypt(struct aead_request *req)
{
    ...
    *subreq = *req;
    ...
    if (!crypto_simd_usable() || (in_atomic() && cryptd_aead_queued(ctx->cryptd_tfm)))
        child = &ctx->cryptd_tfm->base;
    else
