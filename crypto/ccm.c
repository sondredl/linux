                             ...
                             unsigned int keylen)
{
    ...
    crypto_ahash_set_flags(mac, crypto_aead_get_flags(aead) & CRYPTO_TFM_REQ_MASK);
...
static int crypto_ccm_init_tfm(struct crypto_aead *tfm)
{
    ...
    crypto_aead_set_reqsize(
        tfm,
        align + sizeof(struct crypto_ccm_req_priv_ctx) + max(crypto_ahash_reqsize(mac), crypto_skcipher_reqsize(ctr)));
                                    ...
                                    const char             *mac_name)
{
    ...
    err = -EINVAL;
    if (strncmp(mac->base.cra_name, "cbcmac(", 7) != 0 || mac->digestsize != 16)
        goto err_free_inst;
    ...
    if (snprintf(inst->alg.base.cra_name, CRYPTO_MAX_ALG_NAME,
                 "ccm(%s", ctr->base.cra_name + 4)
        >= CRYPTO_MAX_ALG_NAME)
        goto err_free_inst;

    if (snprintf(inst->alg.base.cra_driver_name, CRYPTO_MAX_ALG_NAME,
                 "ccm_base(%s,%s)", ctr->base.cra_driver_name,
                 mac->base.cra_driver_name)
        >= CRYPTO_MAX_ALG_NAME)
        goto err_free_inst;

    inst->alg.base.cra_priority  = (mac->base.cra_priority + ctr->base.cra_priority) / 2;
...
static int crypto_rfc4309_init_tfm(struct crypto_aead *tfm)
{
    ...
    crypto_aead_set_reqsize(
        tfm,
        sizeof(struct crypto_rfc4309_req_ctx) + ALIGN(crypto_aead_reqsize(aead), crypto_tfm_ctx_alignment()) + align + 32);
...
static int cbcmac_create(struct crypto_template *tmpl, struct rtattr **tb)
{
    ...
    inst->alg.descsize   = sizeof(struct cbcmac_desc_ctx) + alg->cra_blocksize;
