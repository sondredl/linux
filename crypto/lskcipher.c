...
static int crypto_lskcipher_crypt_unaligned(
    ...
    u8 *iv, int (*crypt)(struct crypto_lskcipher *tfm, const u8 *src, u8 *dst, unsigned len, u8 *iv, u32 flags))
{
    ...
    BUILD_BUG_ON(MAX_CIPHER_BLOCKSIZE > PAGE_SIZE || MAX_CIPHER_ALIGNMASK >= PAGE_SIZE);
                                               ...
                                               u32 flags))
{
    ...
    if (((unsigned long)src | (unsigned long)dst | (unsigned long)iv) & alignmask)
                                                  ...
                                                  u32 flags))
{
    ...
    while (walk.nbytes)
    {
        err = crypt(tfm, walk.src.virt.addr, walk.dst.virt.addr,
                    walk.nbytes, ivs,
                    flags & ~(walk.nbytes == walk.total ? 0 : CRYPTO_LSKCIPHER_FLAG_FINAL));
                                   ...
                                   unsigned int keylen)
{
    ...
    crypto_lskcipher_set_flags(cipher, crypto_lskcipher_get_flags(tfm) & CRYPTO_TFM_REQ_MASK);
    ...
    struct crypto_template *tmpl, struct rtattr **tb)
{
    ...
    if (ecb_name[0])
    {
        ...
        if (strcmp(ecb_name, cipher_name) && snprintf(inst->alg.co.base.cra_driver_name, CRYPTO_MAX_ALG_NAME, "%s(%s)", tmpl->name, cipher_name) >= CRYPTO_MAX_ALG_NAME)
            goto err_free_inst;
    }
    else
    {
