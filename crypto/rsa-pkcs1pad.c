...
#define _(X)                                                                   \
    {                                                                          \
        "sha3-" #X, rsa_digest_info_sha3_##X, sizeof(rsa_digest_info_sha3_##X) \
    }
    _(256),
    _(384),
    _(512),
...
static int pkcs1pad_verify(struct akcipher_request *req)
{
    ...
    if (WARN_ON(req->dst) || WARN_ON(!digest_size) || !ctx->key_size || sig_size != ctx->key_size)
        return -EINVAL;
...
static int pkcs1pad_init_tfm(struct crypto_akcipher *tfm)
{
    ...
    akcipher_set_reqsize(tfm, sizeof(struct pkcs1pad_request) + crypto_akcipher_reqsize(child_tfm));
