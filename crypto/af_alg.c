...
static int alg_bind(struct socket *sock, struct sockaddr *uaddr, int addr_len)
{
    ...
    BUILD_BUG_ON(offsetof(struct sockaddr_alg_new, salg_name) != offsetof(struct sockaddr_alg, salg_name));
                                    ...
                                    unsigned int optlen)
{
    ...
    else if (IS_REACHABLE(CONFIG_ENCRYPTED_KEYS) && !strcmp(key->type->name, "encrypted"))
    {
        ret = key_data_ptr_encrypted(key, &key_datalen);
    }
    else if (IS_REACHABLE(CONFIG_TRUSTED_KEYS) && !strcmp(key->type->name, "trusted"))
    {
    ...
    for_each_cmsghdr(cmsg, msg)
    {
        ...
        switch (cmsg->cmsg_type)
        {
            case ALG_SET_IV:
                ...
                if (cmsg->cmsg_len < CMSG_LEN(con->iv->ivlen + sizeof(*con->iv)))
                    return -EINVAL;
                break;
