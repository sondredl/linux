...
{
    return (struct aegis128_state){{vld1q_u8(state),
                                    vld1q_u8(state + 16),
                                    vld1q_u8(state + 32),
                                    vld1q_u8(state + 48),
                                    vld1q_u8(state + 64)}};
    ...
    aegis_aes_round(uint8x16_t w)
{
    ...
    #else
        asm("tbl %0.16b, {v16.16b-v19.16b}, %1.16b"
            : "=w"(v)
            : "w"(w));
        w -= 0x40;
        asm("tbx %0.16b, {v20.16b-v23.16b}, %1.16b"
            : "+w"(v)
            : "w"(w));
        w -= 0x40;
        asm("tbx %0.16b, {v24.16b-v27.16b}, %1.16b"
            : "+w"(v)
            : "w"(w));
        w -= 0x40;
        asm("tbx %0.16b, {v28.16b-v31.16b}, %1.16b"
            : "+w"(v)
            : "w"(w));
    #endif
...
static inline __attribute__((always_inline)) struct aegis128_state aegis128_update_neon(struct aegis128_state st,
                                                                                        uint8x16_t            m)
{
...
{
    if (!IS_ENABLED(CONFIG_ARM64) || !IS_ENABLED(CONFIG_CC_IS_GCC) || __builtin_expect(aegis128_have_aes_insn, 1))
        return;
                                        ...
                                        unsigned int size)
{
    ...
    if (size > 0)
    {
        ...
        m = s ^ vqtbx1q_u8(s, vld1q_u8(in + size - AEGIS_BLOCK_SIZE), vld1q_u8(permute + 32 - size));
                               ...
                               unsigned int authsize)
{
    ...
    v = st.v[3] ^ (uint8x16_t)vcombine_u64(vmov_n_u64(8ULL * assoclen), vmov_n_u64(8ULL * cryptlen));
