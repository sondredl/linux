                               ...
                               MPI m, unsigned int pbytes)
{
    ...
    {
        /* padding with zero */
        err = crypto_shash_update(desc, zero, pbytes - inlen) ?: crypto_shash_update(desc, in, inlen);
    }
                                     ...
                                     unsigned int pbytes)
{
    ...
    ret = mpi_ec_get_affine(x, y, point, ec) ? -EINVAL : sm2_z_digest_update(desc, x, pbytes) ?
                         ...
                         const void *key, unsigned int keylen, void *dgst)
{
    ...
    err = sm2_z_digest_update(desc, ec->a, pbytes) ?: sm2_z_digest_update(desc, ec->b, pbytes)         ?
...
static int _sm2_verify(struct mpi_ec_ctx *ec, MPI hash, MPI sig_r, MPI sig_s)
{
    ...
    /* r, s in [1, n-1] */
    if (mpi_cmp_ui(sig_r, 1) < 0 || mpi_cmp(sig_r, ec->n) > 0 || mpi_cmp_ui(sig_s, 1) < 0 || mpi_cmp(sig_s, ec->n) > 0)
    {
        goto leave;
    }
