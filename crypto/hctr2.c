                        ...
                        unsigned int keylen)
{
    ...
    crypto_shash_set_flags(tctx->polyval, crypto_skcipher_get_flags(tfm) & CRYPTO_TFM_REQ_MASK);
...
static int hctr2_crypt(struct skcipher_request *req, bool enc)
{
    ...
    skcipher_request_set_callback(&rctx->u.xctr_req,
                                  req->base.flags,
                                  hctr2_xctr_done, req);
    return crypto_skcipher_encrypt(&rctx->u.xctr_req) ?: hctr2_finish(req);
}
...
static int hctr2_init_tfm(struct crypto_skcipher *tfm)
{
    ...
    BUILD_BUG_ON(offsetofend(struct hctr2_request_ctx, u) != sizeof(struct hctr2_request_ctx));
    subreq_size = max(sizeof_field(struct hctr2_request_ctx, u.hash_desc) + crypto_shash_descsize(polyval),
                      sizeof_field(struct hctr2_request_ctx, u.xctr_req) + crypto_skcipher_reqsize(xctr));

    tctx->hashed_tweak_offset = offsetof(struct hctr2_request_ctx, u) + subreq_size;
    crypto_skcipher_set_reqsize(tfm, tctx->hashed_tweak_offset + crypto_shash_statesize(polyval));
    return 0;
                               ...
                               const char             *polyval_name)
{
    ...
    if (snprintf(inst->alg.base.cra_driver_name, CRYPTO_MAX_ALG_NAME,
                 ...
                 polyval_alg->base.cra_driver_name)
        >= CRYPTO_MAX_ALG_NAME)
        goto err_free_inst;

    inst->alg.base.cra_blocksize = BLOCKCIPHER_BLOCK_SIZE;
    inst->alg.base.cra_ctxsize   = sizeof(struct hctr2_tfm_ctx) + polyval_alg->statesize * 2;
     ...
     */
    inst->alg.base.cra_priority = (2 * xctr_alg->base.cra_priority + 4 * polyval_alg->base.cra_priority + blockcipher_alg->cra_priority) / 7;
