                             ...
                             unsigned int keylen)
{
    ...
    skcipher_request_set_callback(&data->req, CRYPTO_TFM_REQ_MAY_SLEEP | CRYPTO_TFM_REQ_MAY_BACKLOG,
                                  crypto_req_done,
                                  &data->wait);
    ...
    crypto_ahash_set_flags(ghash, crypto_aead_get_flags(aead) & CRYPTO_TFM_REQ_MASK);
...
static int crypto_gcm_encrypt(struct aead_request *req)
{
    ...
    return crypto_skcipher_encrypt(skreq) ?: gcm_encrypt_continue(req, flags);
}
...
static int crypto_gcm_init_tfm(struct crypto_aead *tfm)
{
    ...
    crypto_aead_set_reqsize(tfm,
                            align + offsetof(struct crypto_gcm_req_priv_ctx, u) + max(sizeof(struct skcipher_request) + crypto_skcipher_reqsize(ctr), sizeof(struct ahash_request) + crypto_ahash_reqsize(ghash)));
                                    ...
                                    const char             *ghash_name)
{
    ...
    err = -EINVAL;
    if (strncmp(ctr->base.cra_name, "ctr(", 4) != 0 || ctr->ivsize != 16 || ctr->base.cra_blocksize != 1)
        goto err_free_inst;

    err = -ENAMETOOLONG;
    if (snprintf(inst->alg.base.cra_name, CRYPTO_MAX_ALG_NAME,
                 "gcm(%s", ctr->base.cra_name + 4)
        >= CRYPTO_MAX_ALG_NAME)
        goto err_free_inst;
    ...
    inst->alg.base.cra_priority  = (ghash->base.cra_priority + ctr->base.cra_priority) / 2;
                                 ...
                                 unsigned int keylen)
{
    ...
    crypto_aead_set_flags(child, crypto_aead_get_flags(parent) & CRYPTO_TFM_REQ_MASK);
...
static int crypto_rfc4106_init_tfm(struct crypto_aead *tfm)
{
    ...
    crypto_aead_set_reqsize(
        tfm,
        sizeof(struct crypto_rfc4106_req_ctx) + ALIGN(crypto_aead_reqsize(aead), crypto_tfm_ctx_alignment()) + align + 24);
...
static int crypto_rfc4543_crypt(struct aead_request *req, bool enc)
{
    ...
    aead_request_set_crypt(subreq, req->src, req->dst,
                           enc ? 0 : authsize, iv);
    aead_request_set_ad(subreq, req->assoclen + req->cryptlen - subreq->cryptlen);
...
static int crypto_rfc4543_init_tfm(struct crypto_aead *tfm)
{
    ...
    crypto_aead_set_reqsize(
        tfm,
        sizeof(struct crypto_rfc4543_req_ctx) + ALIGN(crypto_aead_reqsize(aead), crypto_tfm_ctx_alignment()) + align + GCM_AES_IV_SIZE);
