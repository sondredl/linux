...
static int lrw_encrypt(struct skcipher_request *req)
{
    ...
    lrw_init_crypt(req);
    return lrw_xor_tweak_pre(req) ?: crypto_skcipher_encrypt(subreq) ?
...
static int lrw_decrypt(struct skcipher_request *req)
{
    ...
    lrw_init_crypt(req);
    return lrw_xor_tweak_pre(req) ?: crypto_skcipher_decrypt(subreq) ?
                                                                     : lrw_xor_tweak_post(req);
}
...
static int lrw_init_tfm(struct crypto_skcipher *tfm)
{
    ...
    crypto_skcipher_set_reqsize(tfm, crypto_skcipher_reqsize(cipher) + sizeof(struct lrw_request_ctx));
...
static int lrw_create(struct crypto_template *tmpl, struct rtattr **tb)
{
    ...
    inst->alg.base.cra_alignmask = alg->base.cra_alignmask | (__alignof__(be128) - 1);
