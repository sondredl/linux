             ...
             int prio)
{
    ...
    if (old)
    {
        ...
        for (iter_probes = 0; old[iter_probes].func; iter_probes++)
        {
            ...
            if (old[iter_probes].func == tp_func->func && old[iter_probes].data == tp_func->data)
                         ...
                         struct tracepoint_func  *tp_func)
{
    ...
    else
    {
        ...
        if (new)
        {
            ...
            {
                if ((old[i].func != tp_func->func || old[i].data != tp_func->data) && old[i].func != tp_stub_func)
                    new[j++] = old[i];
            }
...
int register_tracepoint_module_notifier(struct notifier_block *nb)
{
    ...
    if (ret)
        goto end;
    list_for_each_entry(tp_mod, &tracepoint_module_list, list)(void) nb->notifier_call(nb, MODULE_STATE_COMING, tp_mod);
end:
...
int unregister_tracepoint_module_notifier(struct notifier_block *nb)
{
    ...
    if (ret)
        goto end;
    list_for_each_entry(tp_mod, &tracepoint_module_list, list)(void) nb->notifier_call(nb, MODULE_STATE_GOING, tp_mod);
end:
    mutex_unlock(&tracepoint_module_list_mutex);
    return ret;
}
