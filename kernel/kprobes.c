...
struct kprobe_insn_page
{
    ...
    #define KPROBE_INSN_PAGE_SIZE(slots) \
        (offsetof(struct kprobe_insn_page, slot_used) + (sizeof(char) * (slots)))
...
static struct kprobe *__get_valid_kprobe(struct kprobe *p)
{
    ...
    if (p != ap)
    {
        list_for_each_entry(list_p, &ap->list, list) if (list_p == p)
...
static bool is_cfi_preamble_symbol(unsigned long addr)
{
    ...
    return str_has_prefix("__cfi_", symbuf) || str_has_prefix("__pfx_", symbuf);
}
                                     ...
                                     struct module **probed_mod)
{
    ...
    *probed_mod = NULL;
    ...
    if (in_gate_area_no_mm((unsigned long)p->addr) || within_kprobe_blacklist((unsigned long)p->addr) || jump_label_text_reserved(p->addr, p->addr) || static_call_text_reserved(p->addr, p->addr) || find_bug((unsigned long)p->addr) || is_cfi_preamble_symbol((unsigned long)p->addr))
    {
        ret = -EINVAL;
        goto out;
    }
    ...
    if (IS_ENABLED(CONFIG_MODULES) && *probed_mod)
    {
         ...
         * kprobes in there.
         */
        if (within_module_init((unsigned long)p->addr, *probed_mod) && !module_is_coming(*probed_mod))
        {
...
static bool aggr_kprobe_disabled(struct kprobe *ap)
{
    ...
    list_for_each_entry(kp, &ap->list, list) if (!kprobe_disabled(kp))
        /*
...
int register_kretprobe(struct kretprobe *rp)
{
    ...
    rp->rh = rethook_alloc((void *)rp, kretprobe_rethook_handler,
                           sizeof(struct kretprobe_instance) + rp->data_size, rp->maxactive);
    if (IS_ERR(rp->rh))
                                   ...
                                   unsigned long val, void *data)
{
    ...
    {
        head = &kprobe_table[i];
        hlist_for_each_entry(p, head, hlist) if (within_module_init((unsigned long)p->addr, mod) || (checkcore && within_module_core((unsigned long)p->addr, mod)))
        {
            /*
