...
static int kcov_mmap(struct file *filep, struct vm_area_struct *vma)
{
    ...
    if (kcov->area == NULL || vma->vm_pgoff != 0 || vma->vm_end - vma->vm_start != size)
    {
        res = -EINVAL;
        goto exit;
    }
                             ...
                             unsigned long arg)
{
    ...
    switch (cmd)
    {
        ...
        case KCOV_REMOTE_ENABLE:
            ...
            if (mode < 0)
                return mode;
            if ((unsigned long)remote_arg->area_size > LONG_MAX / sizeof(unsigned long))
                return -EINVAL;
            kcov->mode        = mode;
            t->kcov           = kcov;
...
static long kcov_ioctl(struct file *filep, unsigned int cmd, unsigned long arg)
{
    ...
    switch (cmd)
    {
        case KCOV_INIT_TRACE:
            ...
            return 0;
        case KCOV_REMOTE_ENABLE:
            if (get_user(remote_num_handles, (unsigned __user *)(arg + offsetof(struct kcov_remote_arg, num_handles))))
                return -EFAULT;
                           ...
                           unsigned int dst_area_size, void *src_area)
{
    ...
    if (dst_len > ((dst_area_size * word_size - count_size) >> entry_size_log))
        return;
...
static int __init kcov_init(void)
{
    ...
    {
        void *area = vmalloc_node(CONFIG_KCOV_IRQ_AREA_SIZE * sizeof(unsigned long), cpu_to_node(cpu));
        if (!area)
            return -ENOMEM;
