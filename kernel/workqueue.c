...
#define assert_rcu_or_pool_mutex()                                                  \
    RCU_LOCKDEP_WARN(!rcu_read_lock_any_held() && !lockdep_is_held(&wq_pool_mutex), \
                     "RCU or wq_pool_mutex should be held")

#define assert_rcu_or_wq_mutex_or_pool_mutex(wq)                                                                    \
    RCU_LOCKDEP_WARN(!rcu_read_lock_any_held() && !lockdep_is_held(&wq->mutex) && !lockdep_is_held(&wq_pool_mutex), \
...
#define for_each_pool_worker(worker, pool)                           \
    list_for_each_entry((worker), &(pool)->workers, node) if (({ lockdep_assert_held(&wq_pool_attach_mutex); false; })) \
    {                                                                \
    }                                                                \
    else
...
static struct pool_workqueue *unbound_pwq(struct workqueue_struct *wq, int cpu)
{
    return rcu_dereference_check(*unbound_pwq_slot(wq, cpu),
                                 lockdep_is_held(&wq_pool_mutex) || lockdep_is_held(&wq->mutex));
}
...
{
    return (work_data >> WORK_STRUCT_COLOR_SHIFT) & ((1 << WORK_STRUCT_COLOR_BITS) - 1);
}
...
static void set_work_pwq(struct work_struct *work, struct pool_workqueue *pwq,
                         unsigned long flags)
{
    set_work_data(work, (unsigned long)pwq | WORK_STRUCT_PENDING | WORK_STRUCT_PWQ | flags);
}

static void set_work_pool_and_keep_pending(struct work_struct *work,
                                           int pool_id, unsigned long flags)
{
    set_work_data(work, ((unsigned long)pool_id << WORK_OFFQ_POOL_SHIFT) | WORK_STRUCT_PENDING | flags);
}
                                            ...
                                            int pool_id, unsigned long flags)
{
     ...
     */
    smp_wmb();
    set_work_data(work, ((unsigned long)pool_id << WORK_OFFQ_POOL_SHIFT) | flags);
    /*
...
{
    return ((unsigned long)offqd->disable << WORK_OFFQ_DISABLE_SHIFT) | ((unsigned long)offqd->flags);
}
...
static void worker_enter_idle(struct worker *worker)
{
    ...
    if (WARN_ON_ONCE(worker->flags & WORKER_IDLE) || WARN_ON_ONCE(!list_empty(&worker->entry) && (worker->hentry.next || worker->hentry.pprev)))
        return;
                                                 ...
                                                 struct work_struct *work)
{
    ...
    hash_for_each_possible(pool->busy_hash, worker, hentry,
                           (unsigned long)work) if (worker->current_work == work && worker->current_func == work->func) return worker;
...
restart:
    ...
    if (ent)
    {
        ...
        if (wq_cpu_intensive_warning_thresh && cnt >= wq_cpu_intensive_warning_thresh && is_power_of_2(cnt + 1 - wq_cpu_intensive_warning_thresh))
...
void wq_worker_tick(struct task_struct *task)
{
     ...
     */
    if ((worker->flags & WORKER_NOT_RUNNING) || READ_ONCE(worker->sleeping) || worker->task->se.sum_exec_runtime - worker->current_at < wq_cpu_intensive_thresh_us * NSEC_PER_USEC)
        return;
                         ...
                         struct work_struct *work)
{
     ...
     */
    if (unlikely(wq->flags & (__WQ_DESTROYING | __WQ_DRAINING) && WARN_ON_ONCE(!is_chained_work(wq))))
        return;
    rcu_read_lock();
retry:
...
static bool clear_pending_if_disabled(struct work_struct *work)
{
    ...
    if (likely((data & WORK_STRUCT_PWQ) || !(data & WORK_OFFQ_DISABLE_MASK)))
        return false;
...
bool queue_rcu_work(struct workqueue_struct *wq, struct rcu_work *rwork)
{
     ...
     */
    if (!test_and_set_bit(WORK_STRUCT_PENDING_BIT, work_data_bits(work)) && !WARN_ON_ONCE(clear_pending_if_disabled(work)))
    {
        rwork->wq = wq;
...
static void set_worker_dying(struct worker *worker, struct list_head *list)
{
    ...
    /* sanity check frenzy */
    if (WARN_ON(worker->current_work) || WARN_ON(!list_empty(&worker->scheduled)) || WARN_ON(!(worker->flags & WORKER_IDLE)))
        return;
        ...
        __acquires(&pool->lock)
{
    ...
    WARN_ON_ONCE(!(pool->flags & POOL_DISASSOCIATED) && raw_smp_processor_id() != pool->cpu);
    ...
    if (unlikely((worker->task && in_atomic()) || lockdep_depth(current) != lockdep_start_depth || rcu_preempt_depth() != rcu_start_depth))
    {
        pr_err(
            "BUG: workqueue leaked atomic, lock or RCU: %s[%d]\n"
                                   ...
                                   struct work_struct      *target_work)
{
    ...
    WARN_ONCE(worker && ((worker->current_pwq->wq->flags & (WQ_MEM_RECLAIM | __WQ_LEGACY)) == WQ_MEM_RECLAIM),
...
static bool __flush_work(struct work_struct *work, bool from_cancel)
{
    ...
    if (from_cancel && !WARN_ON_ONCE(data & WORK_STRUCT_PWQ) && (data & WORK_OFFQ_BH))
    {
        /*
    ...
    wqattrs_pod_type(const struct workqueue_attrs *attrs)
{
    ...
    if (!WARN_ON_ONCE(attrs->affn_scope == WQ_AFFN_NR_TYPES) && likely(pt->nr_pods))
        return pt;
                          ...
                          const cpumask_var_t           unbound_cpumask)
{
    ...
    if (WARN_ON(attrs->affn_scope < 0 || attrs->affn_scope >= WQ_AFFN_NR_TYPES))
...
static int alloc_and_link_pwqs(struct workqueue_struct *wq)
{
    ...
    if (wq->flags & __WQ_ORDERED)
    {
        ...
        WARN(!ret && (wq->pwqs.next != &dfl_pwq->pwqs_node || wq->pwqs.prev != &dfl_pwq->pwqs_node),
...
void workqueue_set_min_active(struct workqueue_struct *wq, int min_active)
{
    ...
    if (WARN_ON((wq->flags & (WQ_BH | WQ_UNBOUND | __WQ_ORDERED)) != WQ_UNBOUND))
        return;
                             ...
                             const char *buf, size_t count)
{
    ...
    if (sscanf(buf, "%d", &attrs->nice) == 1 && attrs->nice >= MIN_NICE && attrs->nice <= MAX_NICE)
        ret = apply_workqueue_attrs_locked(wq, attrs);
    else
        ret = -EINVAL;
...
void __init workqueue_init_early(void)
{
    ...
    system_bh_highpri_wq                = alloc_workqueue("events_bh_highpri",
                                                          WQ_BH | WQ_HIGHPRI, 0);
    BUG_ON(!system_wq || !system_highpri_wq || !system_long_wq || !system_unbound_wq || !system_freezable_wq || !system_power_efficient_wq || !system_freezable_power_efficient_wq || !system_bh_wq || !system_bh_highpri_wq);
}
