                                      ...
                                      const struct watch_notification *n)
{
    ...
    {
        wt = &wf->filters[i];
        if (n->type == wt->type && (wt->subtype_filter[st_index] & st_bit) && (n->info & wt->info_mask) == wt->info_filter)
            return true;
    }
...
long watch_queue_set_size(struct pipe_inode_info *pipe, unsigned int nr_notes)
{
    ...
    if (nr_notes < 1 || nr_notes > 512) /* TODO: choose a better hard limit */
        return -EINVAL;
    ...
    if (nr_pages > pipe->max_usage && (too_many_pipe_buffers_hard(user_bufs) || too_many_pipe_buffers_soft(user_bufs)) && pipe_is_unprivileged_user())
    {
        ret = -EPERM;
        goto error;
    }
                            ...
                            struct watch_notification_filter __user *_filter)
{
    ...
    if (copy_from_user(&filter, _filter, sizeof(filter)) != 0)
        return -EFAULT;
    if (filter.nr_filters == 0 || filter.nr_filters > 16 || filter.__reserved != 0)
        return -EINVAL;
    ...
    {
        if ((tf[i].info_filter & ~tf[i].info_mask) || tf[i].info_mask & WATCH_INFO_LENGTH)
            goto err_filter;
...
again:
    ...
    {
        if (all || (watch->id == id && rcu_access_pointer(watch->queue) == wq))
            goto found;
    }
    spin_unlock(&wlist->lock);
    goto out;
