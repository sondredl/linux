                                ...
                                unsigned long *offset)
{
    ...
    return !!module_address_lookup(addr, symbolsize, offset, NULL, NULL, namebuf) || !!__bpf_address_lookup(addr, symbolsize, offset, namebuf);
}
...
static int update_iter_mod(struct kallsym_iter *iter, loff_t pos)
{
    ...
    if ((!iter->pos_mod_end || iter->pos_mod_end > pos) && get_ksymbol_mod(iter))
        return 1;

    if ((!iter->pos_ftrace_mod_end || iter->pos_ftrace_mod_end > pos) && get_ksymbol_ftrace_mod(iter))
        return 1;

    if ((!iter->pos_bpf_end || iter->pos_bpf_end > pos) && get_ksymbol_bpf(iter))
        return 1;
