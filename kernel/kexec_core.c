...
int sanity_check_segment_list(struct kimage *image)
{
    ...
    if (image->type == KEXEC_TYPE_CRASH)
    {
        for (i = 0; i < nr_segments; i++)
        {
            ...
            if ((mstart < phys_to_boot_phys(crashk_res.start)) || (mend > phys_to_boot_phys(crashk_res.end)))
                return -EADDRNOTAVAIL;
        }
    }
#endif
                                                      ...
                                                      unsigned int   order)
{
    ...
    do
    {
        ...
        if ((epfn >= (KEXEC_CONTROL_MEMORY_LIMIT >> PAGE_SHIFT)) || kimage_is_destination_range(image, addr, eaddr))
        {
...
#define for_each_kimage_entry(image, ptr, entry)                    \
    for (ptr = &image->head; (entry = *ptr) && !(entry & IND_DONE); \
         ptr = (entry & IND_INDIRECTION) ? boot_phys_to_virt((entry & PAGE_MASK)) : ptr + 1)
                                      ...
                                      unsigned long  destination)
{
    ...
    while (1)
    {
        ...
        if (page_to_boot_pfn(page) > (KEXEC_SOURCE_MEMORY_LIMIT >> PAGE_SHIFT))
        {
...
bool kexec_load_permitted(int kexec_image_type)
{
    ...
    limit = (kexec_image_type == KEXEC_TYPE_CRASH) ? &load_limit_panic : &load_limit_reboot;
    mutex_lock(&limit->mutex);
    if (!limit->limit)
    {
        mutex_unlock(&limit->mutex);
