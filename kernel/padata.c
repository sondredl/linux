...
void __init padata_do_multithreaded(struct padata_mt_job *job)
{
    ...
    list_for_each_entry(pw, &works, pw_list) if (job->numa_aware)
    {
...
static int padata_replace(struct padata_instance *pinst)
{
    ...
    list_for_each_entry_continue_reverse(ps, &pinst->pslist, list) if (refcount_dec_and_test(&ps->opd->refcnt))
...
{
    return cpumask_test_cpu(cpu, pinst->cpumask.pcpu) || cpumask_test_cpu(cpu, pinst->cpumask.cbcpu);
}
                             ...
                             const char *buf, size_t count)
{
    ...
    mask_type = !strcmp(attr->name, "serial_cpumask") ? PADATA_CPU_SERIAL : PADATA_CPU_PARALLEL;
...
struct padata_instance *padata_alloc(const char *name)
{
    ...
    pinst->serial_wq = alloc_workqueue("%s_serial", WQ_MEM_RECLAIM | WQ_CPU_INTENSIVE, 1, name);
    if (!pinst->serial_wq)
        goto err_put_cpus;
