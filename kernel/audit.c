...
static void kauditd_hold_skb(struct sk_buff *skb, int error)
{
    ...
    if (error == -EAGAIN)
    {
        if (!audit_backlog_limit || skb_queue_len(&audit_retry_queue) < audit_backlog_limit)
        {
    ...
    if (!audit_backlog_limit || skb_queue_len(&audit_hold_queue) < audit_backlog_limit)
    {
        skb_queue_tail(&audit_hold_queue, skb);
        return;
    }
                              ...
                              void (*err_hook)(struct sk_buff *skb, int error))
{
    ...
    retry:
        ...
        if (rc < 0)
        {
            ...
            if (++failed >= retry_limit || rc == -ECONNREFUSED || rc == -EPERM)
            {
                sk = NULL;
                if (err_hook)
                    (*err_hook)(skb, rc);
                             ...
                             bool *ack)
{
    ...
    switch (msg_type)
    {
        ...
        case AUDIT_TTY_SET:
        {
            ...
            if ((s.enabled != 0 && s.enabled != 1) || (s.log_passwd != 0 && s.log_passwd != 1))
                err = -EINVAL;
...
static void audit_receive(struct sk_buff *skb)
{
    ...
    if (audit_backlog_limit && (skb_queue_len(&audit_queue) > audit_backlog_limit))
    {
                                     ...
                                     int type)
{
    ...
    if (!(auditd_test_task(current) || audit_ctl_owner_current()))
    {
        ...
        while (audit_backlog_limit && (skb_queue_len(&audit_queue) > audit_backlog_limit))
        {
            /* wake kauditd to try and flush the queue */
            wake_up_interruptible(&kauditd_wait);
...
int audit_signal_info(int sig, struct task_struct *t)
{
    ...
    if (auditd_test_task(t) && (sig == SIGTERM || sig == SIGHUP || sig == SIGUSR1 || sig == SIGUSR2))
    {
