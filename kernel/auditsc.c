...
static int audit_match_perm(struct audit_context *ctx, int mask)
{
    ...
    {
        case AUDITSC_NATIVE:
            if ((mask & AUDIT_PERM_WRITE) && audit_match_class(AUDIT_CLASS_WRITE, n))
                return 1;
            if ((mask & AUDIT_PERM_READ) && audit_match_class(AUDIT_CLASS_READ, n))
                return 1;
            if ((mask & AUDIT_PERM_ATTR) && audit_match_class(AUDIT_CLASS_CHATTR, n))
                return 1;
            return 0;
        case AUDITSC_COMPAT: /* 32bit on biarch */
            if ((mask & AUDIT_PERM_WRITE) && audit_match_class(AUDIT_CLASS_WRITE_32, n))
                return 1;
            if ((mask & AUDIT_PERM_READ) && audit_match_class(AUDIT_CLASS_READ_32, n))
                return 1;
            if ((mask & AUDIT_PERM_ATTR) && audit_match_class(AUDIT_CLASS_CHATTR_32, n))
                return 1;
            return 0;
        case AUDITSC_OPEN:
    ...
    for (i = 0; i < rule->field_count; i++)
    {
        ...
        switch (f->type)
        {
            ...
            case AUDIT_DEVMAJOR:
                if (name)
                {
                    if (audit_comparator(MAJOR(name->dev), f->op, f->val) || audit_comparator(MAJOR(name->rdev), f->op, f->val))
                        ++result;
                }
                else if (ctx)
                {
            ...
            case AUDIT_DEVMINOR:
                if (name)
                {
                    if (audit_comparator(MINOR(name->dev), f->op, f->val) || audit_comparator(MINOR(name->rdev), f->op, f->val))
                        ++result;
                }
                else if (ctx)
                {
    ...
    do
    {
        ...
        if (require_data)
        {
            ...
            else
            {
                ...
                if (len_full < len_max)
                    len_full = (encode ? len_tmp * 2 : len_tmp);
                p += len_tmp + 1;
            }
            len_buf += len_tmp;
            buf_head[len_buf] = '\0';
        ...
        if (len_buf >= 0)
        {
            ...
            len_tmp = 0;
            if (require_data || (iter > 0) || ((len_abuf + sizeof(abuf)) > len_rem))
            {
                if (iter == 0)
                {
...
static void audit_log_uring(struct audit_context *ctx)
{
    ...
    if (ctx->return_valid != AUDITSC_INVALID)
        audit_log_format(ab, " success=%s exit=%ld",
                         (ctx->return_valid == AUDITSC_SUCCESS ? "yes" : "no"),
                         ctx->return_code);
    audit_log_format(ab,
                     " items=%d"
...
static void audit_log_exit(void)
{
    ...
    if (context->target_pid && audit_log_pid_context(context, context->target_pid, context->target_auid, context->target_uid, context->target_sessionid, context->target_sid, context->target_comm))
        call_panic = 1;
                               ...
                               int success, long code)
{
     ...
     */
    if (unlikely(code <= -ERESTARTSYS) && (code >= -ERESTART_RESTARTBLOCK) && (code != -ENOIOCTLCMD))
                   ...
                   unsigned int flags)
{
    ...
    if (n)
    {
        if (parent)
        {
            if (n->type == AUDIT_TYPE_PARENT || n->type == AUDIT_TYPE_UNKNOWN)
                goto out;
        }
        else
        {
    ...
    list_for_each_entry_reverse(n, &context->names_list, list)
    {
        if (n->ino)
        {
            ...
            if (n->ino != inode->i_ino || n->dev != inode->i_sb->s_dev)
                continue;
        }
        else if (n->name)
        {
                         ...
                         const unsigned char  type)
{
    ...
    list_for_each_entry(n, &context->names_list, list)
    {
        ...
        if (n->ino == parent->i_ino && n->dev == parent->i_sb->s_dev && !audit_compare_dname_path(dname, n->name->name, n->name_len))
        {
    ...
    list_for_each_entry(n, &context->names_list, list)
    {
        ...
        if (!strcmp(dname->name, n->name->name) || !audit_compare_dname_path(dname, n->name->name, found_parent ? found_parent->name_len : AUDIT_NAME_FULL))
        {
            if (n->type == AUDIT_TYPE_UNKNOWN)
                n->type = type;
            found_child = n;
