...
static inline bool need_counting_irqs(void)
{
    return false;
}
                         ...
                         unsigned long now)
{
    if ((watchdog_enabled & WATCHDOG_SOFTOCKUP_ENABLED) && watchdog_thresh)
    {
         ...
         */
        if (time_after_eq(now, period_ts + get_softlockup_thresh() / NUM_SAMPLE_PERIODS) && need_counting_irqs())
            start_counting_irqs();
...
static __init void lockup_detector_setup(void)
{
    ...
    if (!IS_ENABLED(CONFIG_SYSCTL) && !(watchdog_enabled && watchdog_thresh))
        return;
...
static int proc_watchdog(struct ctl_table *table, int write,
                         void *buffer, size_t *lenp, loff_t *ppos)
{
    return proc_watchdog_common(WATCHDOG_HARDLOCKUP_ENABLED | WATCHDOG_SOFTOCKUP_ENABLED,
