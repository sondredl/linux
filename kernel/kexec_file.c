    ...
    kimage_validate_signature(struct kimage *image)
{
    ...
    if (ret)
    {
         ...
         * down.
         */
        if (!ima_appraise_signature(READING_KEXEC_IMAGE) && security_locked_down(LOCKDOWN_KEXEC))
            return -EPERM;
...
SYSCALL_DEFINE5(kexec_file_load, int, kernel_fd, int, initrd_fd,
                ...
                unsigned long, flags)
{
    int             image_type = (flags & KEXEC_FILE_ON_CRASH) ? KEXEC_TYPE_CRASH : KEXEC_TYPE_DEFAULT;
                                         ...
                                         struct kexec_buf      *kbuf)
{
    ...
    for (i = 0; i < pi->ehdr->e_shnum; i++)
    {
         ...
         */
        if (sechdrs[i].sh_flags & SHF_EXECINSTR && pi->ehdr->e_entry >= sechdrs[i].sh_addr && pi->ehdr->e_entry < (sechdrs[i].sh_addr + sechdrs[i].sh_size) && !WARN_ON(kbuf->image->start != pi->ehdr->e_entry))
        {
                                                  ...
                                                  const char            *name)
{
    ...
    for (i = 0; i < ehdr->e_shnum; i++)
    {
        ...
        for (k = 0; k < sechdrs[i].sh_size / sizeof(Elf_Sym); k++)
        {
            ...
            if (syms[k].st_shndx == SHN_UNDEF || syms[k].st_shndx >= ehdr->e_shnum)
            {
