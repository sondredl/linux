...
{
    if ((krule->listnr != AUDIT_FILTER_EXIT && krule->listnr != AUDIT_FILTER_URING_EXIT) || krule->inode_f || krule->watch || krule->tree || (f->op != Audit_equal && f->op != Audit_not_equal))
        return -EINVAL;
...
static int audit_field_valid(struct audit_entry *entry, struct audit_field *f)
{
    ...
    {
        case AUDIT_MSGTYPE:
            if (entry->rule.listnr != AUDIT_FILTER_EXCLUDE && entry->rule.listnr != AUDIT_FILTER_USER)
                return -EINVAL;
            break;
        case AUDIT_FSTYPE:
...
static int audit_compare_rule(struct audit_krule *a, struct audit_krule *b)
{
    ...
    if (a->flags != b->flags || a->pflags != b->pflags || a->listnr != b->listnr || a->action != b->action || a->field_count != b->field_count)
        return 1;
                                           ...
                                           struct list_head  **p)
{
    ...
    else if (entry->rule.watch)
    {
        ...
        for (h = 0; h < AUDIT_INODE_BUCKETS; h++)
        {
            ...
            list_for_each_entry(e, list, list) if (!audit_compare_rule(&entry->rule, &e->rule))
            {
                found = e;
                goto out;
            }
...
static inline int audit_add_rule(struct audit_entry *entry)
{
    ...
    if (entry->rule.listnr == AUDIT_FILTER_EXIT || entry->rule.listnr == AUDIT_FILTER_URING_EXIT)
    {
