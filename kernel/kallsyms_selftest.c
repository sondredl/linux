...
static int find_symbol(void *data, const char *name, unsigned long addr)
{
    ...
    if (strcmp(name, stat->name) == 0 || (!stat->perf && match_cleanup_name(name, stat->name)))
    {
        stat->real_cnt++;
        stat->addr = addr;
...
static int test_kallsyms_basic_function(void)
{
    ...
    for (i = 0; i < kallsyms_num_syms; i++)
    {
        ...
        if (i >= next)
        {
             ...
             */
            if (stat->addr != stat2->addr || stat->real_cnt != stat2->real_cnt || memcmp(stat->addrs, stat2->addrs, stat->save_cnt * sizeof(stat->addrs[0])))
            {
