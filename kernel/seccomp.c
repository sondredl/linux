...
static inline pid_t seccomp_can_sync_threads(void)
{
    ...
    for_each_thread(caller, thread)
    {
        ...
        if (thread->seccomp.mode == SECCOMP_MODE_DISABLED || (thread->seccomp.mode == SECCOMP_MODE_FILTER && is_ancestor(thread->seccomp.filter, caller->seccomp.filter)))
            continue;
...
static u32 seccomp_actions_logged = SECCOMP_LOG_KILL_PROCESS | SECCOMP_LOG_KILL_THREAD | SECCOMP_LOG_TRAP | SECCOMP_LOG_ERRNO | SECCOMP_LOG_USER_NOTIF | SECCOMP_LOG_TRACE | SECCOMP_LOG_LOG;
...
void secure_computing_strict(int this_syscall)
{
    ...
    if (IS_ENABLED(CONFIG_CHECKPOINT_RESTORE) && unlikely(current->ptrace & PT_SUSPEND_SECCOMP))
        return;
                            ...
                            const bool recheck_after_trace)
{
    ...
    switch (action)
    {
        ...
        default:
            ...
            if (action != SECCOMP_RET_KILL_THREAD || (atomic_read(&current->signal->live) == 1))
            {
                                ...
                                void __user           *buf)
{
    ...
    if ((resp.flags & SECCOMP_USER_NOTIF_FLAG_CONTINUE) && (resp.error || resp.val))
        return -EINVAL;
                                    ...
                                    const char __user *filter)
{
     ...
     */
    if ((flags & SECCOMP_FILTER_FLAG_TSYNC) && (flags & SECCOMP_FILTER_FLAG_NEW_LISTENER) && ((flags & SECCOMP_FILTER_FLAG_TSYNC_ESRCH) == 0))
        return -EINVAL;
     ...
     */
    if ((flags & SECCOMP_FILTER_FLAG_WAIT_KILLABLE_RECV) && ((flags & SECCOMP_FILTER_FLAG_NEW_LISTENER) == 0))
        return -EINVAL;
     ...
     */
    if (flags & SECCOMP_FILTER_FLAG_TSYNC && mutex_lock_killable(&current->signal->cred_guard_mutex))
        goto out_put_fd;
...
static const char seccomp_actions_avail[] =
    SECCOMP_RET_KILL_PROCESS_NAME " " SECCOMP_RET_KILL_THREAD_NAME " " SECCOMP_RET_TRAP_NAME " " SECCOMP_RET_ERRNO_NAME " " SECCOMP_RET_USER_NOTIF_NAME " " SECCOMP_RET_TRACE_NAME " " SECCOMP_RET_LOG_NAME " " SECCOMP_RET_ALLOW_NAME;
