...
static void synchronize_group_exit(struct task_struct *tsk, long code)
{
    ...
    if ((signal->quick_threads == 0) && !(signal->flags & SIGNAL_GROUP_EXIT))
    {
                                 ...
                                 struct task_struct *target)
{
    ...
    return current == parent || (!(wo->wo_flags & __WNOTHREAD) && same_thread_group(current, parent));
}
...
long __do_wait(struct wait_opts *wo)
{
    ...
    if ((wo->wo_type < PIDTYPE_MAX) && (!wo->wo_pid || !pid_has_task(wo->wo_pid, wo->wo_type)))
        goto notask;
