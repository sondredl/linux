                                 ...
                                 int (*func)(struct resource *, void *))
{
    ...
    while (start < end && !find_next_iomem_res(start, end, flags, desc, &res))
    {
        ret = (*func)(&res, arg);
        if (ret)
            break;
                            ...
                            int (*func)(struct resource *, void *))
{
    ...
    i     = 0;
    while ((start < end) && (!find_next_iomem_res(start, end, flags, IORES_DESC_NONE, &res)))
    {
        if (i >= rams_size)
        {
            /* re-alloc */
...
static void revoke_iomem(struct resource *res)
{
     ...
     */
    if (devmem_is_allowed(PHYS_PFN(res->start)) && devmem_is_allowed(PHYS_PFN(res->end)))
    {
        /*
                                           ...
                                           struct resource *r2)
{
    ...
    return r1->flags == r2->flags && r1->end + 1 == r2->start && r1->name == r2->name && r1->desc == r2->desc && !r1->child && !r2->child;
}
...
int iomem_map_sanity_check(resource_size_t addr, unsigned long size)
{
    ...
    for_each_resource(&iomem_resource, p, false)
    {
        ...
        if (p->end < addr)
            continue;
        if (PFN_DOWN(p->start) <= PFN_DOWN(addr) && PFN_DOWN(p->end) >= PFN_DOWN(end))
            continue;
        /*
...
{
    const unsigned int exclusive_system_ram = IORESOURCE_SYSTEM_RAM | IORESOURCE_EXCLUSIVE;
                         ...
                         resource_size_t size, unsigned long flags)
{
     ...
     * @size did not wrap 0.
     */
    return addr > addr - size && addr <= min_t(resource_size_t, base->end, (1ULL << MAX_PHYSMEM_BITS) - 1);
}
                        ...
                        const unsigned long flags)
{
    ...
    {
        if (__region_intersects(base, addr, size, 0, IORES_DESC_NONE) != REGION_DISJOINT)
            continue;
