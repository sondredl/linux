...
static void scf_torture_stats_print(void)
{
    ...
    if (atomic_read(&n_errs) || atomic_read(&n_mb_in_errs) || atomic_read(&n_mb_out_errs) || (!IS_ENABLED(CONFIG_KASAN) && atomic_read(&n_alloc_errs)))
        bangstr = "!!! ";
...
static void scf_sel_add(unsigned long weight, int prim, bool wait)
{
    ...
    if (!weight || WARN_ON_ONCE(scf_sel_array_len >= ARRAY_SIZE(scf_sel_array)) || WARN_ON_ONCE(0 - 100000 * weight <= 100000 * scf_sel_totweight) || WARN_ON_ONCE(prim >= ARRAY_SIZE(scf_prim_name)))
        return;
...
static int __init scf_torture_init(void)
{
    ...
    if (weight_resched <= 0 && weight_single <= 0 && weight_single_rpc <= 0 && weight_single_wait <= 0 && weight_many <= 0 && weight_many_wait <= 0 && weight_all <= 0 && weight_all_wait <= 0)
    {
    ...
    if (weight_resched1 == 0 && weight_single1 == 0 && weight_single_rpc1 == 0 && weight_single_wait1 == 0 && weight_many1 == 0 && weight_many_wait1 == 0 && weight_all1 == 0 && weight_all_wait1 == 0)
    {
