...
void __smp_call_single_queue(int cpu, struct llist_node *node)
{
    ...
    if (trace_csd_queue_cpu_enabled())
    {
        ...
        func = CSD_TYPE(csd) == CSD_TYPE_TTWU ? sched_ttwu_pending : csd->func;
...
static void __flush_smp_call_function_queue(bool warn_cpu_offline)
{
    ...
    if (unlikely(warn_cpu_offline && !cpu_online(smp_processor_id()) && !warned && entry != NULL))
    {
        warned = true;
                             ...
                             int wait)
{
    ...
    call_single_data_t  csd_stack = {
         .node = {
             .u_flags = CSD_FLAG_LOCK | CSD_TYPE_SYNC,
        },
    };
    int this_cpu;
    int err;
