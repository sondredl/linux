...
void relay_reset(struct rchan *chan)
{
    ...
    for_each_possible_cpu(i) if ((buf = *per_cpu_ptr(chan->buf, i)))
        __relay_reset(buf, 0);
    ...
    if (buf->offset != buf->chan->subbuf_size + 1)
    {
        ...
        if (buf->dentry)
            d_inode(buf->dentry)->i_size +=
                buf->chan->subbuf_size - buf->padding[old_subbuf];
        else
            buf->early_bytes += buf->chan->subbuf_size - buf->padding[old_subbuf];
        smp_mb();
                                    ...
                                    size_t            bytes_consumed)
{
    ...
    if (buf->subbufs_produced == buf->subbufs_consumed && buf->offset == buf->bytes_consumed)
        return;
    ...
    {
        if ((read_subbuf == buf->subbufs_produced % n_subbufs) && (buf->offset == subbuf_size))
            return;
        relay_subbufs_consumed(buf->chan, buf->cpu, 1);
        buf->bytes_consumed = 0;
    }
