...
#define AUDIT_FS_WATCH (FS_MOVE | FS_CREATE | FS_DELETE | FS_DELETE_SELF | FS_MOVE_SELF | FS_UNMOUNT)
...
int audit_to_watch(struct audit_krule *krule, char *path, int len, u32 op)
{
    ...
    if (path[0] != '/' || path[len - 1] == '/' || (krule->listnr != AUDIT_FILTER_EXIT && krule->listnr != AUDIT_FILTER_URING_EXIT) || op != Audit_equal || krule->inode_f || krule->watch || krule->tree)
        return -EINVAL;
