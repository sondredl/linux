...
static unsigned long enforced_nproc_rlimit(void)
{
    ...
    if (!uid_eq(current_uid(), GLOBAL_ROOT_UID) || (current_user_ns() != &init_user_ns))
...
static int cmp_map_id(const void *k, const void *e)
{
    ...
    if (key->id >= first && key->id <= last && (id2 >= first && id2 <= last))
        return 0;
                             ...
                             struct uid_gid_extent *extent)
{
    ...
    for (idx = 0; idx < new_map->nr_extents; idx++)
    {
        ...
        if ((prev_upper_first <= upper_last) && (prev_upper_last >= upper_first))
            return true;

        /* Does the lower range intersect a previous extent? */
        if ((prev_lower_first <= lower_last) && (prev_lower_last >= lower_first))
            return true;
    }
    return false;
}
                         ...
                         struct uid_gid_map *parent_map)
{
    ...
    for (; pos; pos = next_line)
    {
        ...
        if ((new_map.nr_extents + 1) == UID_GID_MAP_MAX_EXTENTS && (next_line != NULL))
            goto out;
...
int proc_setgroups_show(struct seq_file *seq, void *v)
{
    ...
    seq_printf(seq, "%s\n",
               (userns_flags & USERNS_SETGROUPS_ALLOWED) ? "allow" : "deny");
    return 0;
}
