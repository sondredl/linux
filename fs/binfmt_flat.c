                           ...
                           long len, int fd)
{
    ...
    /* Check gzip flags */
    if ((buf[3] & ENCRYPTED) || (buf[3] & CONTINUATION) || (buf[3] & RESERVED))
    {
                          ...
                          struct lib_info *libinfo, unsigned long *extra_stack)
{
     ...
     */
    if (rev == OLD_FLAT_VERSION && (flags || IS_ENABLED(CONFIG_BINFMT_FLAT_OLD_ALWAYS_RAM)))
        flags = FLAT_FLAG_RAM;
    ...
    if (!IS_ENABLED(CONFIG_MMU) && !(flags & (FLAT_FLAG_RAM | FLAT_FLAG_GZIP)))
    {
        ...
        len           = data_len + extra + DATA_START_OFFSET_WORDS * sizeof(unsigned long);
        len           = PAGE_ALIGN(len);
        ...
        datapos = ALIGN(realdatastart + DATA_START_OFFSET_WORDS * sizeof(unsigned long),
                        FLAT_DATA_ALIGN);
    ...
    else
    {
        ...
        len     = text_len + data_len + extra + DATA_START_OFFSET_WORDS * sizeof(u32);
        len     = PAGE_ALIGN(len);
        ...
        if (flags & FLAT_FLAG_GZIP)
        {
            ...
            else
            {
                result = decompress_exec(bprm, sizeof(struct flat_hdr),
                                         unz_data, unz_len, 0);
                if (result == 0 && (copy_to_user((void __user *)textpos + sizeof(struct flat_hdr), unz_data, unz_text_len) || copy_to_user((void __user *)datapos, unz_data + unz_text_len, full_data)))
                    result = -EFAULT;
                vfree(unz_data);
            }
    #endif
        ...
        else if (flags & FLAT_FLAG_GZDATA)
        {
            ...
            if (!IS_ERR_VALUE(result))
            {
                ...
                else
                {
                    ...
                    if (result == 0 && copy_to_user((void __user *)datapos, unz_data, full_data))
                        result = -EFAULT;
                    vfree(unz_data);
                }
    #endif
        ...
        if (IS_ERR_VALUE(result))
        {
            ...
            vm_munmap(textpos, text_len + data_len + extra + DATA_START_OFFSET_WORDS * sizeof(u32));
            goto err;
        }
    }
    ...
    if (clear_user((void __user *)(datapos + data_len), bss_len + (memp + memp_size - stack_len - /* end brk */
...
static int load_flat_binary(struct linux_binprm *bprm)
{
    ...
    for (i = 0; i < MAX_SHARED_LIBS; i++)
    {
        ...
        {
            unsigned long         val = libinfo.lib_list[j].loaded ? libinfo.lib_list[j].start_data : UNLOADED_LIB;
