...
static int load_elf_fdpic_binary(struct linux_binprm *bprm)
{
    ...
    if (executable_stack == EXSTACK_ENABLE_X || (executable_stack == EXSTACK_DEFAULT && VM_STACK_FLAGS & VM_EXEC))
                                   ...
                                   struct elf_fdpic_params *interp_params)
{
    ...
    {
        len = sizeof(struct elf_fdpic_loadmap);
        len += sizeof(struct elf_fdpic_loadseg) * interp_params->loadmap->nsegs;
        sp                      = (sp - len) & ~7UL;
                              ...
                              const char              *what)
{
    ...
    for (loop = 0; loop < params->hdr.e_phnum; loop++, phdr++)
    {
        ...
        for (loop = loadmap->nsegs; loop > 0; loop--, seg++)
        {
            ...
            {
                params->ph_addr =
                    (phdr->p_vaddr - seg->p_vaddr) + seg->addr + params->hdr.e_phoff - phdr->p_offset;
                break;
            }
        }
        break;
    ...
    for (loop = 0; loop < params->hdr.e_phnum; loop++, phdr++)
    {
        ...
        for (loop = loadmap->nsegs; loop > 0; loop--, seg++)
        {
            if (phdr->p_vaddr >= seg->p_vaddr && phdr->p_vaddr + phdr->p_memsz <= seg->p_vaddr + seg->p_memsz)
            {
                ...
                params->dynamic_addr =
                    (phdr->p_vaddr - seg->p_vaddr) + seg->addr;

                /* check the dynamic section contains at least
                 * one item, and that the last item is a NULL
                 * entry */
                if (phdr->p_memsz == 0 || phdr->p_memsz % sizeof(Elf_Dyn) != 0)
                    goto dynamic_error;

                tmp = phdr->p_memsz / sizeof(Elf_Dyn);
                dyn = (Elf_Dyn __user *)params->dynamic_addr;
                if (get_user(d_tag, &dyn[tmp - 1].d_tag) || d_tag != 0)
                    goto dynamic_error;
                break;
            }
        }
    ...
    for (loop = 1; loop < nloads; loop++)
    {
        ...
        if (seg->p_vaddr - mseg->p_vaddr == seg->addr - mseg->addr)
        {
            ...
            {
                mseg->p_memsz +=
                    load_addr - (mseg->addr + mseg->p_memsz);
