...
struct mount *__lookup_mnt(struct vfsmount *mnt, struct dentry *dentry)
{
    ...
    hlist_for_each_entry_rcu(p, head, mnt_hash) if (&p->mnt_parent->mnt == mnt && p->mnt_mountpoint == dentry) return p;
    return NULL;
}
...
static int do_umount(struct mount *mnt, int flags)
{
    ...
    if (flags & MNT_DETACH)
    {
        if (mnt->mnt.mnt_flags & MNT_ONRB || !list_empty(&mnt->mnt_list))
...
#define FSMOUNT_VALID_FLAGS \
    (MOUNT_ATTR_RDONLY | MOUNT_ATTR_NOSUID | MOUNT_ATTR_NODEV | MOUNT_ATTR_NOEXEC | MOUNT_ATTR__ATIME | MOUNT_ATTR_NODIRATIME | MOUNT_ATTR_NOSYMFOLLOW)
                ...
                unsigned int, flags)
{
    ...
    if ((flags & (MOVE_MOUNT_BENEATH | MOVE_MOUNT_SET_GROUP)) == (MOVE_MOUNT_BENEATH | MOVE_MOUNT_SET_GROUP))
        return -EINVAL;
...
{
    return (!(kattr->attr_set & MNT_READONLY) || (mnt->mnt.mnt_flags & MNT_READONLY)) && !kattr->mnt_idmap;
}
...
static int do_statmount(struct kstatmount *s)
{
     ...
     */
    if (!is_path_reachable(m, m->mnt.mnt_root, &s->root) && !ns_capable_noaudit(&init_user_ns, CAP_SYS_ADMIN))
        return -EPERM;
                            ...
                            size_t nr_mnt_ids, const struct path *root)
{
     ...
     * mounts to show users.
     */
    if (!is_path_reachable(real_mount(orig->mnt), orig->dentry, root) && !ns_capable_noaudit(&init_user_ns, CAP_SYS_ADMIN))
        return -EPERM;
...
static int mntns_install(struct nsset *nsset, struct ns_common *ns)
{
    ...
    if (!ns_capable(mnt_ns->user_ns, CAP_SYS_ADMIN) || !ns_capable(user_ns, CAP_SYS_CHROOT) || !ns_capable(user_ns, CAP_SYS_ADMIN))
        return -EPERM;
