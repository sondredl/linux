...
static int shift_arg_pages(struct vm_area_struct *vma, unsigned long shift)
{
     ...
     */
    if (length != move_page_tables(vma, old_start, vma, new_start, length, false, true))
        return -ENOMEM;
...
void would_dump(struct linux_binprm *bprm, struct file *file)
{
    ...
    if (inode_permission(idmap, inode, MAY_READ) < 0)
    {
        ...
        while ((user_ns != &init_user_ns) && !privileged_wrt_inode_uidgid(user_ns, idmap, inode))
...
static void bprm_fill_uid(struct linux_binprm *bprm, struct file *file)
{
    ...
    if (!vfsuid_has_mapping(bprm->cred->user_ns, vfsuid) || !vfsgid_has_mapping(bprm->cred->user_ns, vfsgid))
        return;
