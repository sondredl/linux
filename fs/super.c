...
bool super_trylock_shared(struct super_block *sb)
{
    ...
    {
        if (!(sb->s_flags & SB_DYING) && sb->s_root && (sb->s_flags & SB_BORN))
            return true;
        super_unlock_shared(sb);
    }
...
{
    return !(s->s_iflags & SB_I_RETIRED) && s->s_dev == *(dev_t *)fc->sget_key;
}
...
static int wait_for_partially_frozen(struct super_block *sb)
{
    ...
    do
    {
        ...
        down_write(&sb->s_umount);
    } while (ret == 0 && sb->s_writers.frozen != SB_UNFROZEN && sb->s_writers.frozen != SB_FREEZE_COMPLETE);
...
static inline bool may_freeze(struct super_block *sb, enum freeze_holder who)
{
    ...
    if (who & FREEZE_HOLDER_KERNEL)
        return (who & FREEZE_MAY_NEST) || sb->s_writers.freeze_kcount == 0;
    if (who & FREEZE_HOLDER_USERSPACE)
        return (who & FREEZE_MAY_NEST) || sb->s_writers.freeze_ucount == 0;
    return false;
}
