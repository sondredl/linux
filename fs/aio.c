...
static void aio_complete(struct aio_kiocb *iocb)
{
    ...
    *event = iocb->ki_res;
    ...
    if (waitqueue_active(&ctx->wait))
    {
        ...
        list_for_each_entry_safe(curr, next, &ctx->wait.head, w.entry) if (avail >= curr->min_nr)
        {
                         ...
                         bool compat)
{
    ...
    /* prevent overflows */
    if (unlikely(
            (iocb.aio_buf != (unsigned long)iocb.aio_buf) || (iocb.aio_nbytes != (size_t)iocb.aio_nbytes) || ((ssize_t)iocb.aio_nbytes < 0)))
    {
