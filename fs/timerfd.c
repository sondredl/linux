...
{
    return ctx->clockid == CLOCK_REALTIME_ALARM || ctx->clockid == CLOCK_BOOTTIME_ALARM;
}
...
SYSCALL_DEFINE2(timerfd_create, int, clockid, int, flags)
{
    ...
    if ((clockid == CLOCK_REALTIME_ALARM || clockid == CLOCK_BOOTTIME_ALARM) && !capable(CAP_WAKE_ALARM))
        return -EPERM;
