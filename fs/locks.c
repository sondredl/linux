    ...
    locks_check_ctx_file_list(struct file *filp, struct list_head *list, char *list_type)
{
    ...
    list_for_each_entry(flc, list, flc_list) if (flc->flc_file == filp)
        pr_warn(
...
static void __locks_wake_up_blocks(struct file_lock_core *blocker)
{
    while (!list_empty(&blocker->flc_blocked_requests))
    {
        ...
        if ((waiter->flc_flags & (FL_POSIX | FL_FLOCK)) && fl->fl_lmops && fl->fl_lmops->lm_notify)
...
static int __locks_delete_block(struct file_lock_core *waiter)
{
     ...
     */
    if (!smp_load_acquire(&waiter->flc_blocker) && list_empty(&waiter->flc_blocked_requests))
        return status;
...
new_blocker:
    list_for_each_entry(flc, &blocker->flc_blocked_requests, flc_blocked_member) if (conflict(flc, waiter))
    {
        blocker = flc;
        goto new_blocker;
    }
                            ...
                            struct file_lock *conflock)
{
     ...
     */
    if (!(request->c.flc_flags & FL_ACCESS) && (request->c.flc_type != F_UNLCK || request->fl_start != 0 || request->fl_end != OFFSET_MAX))
    {
        new_fl  = locks_alloc_lock();
    ...
    generic_add_lease(struct file *filp, int arg, struct file_lease **flp, void **priv)
{
    ...
    list_for_each_entry(fl, &ctx->flc_lease, c.flc_list)
    {
        if (fl->c.flc_file == filp && fl->c.flc_owner == lease->c.flc_owner)
        {
            my_fl = fl;
            continue;
        }
...
static int generic_delete_lease(struct file *filp, void *owner)
{
    ...
    list_for_each_entry(fl, &ctx->flc_lease, c.flc_list)
    {
        if (fl->c.flc_file == filp && fl->c.flc_owner == owner)
        {
            victim = fl;
            break;
        }
...
int kernel_setlease(struct file *filp, int arg, struct file_lease **lease, void **priv)
{
    if (lease)
                ...
                struct flock *flock)
{
     ...
     */
    if (!error && file_lock->c.flc_type != F_UNLCK && !(file_lock->c.flc_flags & FL_OFDLCK))
    {
    ...
    locks_remove_lease(struct file *filp, struct file_lock_context *ctx)
{
    ...
    percpu_down_read(&file_rwsem);
    spin_lock(&ctx->flc_lock);
    list_for_each_entry_safe(fl, tmp, &ctx->flc_lease, c.flc_list) if (filp == fl->c.flc_file)
        lease_modify(fl, F_UNLCK, &dispose);
    spin_unlock(&ctx->flc_lock);
    percpu_up_read(&file_rwsem);
                            ...
                            loff_t id, char *pfx, int repeat)
{
    ...
    seq_printf(f, "%s ", (type == F_WRLCK) ? "WRITE" : (type == F_RDLCK) ? "READ"
                                                                         : "UNLCK");
    if (inode)
    {
