                                ...
                                struct bdi_writeback *new_wb)
{
    ...
    if (!list_empty(&inode->i_io_list))
    {
        ...
        if (inode->i_state & I_DIRTY_ALL)
        {
            ...
            list_for_each_entry(pos, &new_wb->b_dirty, i_io_list) if (time_after_eq(inode->dirtied_when,
                                                                                    pos->dirtied_when)) break;
                                     ...
                                     struct bdi_writeback *new_wb)
{
    ...
    spin_lock(&inode->i_lock);
    if (!(inode->i_sb->s_flags & SB_ACTIVE) || inode->i_state & (I_WB_SWITCH | I_FREEING | I_WILL_FREE) || inode_to_wb(inode) == new_wb)
    {
        spin_unlock(&inode->i_lock);
        return false;
    }
...
void wbc_detach_inode(struct writeback_control *wbc)
{
    ...
    if (wbc->wb_bytes >= wbc->wb_lcand_bytes && wbc->wb_bytes >= wbc->wb_tcand_bytes)
    {
        max_id    = wbc->wb_id;
    ...
    if (avg_time)
        avg_time += (max_time >> WB_FRN_TIME_AVG_SHIFT) - (avg_time >> WB_FRN_TIME_AVG_SHIFT);
    else
...
restart:
    ...
    list_for_each_entry_continue_rcu(wb, &bdi->wb_list, bdi_node)
    {
        ...
        if (!wb_has_dirty_io(wb) && (base_work->sync_mode == WB_SYNC_NONE || list_empty(&wb->b_dirty_time)))
            continue;
                          ...
                          unsigned long             dirtied_before)
{
     ...
     */
    if ((inode->i_state & I_DIRTY) && (wbc->sync_mode == WB_SYNC_ALL || wbc->tagged_writepages))
        inode->dirtied_when = jiffies;
    ...
    if (mapping_tagged(inode->i_mapping, PAGECACHE_TAG_DIRTY))
    {
         ...
         */
        if (wbc->nr_to_write <= 0 && !inode_dirtied_after(inode, dirtied_before))
        {
    ...
    __writeback_single_inode(struct inode *inode, struct writeback_control *wbc)
{
     ...
     */
    if ((inode->i_state & I_DIRTY_TIME) && (wbc->sync_mode == WB_SYNC_ALL || time_after(jiffies, inode->dirtied_time_when + dirtytime_expire_interval * HZ)))
    {
...
static void wakeup_dirtytime_writeback(struct work_struct *w)
{
    ...
    list_for_each_entry_rcu(bdi, &bdi_list, bdi_list)
    {
        ...
        list_for_each_entry_rcu(wb, &bdi->wb_list, bdi_node) if (!list_empty(&wb->b_dirty_time))
            wb_wakeup(wb);
    }
    rcu_read_unlock();
