                                       ...
                                       unsigned int mode, int sync, void *keyp)
{
    ...
    if (key->xa != ewait->key.xa || key->entry_start != ewait->key.entry_start)
        return 0;
                                   ...
                                   struct page **page)
{
    ...
    for (;;)
    {
        ...
        if (!entry || dax_is_zero_entry(entry) || dax_is_empty_entry(entry))
        {
            /*
...
retry:
    ...
    if (entry)
    {
        ...
        if (order == 0)
        {
            if (dax_is_pmd_entry(entry) && (dax_is_zero_entry(entry) || dax_is_empty_entry(entry)))
            {
                pmd_downgrade = true;
            }
        }
                                  ...
                                  pgoff_t index, bool trunc)
{
    ...
    if (!entry || WARN_ON_ONCE(!xa_is_value(entry)))
        goto out;
    if (!trunc && (xas_get_mark(&xas, PAGECACHE_TAG_DIRTY) || xas_get_mark(&xas, PAGECACHE_TAG_TOWRITE)))
        goto out;
...
{
    return (iter->flags & IOMAP_WRITE) && (vma->vm_flags & VM_SYNC) && (iter->iomap.flags & IOMAP_F_DIRTY);
}
                                 ...
                                 const struct iomap *srcmap, void *daddr)
{
    ...
    bool  zero_edge = srcmap->flags & IOMAP_F_SHARED || srcmap->type == IOMAP_UNWRITTEN;
    void *saddr     = NULL;
    int   ret       = 0;
                             ...
                             struct iov_iter         *iter)
{
     ...
     */
    if (WARN_ON_ONCE(iomap->type != IOMAP_MAPPED && !(iomap->flags & IOMAP_F_SHARED)))
        return -EIO;
                                 ...
                                 struct xa_state *xas, void **entry, bool pmd)
{
    ...
    if (!write && (iomap->type == IOMAP_UNWRITTEN || iomap->type == IOMAP_HOLE))
    {
        if (!pmd)
                                      ...
                                      int *iomap_errp, const struct iomap_ops *ops)
{
    ...
    while ((error = iomap_iter(&iter, ops)) > 0)
    {
        ...
        if (ret != VM_FAULT_SIGBUS && (iter.iomap.flags & IOMAP_F_NEW))
        {
                                     ...
                                     pgoff_t max_pgoff)
{
     ...
     */
    if ((vmf->pgoff & PG_PMD_COLOUR) != ((vmf->address >> PAGE_SHIFT) & PG_PMD_COLOUR))
        return true;
                                      ...
                                      const struct iomap_ops *ops)
{
     ...
     * retried.
     */
    if (!pmd_none(*vmf->pmd) && !pmd_trans_huge(*vmf->pmd) && !pmd_devmap(*vmf->pmd))
    {
        ret = 0;
        goto unlock_entry;
    }
                                  ...
                                  const struct iomap_ops *ops)
{
    ...
    while ((ret = iomap_iter(&src_iter, ops)) > 0 && (ret = iomap_iter(&dst_iter, ops)) > 0)
    {
