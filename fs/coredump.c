...
static __printf(2, 3) int cn_esc_printf(struct core_name *cn, const char *fmt, ...)
{
    ...
    if (ret == 0)
    {
         ...
         */
        if ((cn->used - cur == 1 && cn->corename[cur] == '.') || (cn->used - cur == 2 && cn->corename[cur] == '.' && cn->corename[cur + 1] == '.'))
            cn->corename[cur] = '!';
...
{
    if (suid_dumpable == SUID_DUMP_ROOT && core_pattern[0] != '/' && core_pattern[0] != '|')
    {
        pr_warn(
...
static bool dump_vma_snapshot(struct coredump_params *cprm)
{
    ...
    for (i = 0; i < cprm->vma_count; i++)
    {
        ...
        if (m->dump_size == DUMP_SIZE_MAYBE_ELFHDR_PLACEHOLDER)
        {
            ...
            if (copy_from_user(elfmag, (void __user *)m->start, SELFMAG) || memcmp(elfmag, ELFMAG, SELFMAG) != 0)
            {
                m->dump_size = 0;
            }
            else
