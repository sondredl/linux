                                ...
                                struct fileattr       *fa)
{
     ...
     */
    if ((fa->flags ^ old_ma->flags) & (FS_APPEND_FL | FS_IMMUTABLE_FL) && !capable(CAP_LINUX_IMMUTABLE))
        return -EPERM;
    ...
    else
    {
         ...
         */
        if (old_ma->fsx_projid != fa->fsx_projid && !projid_valid(make_kprojid(&init_user_ns, fa->fsx_projid)))
            return -EINVAL;
    }
    ...
    if ((fa->fsx_xflags & FS_XFLAG_COWEXTSIZE) && !S_ISREG(inode->i_mode) && !S_ISDIR(inode->i_mode))
        return -EINVAL;
     ...
     */
    if ((fa->fsx_xflags & FS_XFLAG_DAX) && !(S_ISREG(inode->i_mode) || S_ISDIR(inode->i_mode)))
        return -EINVAL;
                       ...
                       compat_ulong_t, arg)
{
    ...
    switch (cmd)
    {
        ...
        case FS_IOC32_SETFLAGS:
            cmd = (cmd == FS_IOC32_GETFLAGS) ? FS_IOC_GETFLAGS : FS_IOC_SETFLAGS;
            fallthrough;
        /*
