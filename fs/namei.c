...
int may_linkat(struct mnt_idmap *idmap, const struct path *link)
{
    ...
    if (!vfsuid_valid(i_uid_into_vfsuid(idmap, inode)) || !vfsgid_valid(i_gid_into_vfsgid(idmap, inode)))
        return -EOVERFLOW;
     ...
     */
    if (safe_hardlink_source(idmap, inode) || inode_owner_or_capable(idmap, inode))
        return 0;
                                ...
                                struct nameidata *nd, struct inode *const inode)
{
    ...
    if ((!sysctl_protected_fifos && S_ISFIFO(inode->i_mode)) || (!sysctl_protected_regular && S_ISREG(inode->i_mode)) || likely(!(dir_mode & S_ISVTX)) || vfsuid_eq(i_uid_into_vfsuid(idmap, inode), dir_vfsuid) || vfsuid_eq_kuid(i_uid_into_vfsuid(idmap, inode), current_fsuid()))
        return 0;

    if (likely(dir_mode & 0002) || (dir_mode & 0020 && ((sysctl_protected_fifos >= 2 && S_ISFIFO(inode->i_mode)) || (sysctl_protected_regular >= 2 && S_ISREG(inode->i_mode)))))
    {
        const char *operation = S_ISFIFO(inode->i_mode) ? "sticky_create_fifo" : "sticky_create_regular";
                             ...
                             struct dentry *dentry)
{
    ...
    if (likely(!d_is_symlink(path.dentry)) || ((flags & WALK_TRAILING) && !(nd->flags & LOOKUP_FOLLOW)) || (flags & WALK_NOFOLLOW))
    {
...
static const char *path_init(struct nameidata *nd, unsigned flags)
{
    ...
    else
    {
        ...
        {
            if (f.file->f_cred != current_cred() && !ns_capable(f.file->f_cred->user_ns, CAP_DAC_READ_SEARCH))
            {
                fdput(f);
                return ERR_PTR(-ENOENT);
            }
                      ...
                      struct dentry *victim, bool isdir)
{
    ...
    if (check_sticky(idmap, dir, inode) || IS_APPEND(inode) || IS_IMMUTABLE(inode) || IS_SWAPFILE(inode) || HAS_UNMAPPED_ID(idmap, inode))
        return -EPERM;
    if (isdir)
    {
        if (!d_is_dir(victim))
...
static struct dentry *lock_two_directories(struct dentry *p1, struct dentry *p2)
{
    ...
    { // no common ancestor at the time we'd been called
              ...
              struct dentry *dentry)
{
    ...
    error = -EBUSY;
    if (is_local_mountpoint(dentry) || (dentry->d_inode->i_flags & S_KERNEL_FILE))
        goto out;
