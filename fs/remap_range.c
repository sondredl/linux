                                ...
                                loff_t *req_count, unsigned int remap_flags)
{
     ...
     */
    if (pos_in + count == size_in && (!(remap_flags & REMAP_FILE_DEDUP) || pos_out + count == size_out))
    {
                                         ...
                                         loff_t len, bool *is_same)
{
    ...
    while (len)
    {
         ...
         */
        if (!folio_test_uptodate(src_folio) || !folio_test_uptodate(dst_folio) || src_folio->mapping != src->f_mapping || dst_folio->mapping != dest->f_mapping)
        {
            same = false;
            goto unlock;
        }
