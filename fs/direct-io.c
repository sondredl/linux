...
static ssize_t dio_complete(struct dio *dio, ssize_t ret, unsigned int flags)
{
    ...
    if (dio->result)
    {
        ...
        if (dio_op == REQ_OP_READ && ((offset + transferred) > dio->i_size))
     ...
     */
    if (flags & DIO_COMPLETE_INVALIDATE && ret > 0 && dio_op == REQ_OP_WRITE)
...
static void dio_bio_end_aio(struct bio *bio)
{
    ...
    if (remaining == 0)
    {
         ...
         */
        if (dio->result)
            defer_completion = dio->defer_completion || (dio_op == REQ_OP_WRITE && dio->inode->i_mapping->nrpages);
        if (defer_completion)
        {
                        ...
                        struct buffer_head *map_bh)
{
     ...
     */
    if (sdio->cur_page == page && sdio->cur_page_offset + sdio->cur_page_len == offset && sdio->cur_page_block + (sdio->cur_page_len >> sdio->blkbits) == blocknr)
    {
                             ...
                             int flags)
{
    ...
    if (dio->is_async && retval == 0 && dio->result && (iov_iter_rw(iter) == READ || dio->result == count))
        retval = -EIOCBQUEUED;
    else
