...
struct posix_acl *get_cached_acl(struct inode *inode, int type)
{
    ...
    for (;;)
    {
        ...
        if (!acl || is_uncached_acl(acl) || refcount_inc_not_zero(&acl->a_refcount))
            break;
        rcu_read_unlock();
        cpu_relax();
    }
    ...
    posix_acl_clone(const struct posix_acl *acl, gfp_t flags)
{
    ...
    if (acl)
    {
        int size = sizeof(struct posix_acl) + acl->a_count * sizeof(struct posix_acl_entry);
    ...
    FOREACH_ACL_ENTRY(pa, acl, pe)
    {
        switch (pa->e_tag)
        {
            ...
            case ACL_OTHER:
                ...
                break;
            case ACL_MASK:
                mode      = (mode & ~S_IRWXG) | ((pa->e_perm & S_IRWXO) << 3);
                not_equiv = 1;
                break;
            case ACL_USER:
            case ACL_GROUP:
                          ...
                          struct posix_acl **acl)
{
    ...
    if (error == 0)
        *acl = NULL;
    if (!vfsgid_in_group_p(i_gid_into_vfsgid(idmap, inode)) && !capable_wrt_inode_uidgid(idmap, inode, CAP_FSETID))
        mode &= ~S_ISGID;
    *mode_p = mode;
    return 0;
}
 ...
 */
int posix_acl_to_xattr(struct user_namespace *user_ns, const struct posix_acl *acl,
