...
{
    return ST_VALID | flags_by_mnt(mnt->mnt_flags) | flags_by_sb(mnt->mnt_sb->s_flags);
}
...
static int do_statfs_native(struct kstatfs *st, struct statfs __user *p)
{
    ...
    else
    {
        ...
        {
            if ((st->f_blocks | st->f_bfree | st->f_bavail | st->f_bsize | st->f_frsize) & 0xffffffff00000000ULL)
                return -EOVERFLOW;
            /*
             * f_files and f_ffree may be -1; it's okay to stuff
             * that into 32 bits
             */
            if (st->f_files != -1 && (st->f_files & 0xffffffff00000000ULL))
                return -EOVERFLOW;
            if (st->f_ffree != -1 && (st->f_ffree & 0xffffffff00000000ULL))
                return -EOVERFLOW;
        }
...
static int put_compat_statfs(struct compat_statfs __user *ubuf, struct kstatfs *kbuf)
{
    ...
    {
        if ((kbuf->f_blocks | kbuf->f_bfree | kbuf->f_bavail | kbuf->f_bsize | kbuf->f_frsize) & 0xffffffff00000000ULL)
            return -EOVERFLOW;
