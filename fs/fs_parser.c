...
int fs_param_is_string(struct p_log *log, const struct fs_parameter_spec *p,
                       struct fs_parameter *param, struct fs_parse_result *result)
{
    if (param->type != fs_value_is_string || (!*param->string && !(p->flags & fs_param_can_be_empty)))
        return fs_param_bad_value(log, param);
    return 0;
}
                   ...
                   struct fs_parameter *param, struct fs_parse_result *result)
{
    ...
    {
        case fs_value_is_string:
            if ((!*param->string && !(p->flags & fs_param_can_be_empty)) || kstrtouint(param->string, 0, &result->uint_32) < 0)
                break;
                             ...
                             int low, int high, int special)
{
    ...
    for (i = 0; i < tbl_size; i++)
    {
        ...
        if (tbl[i].value != special && (tbl[i].value < low || tbl[i].value > high))
        {
