                ...
                struct __kernel_timespec __user *, utimes, int, flags)
{
    ...
    if (utimes)
    {
        ...
        if (tstimes[0].tv_nsec == UTIME_OMIT && tstimes[1].tv_nsec == UTIME_OMIT)
            return 0;
    }
                         ...
                         struct __kernel_old_timeval __user *utimes)
{
    ...
    if (utimes)
    {
        ...
        /* This test is needed to catch all invalid values.  If we
           ...
           valid for utimensat.  */
        if (times[0].tv_usec >= 1000000 || times[0].tv_usec < 0 || times[1].tv_usec >= 1000000 || times[1].tv_usec < 0)
            return -EINVAL;
...
SYSCALL_DEFINE2(utime, char __user *, filename, struct utimbuf __user *, times)
{
    ...
    if (times)
    {
        if (get_user(tv[0].tv_sec, &times->actime) || get_user(tv[1].tv_sec, &times->modtime))
            return -EFAULT;
        tv[0].tv_nsec = 0;
        tv[1].tv_nsec = 0;
    }
                ...
                struct old_utimbuf32 __user *, t)
{
    ...
    if (t)
    {
        if (get_user(tv[0].tv_sec, &t->actime) || get_user(tv[1].tv_sec, &t->modtime))
            return -EFAULT;
        tv[0].tv_nsec = 0;
        tv[1].tv_nsec = 0;
    }
                                ...
                                struct old_timeval32 __user *t)
{
    ...
    if (t)
    {
        if (get_user(tv[0].tv_sec, &t[0].tv_sec) || get_user(tv[0].tv_nsec, &t[0].tv_usec) || get_user(tv[1].tv_sec, &t[1].tv_sec) || get_user(tv[1].tv_nsec, &t[1].tv_usec))
            return -EFAULT;
        if (tv[0].tv_nsec >= 1000000 || tv[0].tv_nsec < 0 || tv[1].tv_nsec >= 1000000 || tv[1].tv_nsec < 0)
            return -EINVAL;
        tv[0].tv_nsec *= 1000;
        tv[1].tv_nsec *= 1000;
    }
