                                            ...
                                            unsigned int  features)
{
    ...
    msg.arg.pagefault.address = (features & UFFD_FEATURE_EXACT_ADDRESS) ? real_address : address;
                          ...
                          unsigned long start, unsigned long end)
{
    ...
    list_for_each_entry(unmap_ctx, unmaps, list) if (unmap_ctx->ctx == ctx && unmap_ctx->start == start && unmap_ctx->end == end) return true;
                           ...
                           unsigned long end, struct list_head *unmaps)
{
    ...
    if (!ctx || !(ctx->features & UFFD_FEATURE_EVENT_UNMAP) || has_unmap_ctx(ctx, unmaps, start, end))
        return 0;
...
static int userfaultfd_release(struct inode *inode, struct file *file)
{
    ...
    {
        cond_resched();
        BUG_ON(!!vma->vm_userfaultfd_ctx.ctx ^ !!(vma->vm_flags & __VM_UFFD_FLAGS));
    ...
    for (;;)
    {
        ...
        if (uwq)
        {
            *msg = uwq->msg;
            ...
            {
                fork_nctx = (struct userfaultfd_ctx *)(unsigned long)
                                           ...
                                           struct userfaultfd_wake_range *range)
{
    ...
    do
    {
        ...
        need_wakeup = waitqueue_active(&ctx->fault_pending_wqh) || waitqueue_active(&ctx->fault_wqh);
        cond_resched();
                                ...
                                unsigned long           arg)
{
    ...
    do
    {
        ...
        BUG_ON(!!cur->vm_userfaultfd_ctx.ctx ^ !!(cur->vm_flags & __VM_UFFD_FLAGS));
         ...
         * check alignment.
         */
        if (is_vm_hugetlb_page(cur) && end <= cur->vm_end && end > cur->vm_start)
        {
    ...
    for_each_vma_range(vmi, vma, end)
    {
         ...
         */
        if (vma->vm_userfaultfd_ctx.ctx == ctx && (vma->vm_flags & vm_flags) == vm_flags)
            goto skip;
...
out_unlock:
    ...
    if (!ret)
    {
        ...
        ioctls_out = basic_ioctls ? UFFD_API_RANGE_IOCTLS_BASIC : UFFD_API_RANGE_IOCTLS;
                                    ...
                                    unsigned long           arg)
{
    ...
    if (uffdio_wp.mode & ~(UFFDIO_WRITEPROTECT_MODE_DONTWAKE | UFFDIO_WRITEPROTECT_MODE_WP))
        return -EINVAL;
...
static int userfaultfd_continue(struct userfaultfd_ctx *ctx, unsigned long arg)
{
    ...
    ret = -EINVAL;
    if (uffdio_continue.mode & ~(UFFDIO_CONTINUE_MODE_DONTWAKE | UFFDIO_CONTINUE_MODE_WP))
        goto out;
                            ...
                            unsigned long           arg)
{
    ...
    if (uffdio_move.mode & ~(UFFDIO_MOVE_MODE_ALLOW_SRC_HOLES | UFFDIO_MOVE_MODE_DONTWAKE))
        return -EINVAL;
