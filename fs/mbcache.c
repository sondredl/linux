                                           ...
                                           u32                    key)
{
    ...
    while (node)
    {
        entry = hlist_bl_entry(node, struct mb_cache_entry,
                               e_hash_list);
        if (entry->e_key == key && test_bit(MBE_REUSABLE_B, &entry->e_flags) && atomic_inc_not_zero(&entry->e_refcnt))
            goto out;
        node = node->next;
    }
    entry = NULL;
                                          ...
                                          u64 value)
{
    ...
    {
        if (entry->e_key == key && entry->e_value == value && atomic_inc_not_zero(&entry->e_refcnt))
            goto out;
    }
    entry = NULL;
out:
                                     ...
                                     unsigned long    nr_to_scan)
{
    ...
    while (nr_to_scan-- && !list_empty(&cache->c_list))
    {
        ...
        if (test_bit(MBE_REFERENCED_B, &entry->e_flags) || atomic_cmpxchg(&entry->e_refcnt, 1, 0) != 1)
        {
