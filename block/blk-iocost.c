                                    ...
                                    struct gendisk *disk)
{
    ...
    ioc->vrate_min = DIV64_U64_ROUND_UP((u64)ioc->params.qos[QOS_MIN] * VTIME_PER_USEC, MILLION);
    ioc->vrate_max = DIV64_U64_ROUND_UP((u64)ioc->params.qos[QOS_MAX] * VTIME_PER_USEC, MILLION);
...
static void ioc_now(struct ioc *ioc, struct ioc_now *now)
{
    ...
    do
    {
        ...
        now->vnow = ioc->period_at_vtime + (now->now - ioc->period_at) * vrate;
                                ...
                                bool save, struct ioc_now *now)
{
    ...
    for (lvl = iocg->level - 1; lvl >= 0; lvl--)
    {
        ...
        if (parent_active == parent->active && parent_inuse == parent->inuse)
            break;
...
static bool iocg_kick_delay(struct ioc_gq *iocg, struct ioc_now *now)
{
    ...
    vover     = atomic64_read(&iocg->vtime) + abs_cost_to_cost(iocg->abs_vdebt, hwa) - now->vnow;
    ...
    if (vover_pct <= MIN_DELAY_THR_PCT)
        ...
        new_delay = MIN_DELAY + div_u64((MAX_DELAY - MIN_DELAY) * (vover_pct - MIN_DELAY_THR_PCT), MAX_DELAY_THR_PCT - MIN_DELAY_THR_PCT);
                            ...
                            struct ioc_now *now)
{
    ...
    expires   = now->now_ns + DIV64_U64_ROUND_UP(vshortage, ioc->vtime_base_rate) * NSEC_PER_USEC;
    expires += ioc->timer_slack_ns;

    /* if already active and close enough, don't bother */
    oexpires = ktime_to_ns(hrtimer_get_softexpires(&iocg->waitq_timer));
    if (hrtimer_is_queued(&iocg->waitq_timer) && abs(oexpires - expires) <= ioc->timer_slack_ns)
        return;
...
static bool iocg_is_idle(struct ioc_gq *iocg)
{
    ...
    if (atomic64_read(&iocg->active_period) == atomic64_read(&ioc->cur_period))
        return false;
                                  ...
                                  u32 usage, struct ioc_now *now)
{
    ...
    if (waitqueue_active(&iocg->waitq) || time_after64(vtime, now->vnow - ioc->margins.min))
        return hwm;
...
static void transfer_surpluses(struct list_head *surpluses, struct ioc_now *now)
{
    ...
    list_for_each_entry_safe(iocg, tiocg, &over_hwa, walk_list)
    {
        if (over_target)
            iocg->hweight_after_donation =
                div_u64((u64)iocg->hweight_after_donation * over_target, over_sum);
        list_del_init(&iocg->walk_list);
    }
    ...
    list_for_each_entry(iocg, &inner_walk, walk_list)
    {
        ...
        if (WARN_ON_ONCE(iocg->hweight_active <= 1 || iocg->hweight_donating <= 1 || iocg->hweight_after_donation == 0))
        {
...
static int ioc_check_iocgs(struct ioc *ioc, struct ioc_now *now)
{
    ...
    {
        if (!waitqueue_active(&iocg->waitq) && !iocg->abs_vdebt && !iocg->delay && !iocg_is_idle(iocg))
            continue;
        ...
        if (waitqueue_active(&iocg->waitq) || iocg->abs_vdebt || iocg->delay)
        {
...
static void ioc_timer_fn(struct timer_list *timer)
{
    ...
    list_for_each_entry(iocg, &ioc->active_iocgs, active_list)
    {
         ...
         */
        if ((ppm_rthr != MILLION || ppm_wthr != MILLION) && !atomic_read(&iocg_to_blkg(iocg)->use_delay) && time_after64(vtime, vdone) && time_after64(vtime, now.vnow - MAX_LAGGING_PERIODS * period_vtime) && time_before64(vdone, now.vnow - period_vtime))
            nr_lagging++;
        ...
        if (hw_inuse < hw_active || (!waitqueue_active(&iocg->waitq) && time_before64(vtime, now.vnow - ioc->margins.low)))
        {
    ...
    if (rq_wait_pct > RQ_WAIT_BUSY_PCT || missed_ppm[READ] > ppm_rthr || missed_ppm[WRITE] > ppm_wthr)
    {
        /* clearly missing QoS targets, slow down vrate */
        ioc->busy_level = max(ioc->busy_level, 0);
        ioc->busy_level++;
    }
    else if (rq_wait_pct <= RQ_WAIT_BUSY_PCT * UNBUSY_THR_PCT / 100 && missed_ppm[READ] <= ppm_rthr * UNBUSY_THR_PCT / 100 && missed_ppm[WRITE] <= ppm_wthr * UNBUSY_THR_PCT / 100)
    {
                                      ...
                                      u64 abs_cost, struct ioc_now *now)
{
     ...
     */
    if (margin >= iocg->saved_margin || margin >= margins->low || iocg->inuse == iocg->active)
        return cost;
    ...
    do
    {
        ...
        current_hweight(iocg, NULL, &hwi);
        cost = abs_cost_to_cost(abs_cost, hwi);
    } while (time_after64(vtime + cost, now->vnow) && iocg->inuse != iocg->active);
...
static void ioc_rqos_throttle(struct rq_qos *rqos, struct bio *bio)
{
     ...
     */
    if (!waitqueue_active(&iocg->waitq) && !iocg->abs_vdebt && time_before_eq64(vtime + cost, now.vnow))
    {
                           ...
                           struct bio *bio)
{
    ...
    if (blk_rq_pos(rq) < bio_end && blk_rq_pos(rq) + blk_rq_sectors(rq) == iocg->cursor)
        iocg->cursor = bio_end;
     ...
     * cost assigned.
     */
    if (rq->bio && rq->bio->bi_iocost_cost && time_before_eq64(atomic64_read(&iocg->vtime) + cost, now.vnow))
    {
        iocg_commit_bio(iocg, bio, abs_cost, cost);
        return;
    }
...
static void ioc_rqos_done(struct rq_qos *rqos, struct request *rq)
{
    ...
    if (on_q_ns <= size_nsec || on_q_ns - size_nsec <= ioc->params.qos[pidx] * NSEC_PER_USEC)
                             ...
                             size_t nbytes, loff_t off)
{
    ...
    while ((p = strsep(&body, " \t\n")))
    {
        ...
        switch (tok)
        {
            case QOS_RPPM:
            case QOS_WPPM:
                if (match_strlcpy(buf, &args[0], sizeof(buf)) >= sizeof(buf))
                    goto einval;
