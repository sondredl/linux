...
#define BLKIOLATENCY_EXP_BUCKET_SIZE (BLKIOLATENCY_MAX_WIN_SIZE / (BLKIOLATENCY_NR_EXP_FACTORS - 1))
...
static void iolatency_check_latencies(struct iolatency_grp *iolat, u64 now)
{
    ...
    if (latency_sum_ok(iolat, &stat) && atomic_read(&lat_info->scale_cookie) == DEFAULT_SCALE_COOKIE)
        return;
    ...
    if ((lat_info->last_scale_event >= now || now - lat_info->last_scale_event < BLKIOLATENCY_MIN_ADJUST_TIME))
        goto out;

    if (latency_sum_ok(iolat, &iolat->cur_stat) && latency_sum_ok(iolat, &stat))
    {
        if (latency_stat_samples(iolat, &iolat->cur_stat) < BLKIOLATENCY_MIN_GOOD_SAMPLES)
            goto out;
    ...
    else if (lat_info->scale_lat == 0 || lat_info->scale_lat >= iolat->min_lat_nsec)
    {
        lat_info->last_scale_event = now;
        if (!lat_info->scale_grp || lat_info->scale_lat > iolat->min_lat_nsec)
        {
...
static void blkcg_iolatency_done_bio(struct rq_qos *rqos, struct bio *bio)
{
    ...
    while (blkg && blkg->parent)
    {
        ...
        if (iolat->min_lat_nsec && bio->bi_status != BLK_STS_AGAIN)
        {
            ...
            if (now > window_start && (now - window_start) >= iolat->cur_win_nsec)
            {
