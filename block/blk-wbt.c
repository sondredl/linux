...
{
    return rwb && rwb->enable_state != WBT_STATE_OFF_DEFAULT && rwb->enable_state != WBT_STATE_OFF_MANUAL;
}
                         ...
                         enum wbt_flags wb_acct)
{
    ...
    else if (test_bit(QUEUE_FLAG_WC, &rwb->rqos.disk->queue->queue_flags) && !wb_recent_wait(rwb))
        limit = 0;
    else
        limit = rwb->wb_normal;
...
static int latency_exceeded(struct rq_wb *rwb, struct blk_rq_stat *stat)
{
    ...
    if (thislat > rwb->cur_win_nsec || (thislat > rwb->min_lat_nsec && !stat[READ].nr_samples))
    {
...
static bool close_io(struct rq_wb *rwb)
{
    ...
    return time_before(now, rwb->last_issue + HZ / 10) || time_before(now, rwb->last_comp + HZ / 10);
}
