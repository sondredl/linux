...
int blk_queue_enter(struct request_queue *q, blk_mq_req_flags_t flags)
{
    ...
    while (!blk_try_enter_queue(q, pm))
    {
        ...
        wait_event(q->mq_freeze_wq,
                   (!q->mq_freeze_depth && blk_pm_resume_queue(pm, q)) || blk_queue_dying(q));
...
int __bio_queue_enter(struct request_queue *q, struct bio *bio)
{
    while (!blk_try_enter_queue(q, false))
    {
        ...
        smp_rmb();
        wait_event(q->mq_freeze_wq,
                   (!q->mq_freeze_depth && blk_pm_resume_queue(false, q)) || test_bit(GD_DEAD, &disk->state));
        if (test_bit(GD_DEAD, &disk->state))
            goto dead;
    }
...
{
    return bdev_test_flag(part, BD_MAKE_IT_FAIL) && should_fail(&fail_make_request, bytes);
}
...
static inline int bio_check_eod(struct bio *bio)
{
    ...
    if (nr_sectors && (nr_sectors > maxsector || bio->bi_iter.bi_sector > maxsector - nr_sectors))
    {
        pr_info_ratelimited(
...
void submit_bio_noacct(struct bio *bio)
{
    ...
    if (!bio_flagged(bio, BIO_REMAPPED))
    {
        if (unlikely(bio_check_eod(bio)))
            goto end_io;
        if (bdev_is_partition(bdev) && unlikely(blk_partition_remap(bio)))
            goto end_io;
    }
    ...
    {
        if (WARN_ON_ONCE(bio_op(bio) != REQ_OP_WRITE && bio_op(bio) != REQ_OP_ZONE_APPEND))
            goto end_io;
...
int bio_poll(struct bio *bio, struct io_comp_batch *iob, unsigned int flags)
{
    ...
    if (cookie == BLK_QC_T_NONE || !test_bit(QUEUE_FLAG_POLL, &q->queue_flags))
        return 0;
...
again:
    ...
    if (unlikely(time_after(now, stamp)) && likely(try_cmpxchg(&part->bd_stamp, &stamp, now)) && (end || part_in_flight(part)))
                                      ...
                                      int size)
{
    ...
    list_for_each_entry(cb, &plug->cb_list, list) if (cb->callback == unplug && cb->data == data) return cb;
