...
void __init bdev_cache_init(void)
{
    ...
    bdev_cachep = kmem_cache_create("bdev_cache", sizeof(struct bdev_inode),
                                    0, (SLAB_HWCACHE_ALIGN | SLAB_RECLAIM_ACCOUNT | SLAB_ACCOUNT | SLAB_PANIC),
                                    init_once);
                         ...
                         const struct blk_holder_ops *hops)
{
     ...
     */
    if (whole != bdev && whole->bd_holder && whole->bd_holder != bd_may_claim)
        return false;
    return true;
}
...
int bdev_permission(dev_t dev, blk_mode_t mode, void *holder)
{
    ...
    ret = devcgroup_check_permission(DEVCG_DEV_BLOCK,
                                     MAJOR(dev), MINOR(dev),
                                     ((mode & BLK_OPEN_READ) ? DEVCG_ACC_READ : 0) | ((mode & BLK_OPEN_WRITE) ? DEVCG_ACC_WRITE : 0));
    if (ret)
        return ret;
              ...
              const struct blk_holder_ops *hops, struct file *bdev_file)
{
    ...
    if (holder)
    {
         ...
         */
        if ((mode & BLK_OPEN_WRITE) && !bdev_test_flag(bdev, BD_WRITE_HOLDER) && (disk->event_flags & DISK_EVENT_FLAG_BLOCK_ON_EXCL_WRITE))
        {
