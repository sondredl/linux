...
{
    return !strcmp(e->elevator_name, name) || (e->elevator_alias && !strcmp(e->elevator_alias, name));
}
...
static struct elevator_type *__elevator_find(const char *name)
{
    ...
    list_for_each_entry(e, &elv_list, list) if (elevator_match(e, name)) return e;
    return NULL;
}
...
int elv_register(struct elevator_type *e)
{
    ...
    if (e->icq_size)
    {
        if (WARN_ON(e->icq_size < sizeof(struct io_cq)) || WARN_ON(e->icq_align < __alignof__(struct io_cq)))
            return -EINVAL;
...
{
    if (!queue_is_mq(q) || (q->tag_set && (q->tag_set->flags & BLK_MQ_F_NO_SCHED)))
        return false;
    return true;
}
...
void elevator_init_mq(struct request_queue *q)
{
    ...
    {
        pr_warn(
            "\"%s\" elevator initialization failed, "
