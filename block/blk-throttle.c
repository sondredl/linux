...
static void tg_update_has_rules(struct throtl_grp *tg)
{
    ...
    for (rw = READ; rw <= WRITE; rw++)
    {
        tg->has_rules_iops[rw] =
            (parent_tg && parent_tg->has_rules_iops[rw]) || tg_iops_limit(tg, rw) != UINT_MAX;
        tg->has_rules_bps[rw] =
            (parent_tg && parent_tg->has_rules_bps[rw]) || tg_bps_limit(tg, rw) != U64_MAX;
    }
}
...
static inline void throtl_trim_slice(struct throtl_grp *tg, bool rw)
{
    ...
    io_trim = calculate_io_allowed(tg_iops_limit(tg, rw), time_elapsed) + tg->carryover_ios[rw];
...
static void __tg_update_carryover(struct throtl_grp *tg, bool rw)
{
    ...
    if (bps_limit != U64_MAX)
        tg->carryover_bytes[rw] +=
            calculate_bytes_allowed(bps_limit, jiffy_elapsed) - tg->bytes_disp[rw];
    if (iops_limit != UINT_MAX)
        tg->carryover_ios[rw] +=
            calculate_io_allowed(iops_limit, jiffy_elapsed) - tg->io_disp[rw];
}
                                          ...
                                          u32 iops_limit)
{
    ...
    io_allowed        = calculate_io_allowed(iops_limit, jiffy_elapsed_rnd) + tg->carryover_ios[rw];
                            ...
                            unsigned long *wait)
{
    ...
    if ((bps_limit == U64_MAX && iops_limit == UINT_MAX) || tg->flags & THROTL_TG_CANCELING)
    {
        if (wait)
            *wait = 0;
        return true;
...
static int throtl_dispatch_tg(struct throtl_grp *tg)
{
    ...
    while ((bio = throtl_peek_queued(&sq->queued[READ])) && tg_may_dispatch(tg, bio, NULL))
    {
    ...
    while ((bio = throtl_peek_queued(&sq->queued[WRITE])) && tg_may_dispatch(tg, bio, NULL))
    {
...
static void tg_conf_updated(struct throtl_grp *tg, bool global)
{
                                 ...
                                 global ? tg->td->queue->root_blkg : tg_to_blkg(tg))
    {
        ...
        if (!cgroup_subsys_on_dfl(io_cgrp_subsys) || !blkg->parent || !blkg->parent->parent)
            continue;
    }
    rcu_read_unlock();
                           ...
                           int off)
{
    ...
    if (tg->bps[READ] == bps_dft && tg->bps[WRITE] == bps_dft && tg->iops[READ] == iops_dft && tg->iops[WRITE] == iops_dft)
        return 0;
