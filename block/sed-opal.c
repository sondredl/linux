...
static int cmd_finalize(struct opal_dev *cmd, u32 hsn, u32 tsn)
{
    ...
    hdr->pkt.length = cpu_to_be32(cmd->pos - sizeof(hdr->cp) - sizeof(hdr->pkt));
                          ...
                          struct parsed_resp *resp)
{
    ...
    if (clen == 0 || plen == 0 || slen == 0 || slen > IO_BUFFER_LENGTH - sizeof(*hdr))
    {
...
{
    if (IS_ERR(token) || token->type != OPAL_DTA_TOKENID_TOKEN || token->pos[0] != match)
        return false;
    return true;
}
...
static int revert_lsp(struct opal_dev *dev, void *data)
{
    ...
    add_token_u8(&err, dev, (rev->options & OPAL_PRESERVE) ? OPAL_TRUE : OPAL_FALSE);
    add_token_u8(&err, dev, OPAL_ENDNAME);
    if (err)
    {
...
static int add_user_to_lr(struct opal_dev *dev, void *data)
{
    ...
    err = set_lr_boolean_ace(dev,
                             lkul->l_state == OPAL_RW ? OPAL_LOCKINGRANGE_ACE_WRLOCKED : OPAL_LOCKINGRANGE_ACE_RDLOCKED,
...
{
    u8 enable_disable = opal_mbr->enable_disable == OPAL_MBR_ENABLE ? OPAL_TRUE : OPAL_FALSE;
    ...
    if (opal_mbr->enable_disable != OPAL_MBR_ENABLE && opal_mbr->enable_disable != OPAL_MBR_DISABLE)
        return -EINVAL;
...
{
    u8 mbr_done_tf = mbr_done->done_flag == OPAL_MBR_DONE ? OPAL_TRUE : OPAL_FALSE;
    ...
    if (mbr_done->done_flag != OPAL_MBR_DONE && mbr_done->done_flag != OPAL_MBR_NOT_DONE)
        return -EINVAL;
                               ...
                               struct opal_lock_unlock *lk_unlk)
{
    ...
    if (lk_unlk->session.who < OPAL_USER1 || lk_unlk->session.who > OPAL_USER9)
    {
                                          ...
                                          struct opal_lock_unlock *lk_unlk)
{
    ...
    list_for_each_entry(iter, &dev->unlk_lst, node)
    {
        if ((iter->unlk.flags & OPAL_SAVE_FOR_LOCK) && iter->lr == lk_unlk->session.opal_key.lr && iter->unlk.session.opal_key.key_len > 0)
        {
                                     ...
                                     void __user           *data)
{
    ...
    if (!ret && copy_to_user(data + offsetof(struct opal_lr_status, range_start), (void *)opal_lrst + offsetof(struct opal_lr_status, range_start), sizeof(*opal_lrst) - offsetof(struct opal_lr_status, range_start)))
    {
...
static int opal_set_new_pw(struct opal_dev *dev, struct opal_new_pw *opal_pw)
{
    ...
    if (opal_pw->session.who > OPAL_USER9 || opal_pw->new_user_pw.who > OPAL_USER9)
        return -EINVAL;
