...
{
    if (blkg_rwstat_init(&stats->bytes, gfp) || blkg_rwstat_init(&stats->ios, gfp))
        goto error;

    #ifdef CONFIG_BFQ_CGROUP_DEBUG
    if (blkg_rwstat_init(&stats->merged, gfp) || blkg_rwstat_init(&stats->service_time, gfp) || blkg_rwstat_init(&stats->wait_time, gfp) || blkg_rwstat_init(&stats->queued, gfp) || bfq_stat_init(&stats->time, gfp) || bfq_stat_init(&stats->avg_queue_size_sum, gfp) || bfq_stat_init(&stats->avg_queue_size_samples, gfp) || bfq_stat_init(&stats->dequeue, gfp) || bfq_stat_init(&stats->group_wait_time, gfp) || bfq_stat_init(&stats->idle_time, gfp) || bfq_stat_init(&stats->empty_time, gfp))
        goto error;
    #endif
                                    ...
                                    struct bfq_group *bfqg)
{
    ...
    for (act_idx = 0; act_idx < bfqd->num_actuators; act_idx++)
    {
        ...
        if (async_bfqq && async_bfqq->entity.sched_data != &bfqg->sched_data)
        {
                                     ...
                                     int                ioprio_class)
{
    ...
    { /* leaf not reached yet */
        ...
        struct bfq_service_tree *child_st     = child_sd->service_tree + ioprio_class;
