                                     ...
                                     unsigned *nsegs, struct bio_set *bs)
{
    ...
    *nsegs = 1;
     ...
     */
    tmp = bio->bi_iter.bi_sector + split_sectors - ((lim->discard_alignment >> 9) % granularity);
                         ...
                         unsigned *segs, struct bio_set *bs, unsigned max_bytes)
{
    ...
    bio_for_each_bvec(bv, bio, iter)
    {
        ...
        if (nsegs < lim->max_segments && bytes + bv.bv_len <= max_bytes && bv.bv_offset + bv.bv_len <= PAGE_SIZE)
        {
            nsegs++;
            bytes += bv.bv_len;
        }
                             ...
                             struct scatterlist **sg)
{
    ...
    for_each_bio(bio)
    {
        bio_for_each_bvec(bvec, bio, iter)
        {
             ...
             * to bio
             */
            if (new_bio && __blk_segment_map_sg_merge(q, &bvec, &bvprv, sg))
                goto next_bvec;
                                      ...
                                      struct request *next)
{
    ...
    if (segments >= queue_max_discard_segments(q))
        goto no_merge;
    if (blk_rq_sectors(req) + bio_sectors(next->bio) > blk_rq_get_max_sectors(req, blk_rq_pos(req)))
        goto no_merge;
                                ...
                                struct request *next)
{
     ...
     */
    if ((blk_rq_sectors(req) + blk_rq_sectors(next)) > blk_rq_get_max_sectors(req, blk_rq_pos(req)))
        return 0;
...
static void blk_rq_set_mixed_merge(struct request *rq)
{
    ...
    {
        WARN_ON_ONCE((bio->bi_opf & REQ_FAILFAST_MASK) && (bio->bi_opf & REQ_FAILFAST_MASK) != ff);
        bio->bi_opf |= ff;
    }
                                     ...
                                     struct request *req, struct request *next)
{
     ...
     * set properly.
     */
    if (((req->rq_flags | next->rq_flags) & RQF_MIXED_MERGE) || (req->cmd_flags & REQ_FAILFAST_MASK) != (next->cmd_flags & REQ_FAILFAST_MASK))
    {
