...
{
    return !list_empty_careful(&hctx->dispatch) || sbitmap_any_bit_set(&hctx->ctx_map) || blk_mq_sched_has_work(hctx);
}
...
static bool blk_mq_check_inflight(struct request *rq, void *priv)
{
    ...
    if (rq->part && blk_do_io_stat(rq) && (!bdev_is_partition(mi->part) || rq->part == mi->part) && blk_mq_rq_state(rq) == MQ_RQ_IN_FLIGHT)
...
void blk_mq_wake_waiters(struct request_queue *q)
{
    ...
    queue_for_each_hw_ctx(q, hctx, i) if (blk_mq_hw_queue_mapped(hctx))
...
static struct request *__blk_mq_alloc_requests(struct blk_mq_alloc_data *data)
{
    ...
    if (q->elevator)
    {
         ...
         * dispatch list.
         */
        if ((data->cmd_flags & REQ_OP_MASK) != REQ_OP_FLUSH && !blk_op_is_passthrough(data->cmd_flags))
        {
                                          ...
                                          blk_opf_t opf, blk_mq_req_flags_t flags, unsigned int hctx_idx)
{
     ...
     * a specific queue.
     */
    if (WARN_ON_ONCE(!(flags & BLK_MQ_REQ_NOWAIT)) || WARN_ON_ONCE(!(flags & BLK_MQ_REQ_RESERVED)))
                        ...
                        unsigned int nr_bytes)
{
...
#ifdef CONFIG_BLK_DEV_INTEGRITY
    if (blk_integrity_rq(req) && req_op(req) == REQ_OP_READ && error == BLK_STS_OK)
    ...
    if (unlikely(error && !blk_rq_is_passthrough(req) && !quiet) && !test_bit(GD_DEAD, &req->q->disk->state))
    {
...
static inline void blk_account_io_done(struct request *req, u64 now)
{
     ...
     */
    if (blk_do_io_stat(req) && req->part && !(req->rq_flags & RQF_FLUSH_SEQ))
    {
        const int sgrp = op_stat_group(req_op(req));
...
static inline bool blk_mq_complete_need_ipi(struct request *rq)
{
    ...
    if (cpu == rq->mq_ctx->cpu || (!test_bit(QUEUE_FLAG_SAME_FORCE, &rq->q->queue_flags) && cpus_share_cache(cpu, rq->mq_ctx->cpu) && cpus_equal_capacity(cpu, rq->mq_ctx->cpu)))
        return false;
...
bool blk_mq_complete_request_remote(struct request *rq)
{
     ...
     */
    if ((rq->mq_hctx->nr_ctx == 1 && rq->mq_ctx->cpu == raw_smp_processor_id()) || rq->cmd_flags & REQ_POLLED)
        return false;
...
void blk_mq_start_request(struct request *rq)
{
    ...
    if (test_bit(QUEUE_FLAG_STATS, &q->queue_flags) && !blk_rq_is_passthrough(rq))
    {
...
static void blk_add_rq_to_plug(struct blk_plug *plug, struct request *rq)
{
    ...
    else if (plug->rq_count >= blk_plug_max_rq_count(plug) || (!blk_queue_nomerges(rq->q) && blk_rq_bytes(last) >= BLK_PLUG_FLUSH_SIZE))
    {
...
static bool blk_mq_rq_inflight(struct request *rq, void *priv)
{
     ...
     */
    if (blk_mq_request_started(rq) && !(blk_queue_quiesced(rq->q) && blk_is_flush_data_rq(rq) && blk_mq_request_completed(rq)))
    {
        bool *busy = priv;
                                 ...
                                 struct request       *rq)
{
    ...
    if (!(hctx->flags & BLK_MQ_F_TAG_QUEUE_SHARED) && !(blk_mq_is_shared_tags(hctx->flags)))
    {
...
out:
    ...
    if (!list_empty(list))
    {
        ...
        bool no_tag = prep == PREP_DISPATCH_NO_TAG && ((hctx->flags & BLK_MQ_F_TAG_QUEUE_SHARED) || blk_mq_is_shared_tags(hctx->flags));
...
void blk_mq_start_stopped_hw_queues(struct request_queue *q, bool async)
{
    ...
    queue_for_each_hw_ctx(q, hctx, i)
        blk_mq_start_stopped_hw_queue(hctx, async || (hctx->flags & BLK_MQ_F_BLOCKING));
}
                                                  ...
                                                  struct request_queue *q, blk_opf_t opf)
{
    ...
    if (!rq || rq->q != q)
        return NULL;
    if (type != rq->mq_hctx->type && (type != HCTX_TYPE_READ || rq->mq_hctx->type != HCTX_TYPE_DEFAULT))
        return NULL;
    if (op_is_flush(rq->cmd_flags) != op_is_flush(opf))
        return NULL;
    return rq;
...
{
    struct blk_mq_tags *tags = hctx->sched_tags ? hctx->sched_tags : hctx->tags;
                            ...
                            struct blk_mq_hw_ctx *hctx, unsigned hctx_idx)
{
    ...
    if (set->ops->init_hctx && set->ops->init_hctx(hctx, set->driver_data, hctx_idx))
...
static void blk_mq_map_swqueue(struct request_queue *q)
{
    ...
    for_each_possible_cpu(i)
    {
        ...
        for (j = 0; j < set->nr_maps; j++)
        {
            ...
            if (!set->tags[hctx_idx] && !__blk_mq_alloc_map_and_rqs(set, hctx_idx))
            {
                /*
                                     ...
                                     struct request_queue  *q)
{
     ...
     */
    if (!list_empty(&set->tag_list) && !(set->flags & BLK_MQ_F_TAG_QUEUE_SHARED))
    {
        set->flags |= BLK_MQ_F_TAG_QUEUE_SHARED;
        /* update existing queue */
                                       ...
                                       int                    new_nr_hw_queues)
{
    ...
    if (set->tags)
        memcpy(new_tags, set->tags, set->nr_hw_queues * sizeof(*set->tags));
    kfree(set->tags);
    set->tags = new_tags;
                                                 ...
                                                 struct request_queue *q)
{
    ...
    list_for_each_entry(qe, head, node) if (qe->q == q) return qe;
                                         ...
                                         int                    nr_hw_queues)
{
     ...
     */
    list_for_each_entry(q, &set->tag_list, tag_set_list) if (!blk_mq_elv_switch_none(&head, q)) goto switch_back;
