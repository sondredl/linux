...
{
    return bfqd->busy_queues[0] + bfqd->busy_queues[1] + bfqd->busy_queues[2];
}
                                       ...
                                       bool                   expiration)
{
    ...
    if (next_in_service)
    {
        ...
        parent_sched_may_change = !sd->next_in_service || new_budget_triggers_change;
    }
...
static void bfq_forget_idle(struct bfq_service_tree *st)
{
    ...
    if (RB_EMPTY_ROOT(&st->active) && last_idle && !bfq_gt(last_idle->finish, st->vtime))
    {
        /*
                                          ...
                                          bool               non_blocking_wait_rq)
{
    ...
    if (entity->sched_data->in_service_entity == entity || entity->tree == &st->active)
        /*
                                  ...
                                  bool               expiration)
{
    ...
    for_each_entity(entity)
    {
        ...
        sd = entity->sched_data;
        if (!bfq_update_next_in_service(sd, entity, expiration) && !expiration)
            /*
                                                 ...
                                                 bool                   expiration)
{
     ...
     */
    if (time_is_before_jiffies(sd->bfq_class_idle_last_service + BFQ_CL_IDLE_TIMEOUT))
    {
    ...
    for (; class_idx < BFQ_IOPRIO_CLASSES; class_idx++)
    {
        ...
        entity = __bfq_lookup_next_entity(st + class_idx,
                                          sd->in_service_entity && !expiration);
...
void bfq_add_bfqq_busy(struct bfq_queue *bfqq)
{
    ...
    if (!hlist_unhashed(&bfqq->woken_list_node) && &bfqq->woken_list_node != bfqq->waker_bfqq->woken_list.first)
    {
