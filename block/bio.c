...
{
    return (gfp & ~(__GFP_DIRECT_RECLAIM | __GFP_IO)) | __GFP_NOMEMALLOC | __GFP_NORETRY | __GFP_NOWARN;
}
                             ...
                             struct bio_set *bs)
{
     ...
     * gfp_flags.
     */
    if (current->bio_list && (!bio_list_empty(&current->bio_list[0]) || !bio_list_empty(&current->bio_list[1])) && bs->rescue_workqueue)
...
static int __bio_clone(struct bio *bio, struct bio *bio_src, gfp_t gfp)
{
    ...
    if (bio->bi_bdev)
    {
        if (bio->bi_bdev == bio_src->bi_bdev && bio_flagged(bio_src, BIO_REMAPPED))
    ...
    if (bio_crypt_clone(bio, bio_src, gfp) < 0)
        return -ENOMEM;
    if (bio_integrity(bio_src) && bio_integrity_clone(bio, bio_src, gfp) < 0)
        return -ENOMEM;
    return 0;
}
                    ...
                    unsigned int max_sectors, bool *same_page)
{
    ...
    if (bio->bi_vcnt > 0)
    {
        ...
        if (bio->bi_vcnt >= min(bio->bi_max_vecs, queue_max_segments(q)))
            return 0;
                 ...
                 unsigned int len, unsigned int offset)
{
    ...
    if (bio->bi_vcnt > 0 && bvec_try_merge_page(&bio->bi_io_vec[bio->bi_vcnt - 1], page, len, offset, &same_page))
    {
        bio->bi_iter.bi_size += len;
        return len;
    }
...
void __bio_release_pages(struct bio *bio, bool mark_dirty)
{
    ...
    bio_for_each_folio_all(fi, bio)
    {
        ...
        nr_pages = (fi.offset + fi.length - 1) / PAGE_SIZE - fi.offset / PAGE_SIZE + 1;
        do
        {
...
{
    if (WARN_ON_ONCE(offset > BIO_MAX_SECTORS || size > BIO_MAX_SECTORS || offset + size > bio_sectors(bio)))
        return;
