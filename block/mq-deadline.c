                                             ...
                                             unsigned long         latest_start)
{
    ...
    if (!list_empty(&per_prio->fifo_list[DD_READ]))
    {
        ...
        if (deadline_fifo_request(dd, per_prio, DD_WRITE) && (dd->starved++ >= dd->writes_starved))
                                                      ...
                                                      unsigned long         now)
{
    ...
    prio_cnt = !!dd_queued(dd, DD_RT_PRIO) + !!dd_queued(dd, DD_BE_PRIO) + !!dd_queued(dd, DD_IDLE_PRIO);
    if (prio_cnt < 2)
        return NULL;
...
{
    return !list_empty_careful(&per_prio->dispatch) || !list_empty_careful(&per_prio->fifo_list[DD_READ]) || !list_empty_careful(&per_prio->fifo_list[DD_WRITE]);
}
...
static u32 dd_owned_by_driver(struct deadline_data *dd, enum dd_prio prio)
{
    ...
    return stats->dispatched + stats->merged - atomic_read(&stats->completed);
}
