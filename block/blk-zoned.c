...
unsigned int bdev_nr_zones(struct block_device *bdev)
{
    ...
    if (!bdev_is_zoned(bdev))
        return 0;
    return (bdev_nr_sectors(bdev) + zone_sectors - 1) >> ilog2(zone_sectors);
}
                                      ...
                                      blk_mode_t mode, const struct blk_zone_range *zrange)
{
    ...
    if (zrange->sector + zrange->nr_sectors <= zrange->sector || zrange->sector + zrange->nr_sectors > get_capacity(bdev->bd_disk))
        /* Out of range */
        return -EINVAL;
                                                  ...
                                                  sector_t        sector)
{
    ...
    {
        if (zwplug->zone_no == zno && atomic_inc_not_zero(&zwplug->ref))
        {
            rcu_read_unlock();
            return zwplug;
        }
                                            ...
                                            struct blk_zone_wplug *zwplug)
{
    ...
    while ((bio = bio_list_pop(&zwplug->bio_list)))
    {
        if (disk_zone_is_full(disk, zwplug->zone_no, wp_offset) || (bio_op(bio) != REQ_OP_ZONE_APPEND && bio_offset_from_zone_start(bio) != wp_offset))
        {
            blk_zone_wplug_bio_io_error(zwplug, bio);
            continue;
        }
...
void blk_zone_write_plug_init_request(struct request *req)
{
    ...
    while (!disk_zone_wplug_is_full(disk, zwplug))
    {
        ...
        if (bio->bi_iter.bi_sector != req_back_sector || !blk_rq_merge_ok(req, bio))
            break;

        WARN_ON_ONCE(bio_op(bio) != REQ_OP_WRITE_ZEROES && !bio->__bi_nr_segments);

        bio_list_pop(&zwplug->bio_list);
        if (bio_attempt_back_merge(req, bio, bio->__bi_nr_segments) != BIO_MERGE_OK)
        {
...
static inline bool disk_need_zone_resources(struct gendisk *disk)
{
     ...
     */
    return queue_is_mq(disk->queue) || queue_emulates_zone_append(disk->queue);
}
