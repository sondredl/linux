...
{
    return (offset & lim->virt_boundary_mask) || ((bprv->bv_offset + bprv->bv_len) & lim->virt_boundary_mask);
}
...
{
    if (req_op(req) == REQ_OP_DISCARD && queue_max_discard_segments(req->q) > 1)
        return true;
    return false;
}
                                         ...
                                         const struct queue_limits *lim)
{
     ...
     */
    return lim->chunk_sectors || bio->bi_vcnt != 1 || bio->bi_io_vec->bv_len + bio->bi_io_vec->bv_offset > PAGE_SIZE;
}
...
{
    return IS_ENABLED(CONFIG_BOUNCE) && q->limits.bounce == BLK_BOUNCE_HIGH && max_low_pfn >= max_pfn;
}
...
{
    return bio_op(bio) == REQ_OP_ZONE_APPEND || bio_flagged(bio, BIO_EMULATES_ZONE_APPEND);
}
                                  ...
                                  sector_t          size)
{
    ...
    issue->value = ((issue->value & BIO_ISSUE_RES_MASK) | (blk_time_get_ns() & BIO_ISSUE_TIME_MASK) | ((u64)size << BIO_ISSUE_SIZE_SHIFT));
}
