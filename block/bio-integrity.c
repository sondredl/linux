                           ...
                           unsigned int len, unsigned int offset)
{
    ...
    if (((bip->bip_iter.bi_size + len) >> SECTOR_SHIFT) > queue_max_hw_sectors(q))
        return 0;
    ...
    if (bip->bip_vcnt > 0)
    {
        ...
        if (bip->bip_vcnt >= min(bip->bip_max_vcnt, queue_max_integrity_segments(q)))
            return 0;
                                    ...
                                    int nr_vecs, ssize_t bytes, ssize_t offset)
{
    ...
    for (i = 0; i < nr_vecs; i = j)
    {
        ...
        for (j = i + 1; j < nr_vecs; j++)
        {
            ...
            if (page_folio(pages[j]) != folio || pages[j] != pages[j - 1] + 1)
                break;
...
bool bio_integrity_prep(struct bio *bio)
{
    ...
    if (bio_data_dir(bio) == READ)
    {
        if (!bi->profile->verify_fn || !(bi->flags & BLK_INTEGRITY_VERIFY))
            return true;
    }
    else
    {
        if (!bi->profile->generate_fn || !(bi->flags & BLK_INTEGRITY_GENERATE))
            return true;
    }
...
bool __bio_integrity_endio(struct bio *bio)
{
    ...
    if (bio_op(bio) == REQ_OP_READ && !bio->bi_status && (bip->bip_flags & BIP_BLOCK_INTEGRITY) && bi->profile->verify_fn)
    {
...
void __init bio_integrity_init(void)
{
     ...
     */
    kintegrityd_wq = alloc_workqueue("kintegrityd", WQ_MEM_RECLAIM | WQ_HIGHPRI | WQ_CPU_INTENSIVE, 1);
    if (!kintegrityd_wq)
        panic("Failed to create kintegrityd\n");

    bip_slab = kmem_cache_create("bio_integrity_payload",
                                 sizeof(struct bio_integrity_payload) + sizeof(struct bio_vec) * BIO_INLINE_VECS,
