...
unsigned int part_in_flight(struct block_device *part)
{
    ...
    for_each_possible_cpu(cpu)
    {
        inflight += part_stat_local_read_cpu(part, in_flight[0], cpu) + part_stat_local_read_cpu(part, in_flight[1], cpu);
    }
    if ((int)inflight < 0)
        inflight = 0;
...

{
    ...
    if (disk->major)
    {
        ...
        if (disk->first_minor > MINORMASK || disk->minors > MINORMASK + 1 || disk->first_minor + disk->minors > MINORMASK + 1)
            goto out_exit_elevator;
    }
    else
    {
                       ...
                       struct device_attribute *attr, char *buf)
{
    ...
    return sprintf(buf,
                   ...
                   (unsigned int)div_u64(stat.nsecs[STAT_READ] + stat.nsecs[STAT_WRITE] + stat.nsecs[STAT_DISCARD] + stat.nsecs[STAT_FLUSH],
                                         NSEC_PER_MSEC),
...
static void disk_release(struct device *dev)
{
     ...
     */
    if (queue_is_mq(disk->queue) && test_bit(GD_OWNS_QUEUE, &disk->state) && !test_bit(GD_ADDED, &disk->state))
