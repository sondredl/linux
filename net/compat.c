...
#define CMSG_COMPAT_FIRSTHDR(msg) \
    (((msg)->msg_controllen) >= sizeof(struct compat_cmsghdr) ? (struct compat_cmsghdr __user *)((msg)->msg_control_user) : (struct compat_cmsghdr __user *)NULL)

#define CMSG_COMPAT_OK(ucmlen, ucmsg, mhdr) \
    ((ucmlen) >= sizeof(struct compat_cmsghdr) && (ucmlen) <= (unsigned long)((mhdr)->msg_controllen - ((char __user *)(ucmsg) - (char __user *)(mhdr)->msg_control_user)))

static inline struct compat_cmsghdr __user *cmsg_compat_nxthdr(struct msghdr                *msg,
                                                               struct compat_cmsghdr __user *cmsg, int cmsg_len)
{
    char __user *ptr = (char __user *)cmsg + CMSG_COMPAT_ALIGN(cmsg_len);
    if ((unsigned long)(ptr + 1 - (char __user *)msg->msg_control_user) > msg->msg_controllen)
        return NULL;
...
int put_cmsg_compat(struct msghdr *kmsg, int level, int type, int len, void *data)
{
    ...
    if (!COMPAT_USE_64BIT_TIME)
    {
        ...
        if (level == SOL_SOCKET && (type == SO_TIMESTAMPNS_OLD || type == SO_TIMESTAMPING_OLD))
        {
