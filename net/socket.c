                           ...
                           struct sk_buff *skb)
{
    ...
    if ((tsflags & SOF_TIMESTAMPING_SOFTWARE) && ktime_to_timespec64_cond(skb->tstamp, tss.ts + 0))
        empty = 0;
    if (shhwtstamps && (tsflags & SOF_TIMESTAMPING_RAW_HARDWARE) && !skb_is_swtx_tstamp(skb, false_tstamp))
    {
        if_index = 0;
        ...
        if (ktime_to_timespec64_cond(hwtstamp, tss.ts + 2))
        {
            ...
            if ((tsflags & SOF_TIMESTAMPING_OPT_PKTINFO) && !skb_is_err_queue(skb))
                           ...
                           unsigned int allowed_msghdr_flags)
{
     ...
     */
    if (used_address && msg_sys->msg_name && used_address->name_len == msg_sys->msg_namelen && !memcmp(&used_address->name, msg_sys->msg_name, used_address->name_len))
    {
                       ...
                       struct timespec64 *timeout)
{
    ...
    if (timeout && poll_select_set_timeout(&end_time, timeout->tv_sec, timeout->tv_nsec))
        return -EINVAL;
