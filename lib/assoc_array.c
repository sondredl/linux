                       ...
                       const void                   *index_key)
{
    ...
    if (assoc_array_walk(array, ops, index_key, &result) != assoc_array_walk_found_terminal_node)
        return NULL;
                                                  ...
                                                  struct assoc_array_walk_result *result)
{
    ...
    for (i = 0; i < ASSOC_ARRAY_FAN_OUT; i++)
    {
        ...
        if (assoc_array_ptr_is_leaf(ptr) && ops->compare_object(assoc_array_ptr_to_leaf(ptr), index_key))
        {
                                            ...
                                            const void                   *index_key)
{
    ...
    switch (assoc_array_walk(array, ops, index_key, &result))
    {
        case assoc_array_walk_found_terminal_node:
            ...
            {
                ptr = node->slots[slot];
                if (ptr && assoc_array_ptr_is_leaf(ptr) && ops->compare_object(assoc_array_ptr_to_leaf(ptr), index_key))
                    goto found_leaf;
            }
            fallthrough;
        case assoc_array_walk_tree_empty:
...
retry_compress:
    ...
    if (nr_free == ASSOC_ARRAY_FAN_OUT - 1)
    {
        ...
        if (assoc_array_ptr_is_meta(ptr) && assoc_array_ptr_is_shortcut(ptr))
        {
