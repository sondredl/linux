...
{
    brace_index--;
    if (!last_parent || brace_index < 0 || (open_brace[brace_index] != xbc_node_index(last_parent)))
...
static inline __init struct xbc_node *find_match_node(struct xbc_node *node, char *k)
{
    while (node)
    {
        ...
        *q++ = '\0';
        switch (c)
        {
            ...
            case '+':
                if (*q++ != '=')
                {
                    ret = xbc_parse_error(c == '+' ? "Wrong '+' operator" : "Wrong ':' operator",
                                          q - 2);
                    break;
                }
                fallthrough;
...
int __init xbc_init(const char *data, size_t size, const char **emsg, int *epos)
{
    ...
    {
        if (emsg)
            *emsg = size ? "Config data is too big" : "Config data is empty";
        return -ERANGE;
    }
