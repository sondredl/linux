                                     ...
                                     unsigned int left_pages, gfp_t gfp_mask)
{
    ...
    if (sgt_append->prv)
    {
        unsigned long next_pfn = (page_to_phys(sg_page(sgt_append->prv)) + sgt_append->prv->offset + sgt_append->prv->length) / PAGE_SIZE;
    ...
    {
        seg_len += PAGE_SIZE;
        if (seg_len >= max_segment || !pages_are_mergeable(pages[i], pages[i - 1]))
        {
            chunks++;
            seg_len = 0;
        }
    ...
    for (i = 0; i < chunks; i++)
    {
        ...
        {
            seg_len += PAGE_SIZE;
            if (seg_len >= max_segment || !pages_are_mergeable(pages[j], pages[j - 1]))
                break;
        }
...
static bool sg_miter_get_next_page(struct sg_mapping_iter *miter)
{
    if (!miter->__remaining)
    {
        ...
        miter->__remaining = sg->offset + sg->length - (miter->piter.sg_pgoffset << PAGE_SHIFT) - miter->__offset;
