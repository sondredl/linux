...
#define DEPOT_MAX_POOLS \
    (((1LL << (DEPOT_POOL_INDEX_BITS)) - 1 < DEPOT_POOLS_CAP) ? (1LL << (DEPOT_POOL_INDEX_BITS)) - 1 : DEPOT_POOLS_CAP)
 ...
 */
static inline int stackdepot_memcmp(const unsigned long *u1, const unsigned long *u2,
                                    unsigned int n)
{
    for (; n--; u1++, u2++)
    {
