                         ...
                         struct flag_settings      *modifiers)
{
    ...
    list_for_each_entry(dt, &ddebug_tables, link)
    {
        ...
        if (query->module && !match_wildcard(query->module, dt->mod_name))
            continue;
        ...
        for (i = 0; i < dt->num_ddebugs; i++)
        {
            ...
            if (query->filename && !match_wildcard(query->filename, dp->filename) && !match_wildcard(query->filename, kbasename(dp->filename)) && !match_wildcard(query->filename, trim_prefix(dp->filename)))
                continue;

            /* match against the function */
            if (query->function && !match_wildcard(query->function, dp->function))
                continue;
                              ...
                              struct ddebug_query *query, const char *modname)
{
    ...
    for (i = 0; i < nwords; i += 2)
    {
        ...
        {
            string_unescape_inplace(arg, UNESCAPE_SPACE | UNESCAPE_OCTAL | UNESCAPE_SPECIAL);
