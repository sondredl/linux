                                ...
                                unsigned int key_id, bool should_create_root)
{
    ...
    if (should_create_root && memcmp(world->next_root_buf, root->buf, sizeof(root->buf)))
    {
                            ...
                            const char                        **errmsg)
{
    ...
    if (stats_info->stats.user_count != expect_stats_info->stats.user_count)
    {
        if (errmsg)
            *errmsg = "Incorrect user count";
        return -EINVAL;
    }
    if (stats_info->stats.delta_user_count != expect_stats_info->stats.delta_user_count)
    {
        if (errmsg)
...
static int
    check_expect_stats_key_id(const struct objagg_obj_stats_info *stats_info,
                              ...
                              const char                        **errmsg)
{
    if (obj_to_key_id(stats_info->objagg_obj) != expect_stats_info->key_id)
    {
        if (errmsg)
