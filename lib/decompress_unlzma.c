                                    ...
                                    int lc, uint32_t literal_pos_mask)
{
    ...
    prob = (p + LZMA_LITERAL + (LZMA_LIT_SIZE * (((get_pos(wr) & literal_pos_mask) << lc) + (wr->previous_byte >> (8 - lc)))));
                                    ...
                                    int pos_state, uint16_t *prob)
{
    ...
    if (cst->state < 4)
    {
        ...
        prob =
            p + LZMA_POS_SLOT + ((len < LZMA_NUM_LEN_TO_POS_STATES ? len : LZMA_NUM_LEN_TO_POS_STATES - 1) << LZMA_NUM_POS_SLOT_BITS);
