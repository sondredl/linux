...
static noinline void check_xa_shrink(struct xarray *xa)
{
    ...
    for (order = 0; order < max_order; order++)
    {
        ...
        node = xa_head(xa);
        rcu_read_unlock();
        XA_BUG_ON(xa, xa_store_index(xa, ULONG_MAX, GFP_KERNEL) != NULL);
        rcu_read_lock();
...
static noinline void check_reserve(struct xarray *xa)
{
    ...
    XA_BUG_ON(xa, xa_cmpxchg(xa, 12345678, XA_ZERO_ENTRY,
                             xa_mk_value(12345678), GFP_NOWAIT)
                      != NULL);
    xa_release(xa, 12345678);
    xa_erase_index(xa, 12345678);
    XA_BUG_ON(xa, !xa_empty(xa));
    ...
    XA_BUG_ON(xa, xa_reserve(xa, 12345678, GFP_KERNEL) != 0);
    XA_BUG_ON(xa, xa_insert(xa, 12345678, xa_mk_value(12345678), 0) != -EBUSY);
    XA_BUG_ON(xa, xa_empty(xa));
...
retry:
    ...
    XA_BUG_ON(xa, xa_load(xa, start + (1UL << order) - 1) != xa_mk_index(start));
...
static noinline void check_multi_find_1(struct xarray *xa, unsigned order)
{
    ...
    index = 0;
    XA_BUG_ON(xa, xa_find(xa, &index, ULONG_MAX, XA_PRESENT) != xa_mk_value(multi));
    XA_BUG_ON(xa, index != multi);
    index = multi + 1;
    XA_BUG_ON(xa, xa_find(xa, &index, ULONG_MAX, XA_PRESENT) != xa_mk_value(multi));
    XA_BUG_ON(xa, (index < multi) || (index >= next));
    XA_BUG_ON(xa, xa_find_after(xa, &index, ULONG_MAX, XA_PRESENT) != xa_mk_value(next));
...
static noinline void check_find_1(struct xarray *xa)
{
    ...
    for (i = 0; i < 100; i++)
    {
        ...
        for (j = 0; j < i; j++)
        {
            XA_BUG_ON(xa, xa_store_index(xa, j, GFP_KERNEL) != NULL);
    ...
    for (order = 0; order < 20; order++)
    {
             ...
             offset += (1UL << order))
        {
                 ...
                 index += (1UL << order))
            {
                ...
                XA_BUG_ON(xa, xa_load(xa, index) != xa_mk_index(index));
