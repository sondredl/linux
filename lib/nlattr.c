                                       ...
                                       unsigned int             validate)
{
    ...
    if (pt->validation_type == NLA_VALIDATE_RANGE_WARN_TOO_LONG && pt->type == NLA_BINARY && value > range.max)
    {
                        ...
                        struct netlink_ext_ack *extack, unsigned int depth)
{
    ...
    {
        if ((pt->type == NLA_NESTED || pt->type == NLA_NESTED_ARRAY) && !(nla->nla_type & NLA_F_NESTED))
        {
            NL_SET_ERR_MSG_ATTR_POL(extack, nla, pt,
                                    "NLA_F_NESTED is missing");
            return -EINVAL;
        }
        if (pt->type != NLA_NESTED && pt->type != NLA_NESTED_ARRAY && pt->type != NLA_UNSPEC && (nla->nla_type & NLA_F_NESTED))
        {
            NL_SET_ERR_MSG_ATTR_POL(extack, nla, pt,
...
struct nlattr *nla_find(const struct nlattr *head, int len, int attrtype)
{
    ...
    nla_for_each_attr(nla, head, len, rem) if (nla_type(nla) == attrtype) return (struct nlattr *)nla;
