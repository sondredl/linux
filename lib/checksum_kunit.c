...
static void assert_setup_correct(struct kunit *test)
{
    ...
    CHECK_EQ(sizeof(expected_results) / sizeof(expected_results[0]),
             MAX_LEN);
    CHECK_EQ(sizeof(init_sums_no_overflow) / sizeof(init_sums_no_overflow[0]),
             MAX_LEN);
}
...
static void test_ip_fast_csum(struct kunit *test)
{
    ...
    for (int len = IPv4_MIN_WORDS; len < IPv4_MAX_WORDS; len++)
    {
        for (int index = 0; index < NUM_IP_FAST_CSUM_TESTS; index++)
        {
            ...
            expected =
                expected_fast_csum[(len - IPv4_MIN_WORDS) * NUM_IP_FAST_CSUM_TESTS + index];
...
static void test_csum_ipv6_magic(struct kunit *test)
{
    ...
    const int proto_offset = sizeof(struct in6_addr) + sizeof(struct in6_addr) + sizeof(int);
    const int csum_offset  = sizeof(struct in6_addr) + sizeof(struct in6_addr) + sizeof(int) + sizeof(char);

    for (int i = 0; i < NUM_IPv6_TESTS; i++)
    {
        saddr = (const struct in6_addr *)(random_buf + i);
        daddr = (const struct in6_addr *)(random_buf + i + daddr_offset);
