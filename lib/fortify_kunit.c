...
static void fortify_test_strscpy(struct kunit *test)
{
    ...
    KUNIT_ASSERT_EQ(test, strscpy(pad.buf, src, sizeof(pad.buf) + unconst - 1),
                    -E2BIG);
    KUNIT_EXPECT_EQ(test, fortify_write_overflows, 0);
    ...
    KUNIT_ASSERT_EQ(test, strscpy(pad.buf, src, sizeof(pad.buf) + unconst),
                    -E2BIG);
    KUNIT_EXPECT_EQ(test, fortify_write_overflows, 0);
    ...
    /* And much further... */
    KUNIT_ASSERT_EQ(test, strscpy(pad.buf, src, sizeof(src) * 2 + unconst),
                    -E2BIG);
    /* Should catch the overflow. */
    KUNIT_EXPECT_EQ(test, fortify_write_overflows, 2);
    KUNIT_EXPECT_EQ(test, pad.buf[sizeof(pad.buf) - 1], '\0');
