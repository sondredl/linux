...
static int __init crc32c_test(void)
{
    ...
    for (i = 0; i < 100; i++)
    {
        ...
        crc ^= __crc32c_le(test[i].crc, test_buf + test[i].start, test[i].length);
    }
    ...
    for (i = 0; i < 100; i++)
    {
        if (test[i].crc32c_le != __crc32c_le(test[i].crc, test_buf + test[i].start, test[i].length))
            errors++;
    }
...
static int __init crc32c_combine_test(void)
{
    ...
    for (i = 0; i < 10; i++)
    {
        ...
        for (j = 0; j <= test[i].length; ++j)
        {
            ...
            crc1 = __crc32c_le(test[i].crc, test_buf + test[i].start, len1);
            crc2 = __crc32c_le(0, test_buf + test[i].start + len1, len2);

            if (!(crc_full == __crc32c_le_combine(crc1, crc2, len2) && crc_full == test[i].crc32c_le))
                errors++;
            runs++;
            cond_resched();
        }
...
static int __init crc32_test(void)
{
    ...
    for (i = 0; i < 100; i++)
    {
        ...
        crc ^= crc32_le(test[i].crc, test_buf + test[i].start, test[i].length);

        crc ^= crc32_be(test[i].crc, test_buf + test[i].start, test[i].length);
    }
    ...
    for (i = 0; i < 100; i++)
    {
        if (test[i].crc_le != crc32_le(test[i].crc, test_buf + test[i].start, test[i].length))
            errors++;

        if (test[i].crc_be != crc32_be(test[i].crc, test_buf + test[i].start, test[i].length))
            errors++;
    }
    nsec = ktime_get_ns() - nsec;
...
static int __init crc32_combine_test(void)
{
    ...
    for (i = 0; i < 10; i++)
    {
        ...
        for (j = 0; j <= test[i].length; ++j)
        {
            ...
            crc1 = crc32_le(test[i].crc, test_buf + test[i].start, len1);
            crc2 = crc32_le(0, test_buf + test[i].start + len1, len2);

            if (!(crc_full == crc32_le_combine(crc1, crc2, len2) && crc_full == test[i].crc_le))
                errors++;
            runs++;
            cond_resched();
        }
