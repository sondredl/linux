                                  ...
                                  struct maple_subtree_state *mast, unsigned char count)
{
    ...
    if (((mast->orig_l->min != 0) || (mast->orig_r->max != ULONG_MAX)) && unlikely(mast->bn->b_end <= mt_min_slots[mast->bn->type]))
                                            ...
                                            struct ma_wr_state *r_wr_mas)
{
    ...
    else if ((r_mas->last == r_wr_mas->r_max) && (r_mas->last < r_mas->max) && !mas_slot_locked(r_mas, r_wr_mas->slots, r_mas->offset + 1))
    {
...
static inline void mas_wr_extend_null(struct ma_wr_state *wr_mas)
{
    ...
    else
    {
        ...
        if ((mas->last == wr_mas->end_piv) && (mas->end != wr_mas->offset_end) && !wr_mas->slots[wr_mas->offset_end + 1])
        {
            wr_mas->offset_end++;
