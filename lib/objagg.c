                             ...
                             struct objagg_hints *objagg_hints, void *priv)
{
    ...
    if (WARN_ON(!ops || !ops->root_create || !ops->root_destroy || !ops->delta_check || !ops->delta_create || !ops->delta_destroy))
...
static int objagg_stats_info_sort_cmp_func(const void *a, const void *b)
{
    ...
    if (stats_info1->stats.delta_user_count != stats_info2->stats.delta_user_count)
        return stats_info2->stats.delta_user_count - stats_info1->stats.delta_user_count;
                                ...
                                const void                    *obj)
{
    ...
    ptr += ht->p.key_offset;
    return ops->hints_obj_cmp ? ops->hints_obj_cmp(ptr, arg->key) : memcmp(ptr, arg->key, ht->p.key_len);
}
