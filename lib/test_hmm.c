...
{
    return (mdevice->zone_device_type == HMM_DMIRROR_MEMORY_DEVICE_PRIVATE) ? true : false;
}

static enum migrate_vma_direction
    dmirror_select_device(struct dmirror *dmirror)
{
    return (dmirror->mdevice->zone_device_type == HMM_DMIRROR_MEMORY_DEVICE_PRIVATE) ? MIGRATE_VMA_SELECT_DEVICE_PRIVATE : MIGRATE_VMA_SELECT_DEVICE_COHERENT;
}
                                        ...
                                        unsigned long                    cur_seq)
{
     ...
     */
    if (range->event == MMU_NOTIFY_MIGRATE && range->owner == dmirror->mdevice)
        return true;
                                  ...
                                  struct page          **ppage)
{
    ...
    switch (mdevice->zone_device_type)
    {
        ...
        case HMM_DMIRROR_MEMORY_DEVICE_COHERENT:
            devmem->pagemap.range.start = (MINOR(mdevice->cdevice.dev) - 2) ? spm_addr_dev0 : spm_addr_dev1;
            devmem->pagemap.range.end   = devmem->pagemap.range.start + DEVMEM_CHUNK_SIZE - 1;
                                           ...
                                           struct dmirror     *dmirror)
{
        ...
        src++, dst++)
    {
        ...
        *dst = migrate_pfn(page_to_pfn(dpage));
        if ((*src & MIGRATE_PFN_WRITE) || (!spage && args->vma->vm_flags & VM_WRITE))
                                                      ...
                                                      struct dmirror     *dmirror)
{
        ...
        src++, dst++)
    {
        ...
        if (WARN_ON(!is_device_private_page(spage) && !is_device_coherent_page(spage)))
            continue;
    ...
    dmirror_successful_migrated_pages(struct migrate_vma *migrate)
{
    ...
    {
        if (migrate->src[i] & MIGRATE_PFN_VALID && migrate->src[i] & MIGRATE_PFN_MIGRATE)
            cpages++;
    }
    return cpages;
}
