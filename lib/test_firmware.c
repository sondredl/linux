...
static struct test_firmware_upload *upload_lookup_name(const char *name)
{
    ...
    list_for_each_entry(tst, &test_upload_list, node) if (strncmp(name, tst->name, strlen(tst->name)) == 0) return tst;
                           ...
                           char                    *buf)
{
    ...
    len += scnprintf(buf + len, PAGE_SIZE - len,
                     "send_uevent:\t\t%s\n",
                     test_fw_config->send_uevent ? "FW_ACTION_UEVENT" : "FW_ACTION_NOUEVENT");
    len += scnprintf(buf + len, PAGE_SIZE - len,
                     "into_buf:\t\t%s\n",
                                        ...
                                        const char *buf, size_t count)
{
    ...
    if (rc == count)
        test_fw_config->req_firmware = test_fw_config->sync_direct ? request_firmware_direct : request_firmware;
    return rc;
}
                                              ...
                                              const char *buf, size_t count)
{
    ...
    if (firmware->size != sizeof(test_data) || memcmp(firmware->data, test_data, sizeof(test_data)) != 0)
    {
...
static int test_fw_run_batch_request(void *data)
{
    ...
    if (test_fw_config->into_buf)
    {
        ...
        if (test_fw_config->partial)
            req->rc = request_partial_firmware_into_buf(&req->fw,
                                                        req->name,
                                                        req->dev,
                                                        test_buf,
                                                        test_fw_config->buf_size,
...
static ssize_t trigger_batched_requests_async_store(struct device           *dev,
                                                    struct device_attribute *attr,
                                                    const char *buf, size_t count)
{
    struct test_batched_req *req;
    ...
    send_uevent = test_fw_config->send_uevent ? FW_ACTION_UEVENT : FW_ACTION_NOUEVENT;
                                                 ...
                                                 const u8 *data, u32 size)
{
    ...
    if (tst->inject.err_code == FW_UPLOAD_ERR_NONE || strncmp(tst->inject.prog, progress, strlen(progress)) != 0)
        return FW_UPLOAD_ERR_NONE;
                                ...
                                char                    *buf)
{
    ...
    mutex_lock(&test_fw_mutex);
    list_for_each_entry(tst_iter, &test_upload_list, node) if (tst_iter->name == test_fw_config->upload_name)
    {
        tst = tst_iter;
        break;
    }
