                      ...
                      const uint8_t *src)
{
    ...
    for (i = 0; i < nwords; i++, src += 4)
        dst[i] = ((u32)swap_bits(bch, src[0]) << 24) | ((u32)swap_bits(bch, src[1]) << 16) | ((u32)swap_bits(bch, src[2]) << 8) | swap_bits(bch, src[3]);

    memcpy(pad, src, BCH_ECC_BYTES(bch) - 4 * nwords);
    dst[nwords] = ((u32)swap_bits(bch, pad[0]) << 24) | ((u32)swap_bits(bch, pad[1]) << 16) | ((u32)swap_bits(bch, pad[2]) << 8) | swap_bits(bch, pad[3]);
}
                ...
                unsigned int len, uint8_t *ecc)
{
    ...
    while (mlen--)
    {
        ...
        w = cpu_to_be32(*pdata++);
        if (bch->swap_bits)
            w = (u32)swap_bits(bch, w) | ((u32)swap_bits(bch, w >> 8) << 8) | ((u32)swap_bits(bch, w >> 16) << 16) | ((u32)swap_bits(bch, w >> 24) << 24);
        w ^= r[0];
                              ...
                              unsigned int *roots)
{
    ...
    for (i = 0; i < m; i++)
    {
        rows[i + 1] = bch->a_pow_tab[4 * i] ^ (a ? bch->a_pow_tab[mod_s(bch, k)] : 0) ^ (b ? bch->a_pow_tab[mod_s(bch, j)] : 0);
        j++;
        k += 2;
    }
    /*
