...
{
    return !test_bit(IO_ACCT_STALLED_BIT, &acct->flags) && !wq_list_empty(&acct->work_list);
}
                                   ...
                                   task_work_func_t   func)
{
     ...
     * runs the task_work.
     */
    if (test_bit(0, &worker->create_state) || test_and_set_bit_lock(0, &worker->create_state))
        goto fail_release;
...
void io_wq_enqueue(struct io_wq *wq, struct io_wq_work *work)
{
     ...
     */
    if (test_bit(IO_WQ_BIT_EXIT, &wq->state) || (work->flags & IO_WQ_WORK_CANCEL))
    {
    ...
    if (do_create && ((work_flags & IO_WQ_WORK_CONCURRENT) || !atomic_read(&acct->nr_running)))
    {
        bool did_create;
