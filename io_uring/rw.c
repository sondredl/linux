...
static bool io_rw_should_reissue(struct io_kiocb *req)
{
    ...
    if (!S_ISBLK(mode) && !S_ISREG(mode))
        return false;
    if ((req->flags & REQ_F_NOWAIT) || (io_wq_current_is_worker() && !(ctx->flags & IORING_SETUP_IOPOLL)))
        return false;
    /*
...
static ssize_t loop_rw_iter(int ddir, struct io_rw *rw, struct iov_iter *iter)
{
    ...
    if (kiocb->ki_flags & IOCB_HIPRI)
        return -EOPNOTSUPP;
    if ((kiocb->ki_flags & IOCB_NOWAIT) && !(kiocb->ki_filp->f_flags & O_NONBLOCK))
        return -EAGAIN;
...
static bool io_rw_should_retry(struct io_kiocb *req)
{
     ...
     */
    if (io_file_can_poll(req) || !(req->file->f_op->fop_flags & FOP_BUFFER_RASYNC))
        return false;
...
static int __io_read(struct io_kiocb *req, unsigned int issue_flags)
{
    ...
    else if (ret == req->cqe.res || ret <= 0 || !force_nonblock || (req->flags & REQ_F_NOWAIT) || !need_complete_io(req))
    {
...
int io_write(struct io_kiocb *req, unsigned int issue_flags)
{
    ...
    if (force_nonblock)
    {
        ...
        if (!(kiocb->ki_flags & IOCB_DIRECT) && !(req->file->f_op->fop_flags & FOP_BUFFER_WASYNC) && (req->flags & REQ_F_ISREG))
            goto ret_eagain;
