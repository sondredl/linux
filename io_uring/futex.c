...
{
    if (test_bit(0, &iof->futexv_owned) || test_and_set_bit_lock(0, &iof->futexv_owned))
        return false;
    return true;
}
                    ...
                    unsigned int issue_flags)
{
    ...
    {
        if (req->cqe.user_data != cd->data && !(cd->flags & IORING_ASYNC_CANCEL_ANY))
            continue;
...
int io_futex_prep(struct io_kiocb *req, const struct io_uring_sqe *sqe)
{
    ...
    if (unlikely(sqe->len || sqe->futex_flags || sqe->buf_index || sqe->file_index))
        return -EINVAL;
    ...
    if (!futex_validate_input(iof->futex_flags, iof->futex_val) || !futex_validate_input(iof->futex_flags, iof->futex_mask))
        return -EINVAL;
...
int io_futexv_prep(struct io_kiocb *req, const struct io_uring_sqe *sqe)
{
    ...
    if (unlikely(sqe->fd || sqe->buf_index || sqe->file_index || sqe->addr2 || sqe->futex_flags || sqe->addr3))
        return -EINVAL;
