                             ...
                             bool                       is_timeout_link)
{
    ...
    if (flags & ~(IORING_TIMEOUT_ABS | IORING_TIMEOUT_CLOCK_MASK | IORING_TIMEOUT_ETIME_SUCCESS | IORING_TIMEOUT_MULTISHOT))
        return -EINVAL;
                             ...
                             bool cancel_all)
{
    ...
    list_for_each_entry_safe(timeout, tmp, &ctx->timeout_list, list)
    {
        ...
        if (io_match_task(req, tsk, cancel_all) && io_kill_timeout(req, -ECANCELED))
            canceled++;
    }
    spin_unlock_irq(&ctx->timeout_lock);
    spin_unlock(&ctx->completion_lock);
