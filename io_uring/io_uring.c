...
#define SQE_COMMON_FLAGS (IOSQE_FIXED_FILE | IOSQE_IO_LINK | IOSQE_IO_HARDLINK | IOSQE_ASYNC)

#define SQE_VALID_FLAGS (SQE_COMMON_FLAGS | IOSQE_BUFFER_SELECT | IOSQE_IO_DRAIN | IOSQE_CQE_SKIP_SUCCESS)

#define IO_REQ_CLEAN_FLAGS (REQ_F_BUFFER_SELECTED | REQ_F_NEED_CLEANUP | REQ_F_POLLED | REQ_F_INFLIGHT | REQ_F_CREDS | REQ_F_ASYNC_DATA)
...
static void io_prep_async_work(struct io_kiocb *req)
{
    ...
    if (req->file && (req->flags & REQ_F_ISREG))
    {
        ...
        if (should_hash && (req->file->f_flags & O_DIRECT) && (req->file->f_op->fop_flags & FOP_DIO_PARALLEL_WRITE))
            should_hash = false;
    ...
    __must_hold(&ctx->uring_lock)
{
    ...
    __wq_list_for_each(node, &state->compl_reqs)
    {
        ...
        if (!(req->flags & REQ_F_CQE_SKIP) && unlikely(!io_fill_cqe_req(ctx, req)))
        {
            if (ctx->lockless_cq)
            {
                spin_lock(&ctx->completion_lock);
...
static int io_iopoll_check(struct io_ring_ctx *ctx, long min)
{
    ...
    do
    {
        ...
        if (wq_list_empty(&ctx->iopoll_list) || io_task_work_pending(ctx))
        {
            ...
            if (task_work_pending(current) || wq_list_empty(&ctx->iopoll_list))
            {
                mutex_unlock(&ctx->uring_lock);
                io_run_task_work();
                mutex_lock(&ctx->uring_lock);
...
{
    io_tw_lock(req->ctx, ts);
    return io_issue_sqe(req, IO_URING_F_NONBLOCK | IO_URING_F_MULTISHOT | IO_URING_F_COMPLETE_DEFER);
}
...
void io_wq_submit_work(struct io_wq_work *work)
{
    ...
    if (req->flags & REQ_F_APOLL_MULTISHOT)
    {
        ...
        if (!io_file_can_poll(req))
            goto fail;
        if (req->file->f_flags & O_NONBLOCK || req->file->f_mode & FMODE_NOWAIT)
        {
            err = -ECANCELED;
    ...
    __must_hold(&ctx->uring_lock)
{
    ...
    else if (unlikely(req->flags & (IO_REQ_LINK_FLAGS | REQ_F_FORCE_ASYNC | REQ_F_FAIL)))
    {
        if (req->flags & IO_REQ_LINK_FLAGS)
        {
            link->head = req;
    ...
    __must_hold(&ctx->uring_lock)
{
    ...
    do
    {
         ...
         */
        if (unlikely(io_submit_sqe(ctx, req, sqe)) && !(ctx->flags & IORING_SETUP_SUBMIT_ALL))
        {
            left--;
            break;
        }
                                                ...
                                                bool                cancel_all)
{
    ...
    if ((!(ctx->flags & IORING_SETUP_SQPOLL) && cancel_all) || (ctx->sq_data && ctx->sq_data->thread == current))
    {
        while (!wq_list_empty(&ctx->iopoll_list))
        {
...
__cold void io_uring_cancel_generic(bool cancel_all, struct io_sq_data *sqd)
{
    ...
    do
    {
        ...
        xa_for_each(&tctx->xa, index, node)
        {
            ...
            {
                WARN_ON_ONCE(node->ctx->submitter_task && node->ctx->submitter_task != current);
                goto end_wait;
            }
        }
        /*
                ...
                size_t, argsz)
{
    ...
    if (unlikely(flags & ~(IORING_ENTER_GETEVENTS | IORING_ENTER_SQ_WAKEUP | IORING_ENTER_SQ_WAIT | IORING_ENTER_EXT_ARG | IORING_ENTER_REGISTERED_RING)))
        return -EINVAL;
                                  ...
                                  struct io_uring_params __user *params)
{
    ...
    if ((ctx->flags & IORING_SETUP_DEFER_TASKRUN) && !(ctx->flags & IORING_SETUP_IOPOLL) && !(ctx->flags & IORING_SETUP_SQPOLL))
    ...
    if (ctx->flags & IORING_SETUP_SQPOLL)
    {
        ...
        if (ctx->flags & (IORING_SETUP_COOP_TASKRUN | IORING_SETUP_TASKRUN_FLAG | IORING_SETUP_DEFER_TASKRUN))
            goto err;
    ...
    {
        if (ctx->flags & IORING_SETUP_TASKRUN_FLAG && !(ctx->flags & IORING_SETUP_DEFER_TASKRUN))
            goto err;
     ...
     * that.
     */
    if (ctx->flags & IORING_SETUP_DEFER_TASKRUN && !(ctx->flags & IORING_SETUP_SINGLE_ISSUER))
    {
        goto err;
    }
...
static long io_uring_setup(u32 entries, struct io_uring_params __user *params)
{
    ...
    if (p.flags & ~(IORING_SETUP_IOPOLL | IORING_SETUP_SQPOLL | IORING_SETUP_SQ_AFF | IORING_SETUP_CQSIZE | IORING_SETUP_CLAMP | IORING_SETUP_ATTACH_WQ | IORING_SETUP_R_DISABLED | IORING_SETUP_SUBMIT_ALL | IORING_SETUP_COOP_TASKRUN | IORING_SETUP_TASKRUN_FLAG | IORING_SETUP_SQE128 | IORING_SETUP_CQE32 | IORING_SETUP_SINGLE_ISSUER | IORING_SETUP_DEFER_TASKRUN | IORING_SETUP_NO_MMAP | IORING_SETUP_REGISTERED_FD_ONLY | IORING_SETUP_NO_SQARRAY))
        return -EINVAL;
...
#define BUILD_BUG_SQE_ELEM_SIZE(eoffset, esize, ename) \
    ...
    BUILD_BUG_ON(sizeof(struct io_uring_files_update) != sizeof(struct io_uring_rsrc_update));
    BUILD_BUG_ON(sizeof(struct io_uring_rsrc_update) > sizeof(struct io_uring_rsrc_update2));

    /* ->buf_index is u16 */
    BUILD_BUG_ON(offsetof(struct io_uring_buf_ring, bufs) != 0);
    BUILD_BUG_ON(offsetof(struct io_uring_buf, resv) != offsetof(struct io_uring_buf_ring, tail));
