                            ...
                            struct io_hash_table *table)
{
    ...
    if (cd->flags & (IORING_ASYNC_CANCEL_FD | IORING_ASYNC_CANCEL_OP | IORING_ASYNC_CANCEL_ANY))
...
int io_poll_remove_prep(struct io_kiocb *req, const struct io_uring_sqe *sqe)
{
    ...
    if (flags & ~(IORING_POLL_UPDATE_EVENTS | IORING_POLL_UPDATE_USER_DATA | IORING_POLL_ADD_MULTI))
        return -EINVAL;
...
int io_poll_remove(struct io_kiocb *req, unsigned int issue_flags)
{
    ...
    struct io_cancel_data  cd          = {
                  .ctx  = ctx,
                  .data = poll_update->old_user_data,
    };
    struct io_hash_bucket *bucket;
