...
int io_shutdown_prep(struct io_kiocb *req, const struct io_uring_sqe *sqe)
{
    ...
    if (unlikely(sqe->off || sqe->addr || sqe->rw_flags || sqe->buf_index || sqe->splice_fd_in))
        return -EINVAL;
...
{
    if ((req->flags & (REQ_F_APOLL_MULTISHOT | REQ_F_BUFFER_SELECT)) == (REQ_F_APOLL_MULTISHOT | REQ_F_BUFFER_SELECT))
    {
        int hdr;
...
static int io_recvmsg_prep_setup(struct io_kiocb *req)
{
    ...
    #define RECVMSG_FLAGS (IORING_RECVSEND_POLL_FIRST | IORING_RECV_MULTISHOT | IORING_RECVSEND_BUNDLE)
                                  ...
                                  bool mshot_finished, unsigned issue_flags)
{
     ...
     * receive from this socket.
     */
    if ((req->flags & REQ_F_APOLL_MULTISHOT) && !mshot_finished && io_req_post_cqe(req, *ret, cflags | IORING_CQE_F_MORE))
    {
                                ...
                                unsigned int flags, bool *finished)
{
    ...
    *finished = err <= 0;
    ...
    BUILD_BUG_ON(offsetof(struct io_recvmsg_multishot_hdr, addr) != sizeof(struct io_uring_recvmsg_out));
    ...
    return sizeof(struct io_uring_recvmsg_out) + kmsg->namelen + kmsg->controllen + err;
}
                              ...
                              size_t *len, unsigned int issue_flags)
{
     ...
     */
    if (!(issue_flags & IO_URING_F_UNLOCKED) && sr->flags & IORING_RECVSEND_BUNDLE)
    {
        struct buf_sel_arg arg = {
            .iovs    = &kmsg->fast_iov,
            .nr_iovs = 1,
...
int io_send_zc(struct io_kiocb *req, unsigned int issue_flags)
{
    ...
    if (!(req->flags & REQ_F_POLLED) && (zc->flags & IORING_RECVSEND_POLL_FIRST))
        return -EAGAIN;
...
void io_sendrecv_fail(struct io_kiocb *req)
{
    ...
    if ((req->flags & REQ_F_NEED_CLEANUP) && (req->opcode == IORING_OP_SEND_ZC || req->opcode == IORING_OP_SENDMSG_ZC))
        req->cqe.flags |= IORING_CQE_F_MORE;
}

    #define ACCEPT_FLAGS (IORING_ACCEPT_MULTISHOT | IORING_ACCEPT_DONTWAIT | IORING_ACCEPT_POLL_FIRST)
...
int io_accept_prep(struct io_kiocb *req, const struct io_uring_sqe *sqe)
{
    ...
    if (accept->file_slot)
    {
        if (accept->flags & SOCK_CLOEXEC)
            return -EINVAL;
        if (accept->iou_flags & IORING_ACCEPT_MULTISHOT && accept->file_slot != IORING_FILE_INDEX_ALLOC)
            return -EINVAL;
    }
...
int io_accept(struct io_kiocb *req, unsigned int issue_flags)
{
    ...
    if (!(req->flags & REQ_F_POLLED) && accept->iou_flags & IORING_ACCEPT_POLL_FIRST)
        return -EAGAIN;
...
retry:
    ...
    if (IS_ERR(file))
    {
        if (!fixed)
            put_unused_fd(fd);
        ret = PTR_ERR(file);
        if (ret == -EAGAIN && force_nonblock && !(accept->iou_flags & IORING_ACCEPT_DONTWAIT))
        {
            /*
