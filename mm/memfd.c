...
{
    return folio_ref_count(folio) - folio_mapcount(folio) != folio_nr_pages(folio);
}
...
#define F_ALL_SEALS (F_SEAL_SEAL | F_SEAL_EXEC | F_SEAL_SHRINK | F_SEAL_GROW | F_SEAL_WRITE | F_SEAL_FUTURE_WRITE)
                ...
                unsigned int, flags)
{
    ...
    else
    {
        ...
        if (flags & ~(unsigned int)(MFD_ALL_FLAGS | (MFD_HUGE_MASK << MFD_HUGE_SHIFT)))
            return -EINVAL;
    }
