...
#ifdef CONFIG_NUMA
    if (auto_movable_numa_aware && !auto_movable_can_online_movable(nid, group, nr_pages))
        goto kernel_zone;
#endif /* CONFIG_NUMA */
                       ...
                       struct zone *zone, struct memory_group *group)
{
     ...
     * aligned.
     */
    if (WARN_ON_ONCE(!nr_pages || !pageblock_aligned(pfn) || !IS_ALIGNED(pfn + nr_pages, PAGES_PER_SECTION)))
        return -EINVAL;
...
int __ref add_memory_resource(int nid, struct resource *res, mhp_t mhp_flags)
{
     ...
     */
    if ((mhp_flags & MHP_MEMMAP_ON_MEMORY) && mhp_supports_memmap_on_memory())
    {
                              ...
                              const char *resource_name, mhp_t mhp_flags)
{
    ...
    if (!resource_name || strstr(resource_name, "System RAM (") != resource_name || resource_name[strlen(resource_name) - 1] != ')')
        return -EINVAL;
                        ...
                        struct zone *zone, struct memory_group *group)
{
     ...
     * aligned.
     */
    if (WARN_ON_ONCE(!nr_pages || !pageblock_aligned(start_pfn) || !IS_ALIGNED(start_pfn + nr_pages, PAGES_PER_SECTION)))
        return -EINVAL;
     ...
     */
    if (WARN_ON_ONCE(page_zone(pfn_to_page(start_pfn)) != zone || page_zone(pfn_to_page(end_pfn - 1)) != zone))
    {
        ret    = -EINVAL;
...
int offline_and_remove_memory(u64 start, u64 size)
{
    ...
    if (!IS_ALIGNED(start, memory_block_size_bytes()) || !IS_ALIGNED(size, memory_block_size_bytes()) || !size)
        return -EINVAL;
