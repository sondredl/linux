                               ...
                               int nid, int flags)
{
    ...
    if (movable_node_is_enabled() && memblock_is_hotpluggable(m) && !(flags & MEMBLOCK_HOTPLUG))
        return true;
...
static void __init free_unused_memmap(void)
{
    ...
    if (!IS_ENABLED(CONFIG_HAVE_ARCH_PFN_VALID) || IS_ENABLED(CONFIG_SPARSEMEM_VMEMMAP))
        return;
