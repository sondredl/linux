                               ...
                               struct address_space *mapping)
{
    ...
    xas_for_each(&xas, folio, end_index)
    {
        ...
        addr = vma->vm_start + ((xas.xa_index - vma->vm_pgoff) << PAGE_SHIFT);
        xas_pause(&xas);
        rcu_read_unlock();
                                             ...
                                             struct mm_walk *walk)
{
    ...
    pageout_anon_only_filter = pageout && !vma_is_anonymous(vma) && !can_do_file_pageout(vma);
...
restart:
    ...
    for (; addr < end; pte += nr, addr += nr * PAGE_SIZE)
    {
         ...
         */
        if (!folio_test_lru(folio) || folio_mapcount(folio) != folio_nr_pages(folio))
            continue;
                            ...
                            unsigned long start_addr, unsigned long end_addr)
{
    ...
    *prev = vma;
     ...
     */
    if (!vma_is_anonymous(vma) && (!can_do_file_pageout(vma) && (vma->vm_flags & VM_MAYSHARE)))
        return 0;
