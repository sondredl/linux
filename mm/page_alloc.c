                       ...
                       int order, int migratetype)
{
     ...
     */
    if (order < pageblock_order && migratetype == MIGRATE_MOVABLE && capc->cc->migratetype != MIGRATE_MOVABLE)
        return false;
                                   ...
                                   int migratetype, fpi_t fpi_flags)
{
    ...
    while (order < MAX_PAGE_ORDER)
    {
        ...
        if (unlikely(order >= pageblock_order))
        {
            ...
            if (migratetype != buddy_mt && (!migratetype_is_mergeable(migratetype) || !migratetype_is_mergeable(buddy_mt)))
                goto done_merging;
        }
                            ...
                            gfp_t gfp_flags)
{
    ...
    if (!should_skip_kasan_unpoison(gfp_flags) && kasan_unpoison_pages(page, order, init))
    {
              ...
              unsigned int alloc_flags)
{
    ...
    if (IS_ENABLED(CONFIG_CMA))
    {
         ...
         * is in the CMA area.
         */
        if (alloc_flags & ALLOC_CMA && zone_page_state(zone, NR_FREE_CMA_PAGES) > zone_page_state(zone, NR_FREE_PAGES) / 2)
        {
                                   ...
                                   unsigned int order)
{
    ...
    {
        free_high = (pcp->free_count >= batch && (pcp->flags & PCPF_PREV_FREE_HIGH_ORDER) && (!(pcp->flags & PCPF_FREE_HIGH_BATCH) || pcp->count >= READ_ONCE(batch)));
    ...
    if (pcp->count >= high)
    {
        free_pcppages_bulk(zone, nr_pcp_free(pcp, batch, high, free_high),
                           pcp, pindex);
        if (test_bit(ZONE_BELOW_HIGH, &zone->flags) && zone_watermark_ok(zone, 0, high_wmark_pages(zone), ZONE_MOVABLE, 0))
...
static __always_inline struct page *rmqueue_buddy(struct zone *preferred_zone, struct zone *zone,
...
static int nr_pcp_alloc(struct per_cpu_pages *pcp, struct zone *zone, int order)
{
    ...
    if (!order)
    {
         ...
         */
        if (batch <= max_nr_alloc && pcp->alloc_factor < CONFIG_PCP_BATCH_SCALE_MAX)
            pcp->alloc_factor++;
...
static inline struct page *__rmqueue_pcplist(struct zone *zone, unsigned int order,
                                             int                   migratetype,
                                             unsigned int          alloc_flags,
                                             struct per_cpu_pages *pcp,
                                             struct list_head     *list)
 ...
 */
__no_sanitize_memory static inline struct page *rmqueue(struct zone *preferred_zone,
                                                        struct zone *zone, unsigned int order,
...
out:
    ...
    if ((alloc_flags & ALLOC_KSWAPD) && unlikely(test_bit(ZONE_BOOSTED_WATERMARK, &zone->flags)))
    {
...
#endif
        if ((alloc_flags & (ALLOC_HIGHATOMIC | ALLOC_OOM)) && !free_area_empty(area, MIGRATE_HIGHATOMIC))
        {
            return true;
        }
    }
...
retry:
    ...
    check_alloc_wmark:
                                 ...
                                 gfp_mask))
        {
            ...
            if (!node_reclaim_enabled() || !zone_allows_reclaim(ac->preferred_zoneref->zone, zone))
                continue;
...
restart:
     ...
     */
    if (can_direct_reclaim && can_compact && (costly_order || (order > 0 && ac->migratetype != MIGRATE_MOVABLE))
                                      ...
                                      struct page     **page_array)
{
    ...
    for_each_zone_zonelist_nodemask(zone, z, ac.zonelist, ac.highest_zoneidx, ac.nodemask)
    {
        ...
        if (cpusets_enabled() && (alloc_flags & ALLOC_CPUSET) && !__cpuset_zone_allowed(zone, gfp))
        {
            continue;
        }

        if (nr_online_nodes > 1 && zone != ac.preferred_zoneref->zone && zone_to_nid(zone) != zone_to_nid(ac.preferred_zoneref->zone))
        {
            goto failed;
        }
...
out:
    if (memcg_kmem_online() && (gfp & __GFP_ACCOUNT) && page && unlikely(__memcg_kmem_charge_page(page, gfp, order) != 0))
    {
                                             ...
                                             gfp_t                   gfp_mask)
{
    ...
    gfp_mask = (gfp_mask & ~__GFP_DIRECT_RECLAIM) | __GFP_COMP | __GFP_NOWARN | __GFP_NORETRY | __GFP_NOMEMALLOC;
...
static void zone_set_pageset_high_and_batch(struct zone *zone, int cpu_online)
{
    ...
    if (zone->pageset_high_min == new_high_min && zone->pageset_high_max == new_high_max && zone->pageset_batch == new_batch)
        return;
                                                        ...
                                                        int write, void *buffer, size_t *length, loff_t *ppos)
{
    ...
    if (percpu_pagelist_high_fraction && percpu_pagelist_high_fraction < MIN_PERCPU_PAGELIST_HIGH_FRACTION)
    {
    ...
    while (pfn < end || !list_empty(&cc->migratepages))
    {
        ...
        if (trace_mm_alloc_contig_migrate_range_info_enabled())
        {
            ...
            list_for_each_entry(page, &cc->migratepages, lru)
            {
                ...
                total_mapped += folio_mapped(folio) * folio_nr_pages(folio);
            }
        }
...
static bool try_to_accept_memory(struct zone *zone, unsigned int order)
{
    ...
    to_accept = high_wmark_pages(zone) - (zone_page_state(zone, NR_FREE_PAGES) - __zone_watermark_unusable_free(zone, order, 0));
