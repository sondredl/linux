                                   ...
                                   unsigned long pfn, int flags)
{
     ...
     */
    if (folio_test_reserved(folio) || folio_test_slab(folio) || folio_test_pgtable(folio) || folio_test_offline(folio))
        return true;
     ...
     */
    mapping = folio_mapping(folio);
    if (!(flags & MF_MUST_KILL) && !folio_test_dirty(folio) && mapping && mapping_can_writeback(mapping))
    {
                           ...
                           struct address_space *mapping, pgoff_t index, int flags)
{
    ...
    list_for_each_entry(tk, to_kill, nd) if (tk->size_shift)
...
retry:
    ...
    if (res == 2)
    { /* fallback to normal page handling */
        *hugetlb = 0;
        return 0;
    }
...
try_again:
     ...
     */
    if (folio_test_lru(folio) && !folio_test_swapcache(folio) && folio->mapping == NULL)
    {
...
int unpoison_memory(unsigned long pfn)
{
    ...
    if (folio_test_slab(folio) || folio_test_pgtable(folio) || folio_test_reserved(folio) || folio_test_offline(folio))
        goto unlock_mutex;
