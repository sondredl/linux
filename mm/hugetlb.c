...
{
#ifdef CONFIG_CGROUP_HUGETLB
    return rg->reservation_counter == org->reservation_counter && rg->css == org->css;
...
retry:
     ...
     * operation.
     */
    if (actual_regions_needed > in_regions_needed && resv->region_cache_count < resv->adds_in_progress + (actual_regions_needed - in_regions_needed))
    {
...
#define for_each_node_mask_to_alloc(next_node, nr_nodes, node, mask)                 \
    for (nr_nodes = nodes_weight(*mask);                                             \
         nr_nodes > 0 && ((node = hstate_next_node_to_alloc(next_node, mask)) || 1); \
         nr_nodes--)

#define for_each_node_mask_to_free(hs, nr_nodes, node, mask)                 \
    for (nr_nodes = nodes_weight(*mask);                                     \
         nr_nodes > 0 && ((node = hstate_next_node_to_free(hs, mask)) || 1); \
         nr_nodes--)
                                            ...
                                            struct folio  *folio)
{
     ...
     */
    if (hstate_is_gigantic(h) || hugetlb_cma_folio(folio, huge_page_order(h)))
    {
                              ...
                              nodemask_t *nodes_allowed)
{
    ...
    if (nid != NUMA_NO_NODE)
    {
        ...
        count += persistent_huge_pages(h) - (h->nr_huge_pages_node[nid] - h->surplus_huge_pages_node[nid]);
        /*
                            ...
                            struct page *ref_page, zap_flags_t zap_flags)
{
    ...
    for (; address < end; address += sz)
    {
        ...
        if (unlikely(!pte_present(pte)))
        {
             ...
             * pte with a marker.
             */
            if (pte_swp_uffd_wp_any(pte) && !(zap_flags & ZAP_FLAG_DROP_MARKER))
        ...
        if (huge_pte_uffd_wp(pte) && !(zap_flags & ZAP_FLAG_DROP_MARKER))
            set_huge_pte_at(mm, address, ptep,
                            make_pte_marker(PTE_MARKER_UFFD_WP),
                            sz);
         ...
         * reservation bit.
         */
        if (!h->surplus_huge_pages && __vma_private_lock(vma) && folio_test_anon(page_folio(page)))
        {
...
retry_avoidcopy:
    ...
    VM_BUG_ON_PAGE(folio_test_anon(old_folio) && PageAnonExclusive(&old_folio->page), &old_folio->page);
     ...
     */
    if (is_vma_resv_set(vma, HPAGE_RESV_OWNER) && old_folio != pagecache_folio)
        outside_reserve = 1;
                                  ...
                                  struct vm_fault      *vmf)
{
    ...
    new_pte = make_huge_pte(vma, &folio->page, ((vma->vm_flags & VM_WRITE) && (vma->vm_flags & VM_SHARED)));
    /*
                         ...
                         unsigned long address, unsigned int flags)
{
     ...
     */
    if ((flags & (FAULT_FLAG_WRITE | FAULT_FLAG_UNSHARE)) && !(vma->vm_flags & VM_MAYSHARE) && !huge_pte_write(vmf.orig_pte))
    {
        if (vma_needs_reservation(h, vma, vmf.address) < 0)
        {
            ret = VM_FAULT_OOM;
    ...
    if (userfaultfd_wp(vma) && huge_pte_uffd_wp(huge_ptep_get(vmf.pte)) && (flags & FAULT_FLAG_WRITE) && !huge_pte_write(vmf.orig_pte))
    {
                                          ...
                                          unsigned long addr, pgoff_t idx)
{
     ...
     */
    if (pmd_index(addr) != pmd_index(saddr) || vm_flags != svm_flags || !range_in_vma(svma, sbase, s_end) || !svma->vm_private_data)
        return 0;
                                          ...
                                          unsigned long *start, unsigned long *end)
{
     ...
     */
    if (!(vma->vm_flags & VM_MAYSHARE) || !(v_end > v_start) || (*end <= v_start) || (*start >= v_end))
        return;
...
bool isolate_hugetlb(struct folio *folio, struct list_head *list)
{
    ...
    spin_lock_irq(&hugetlb_lock);
    if (!folio_test_hugetlb(folio) || !folio_test_hugetlb_migratable(folio) || !folio_try_get(folio))
    {
        ret = false;
        goto unlock;
    }
