...
{
    if (folio_test_swapcache(folio) && !folio_mapped(folio) && folio_trylock(folio))
    {
                              ...
                              struct vm_fault *vmf)
{
    ...
    folio = swap_use_vma_readahead() ? swap_vma_readahead(entry, gfp_mask, mpol, ilx, vmf) : swap_cluster_readahead(entry, gfp_mask, mpol, ilx);
    mpol_cond_put(mpol);
