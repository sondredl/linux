                                 ...
                                 loff_t start_byte, loff_t end_byte)
{
    ...
    xas_for_each(&xas, folio, max)
    {
        ...
        if (xa_is_value(folio))
            continue;
        if (folio_test_dirty(folio) || folio_test_locked(folio) || folio_test_writeback(folio))
            break;
    }
    rcu_read_unlock();
    return folio != NULL;
                                        ...
                                        int state, enum behavior behavior)
{
    ...
    if (bit_nr == PG_locked && !folio_test_uptodate(folio) && folio_test_workingset(folio))
    {
                              ...
                              struct folio *folio, loff_t fpos, size_t size)
{
    ...
    while (spliced < size && !pipe_full(pipe->head, pipe->tail, pipe->max_usage))
    {
...
void kiocb_invalidate_post_direct_write(struct kiocb *iocb, size_t count)
{
    ...
    if (mapping->nrpages && invalidate_inode_pages2_range(mapping, iocb->ki_pos >> PAGE_SHIFT, (iocb->ki_pos + count - 1) >> PAGE_SHIFT))
