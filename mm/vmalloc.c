                       ...
                       phys_addr_t phys_addr, pgprot_t prot)
{
    ...
    if (addr != (unsigned long)area->addr || (void *)end != area->addr + get_vm_area_size(area))
    {
                                ...
                                unsigned long end)
{
    ...
    if ((end - start) >> PAGE_SHIFT > totalram_pages())
        return -E2BIG;
    if (start < (unsigned long)area->addr || (void *)end > area->addr + get_vm_area_size(area))
        return -ERANGE;
    return 0;
}
...
static void free_vmap_area_noflush(struct vmap_area *va)
{
     ...
     */
    vn = is_vn_id_valid(vn_id) ? id_to_node(vn_id) : addr_to_node(va->va_start);
...
static void _vm_unmap_aliases(unsigned long start, unsigned long end, int flush)
{
    ...
    for_each_possible_cpu(cpu)
    {
        ...
        xa_for_each(&vbq->vmap_blocks, idx, vb)
        {
             ...
             * space to be flushed.
             */
            if (!purge_fragmented_block(vb, &purge_list, false) && vb->dirty_max && vb->dirty != VMAP_BBMAP_BITS)
            {
...
again:
    ...
    if (!want_init_on_free() && want_init_on_alloc(gfp_mask) && (gfp_mask & __GFP_SKIP_ZERO))
