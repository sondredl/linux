...
static bool compaction_restarting(struct zone *zone, int order)
{
    ...
    return zone->compact_defer_shift == COMPACT_MAX_DEFER_SHIFT && zone->compact_considered >= 1UL << zone->compact_defer_shift;
}
                               ...
                               unsigned long end_pfn, isolate_mode_t mode)
{
    ...
    isolate_success_no_list:
         ...
         */
        if (cc->nr_migratepages >= COMPACT_CLUSTER_MAX && !cc->finish_pageblock && !cc->contended)
        {
            ++low_pfn;
            break;
        }
 ...
 */
int isolate_migratepages_range(struct compact_control *cc, unsigned long start_pfn,
                               unsigned long end_pfn)
{
...
static isolate_migrate_t isolate_migratepages(struct compact_control *cc)
{
    ...
    const isolate_mode_t isolate_mode =
        (sysctl_compact_unevictable_allowed ? ISOLATE_UNEVICTABLE : 0) | (cc->mode != MIGRATE_SYNC ? ISOLATE_ASYNC_MIGRATE : 0);
         ...
         block_end_pfn += pageblock_nr_pages)
    {
         ...
         */
        if ((pageblock_aligned(low_pfn) || low_pfn == cc->zone->zone_start_pfn) && !fast_find_block && !isolation_suitable(cc, page))
            continue;
                                  ...
                                  unsigned long wmark_target)
{
     ...
     */
    watermark = (order > PAGE_ALLOC_COSTLY_ORDER) ? low_wmark_pages(zone) : min_wmark_pages(zone);
    ...
    compact_zone(struct compact_control *cc, struct capture_control *capc)
{
    ...
    rescan:
        ...
        if (err)
        {
             ...
             */
            if (!pageblock_aligned(cc->migrate_pfn) && !cc->ignore_skip_hint && !cc->finish_pageblock && (cc->mode < MIGRATE_SYNC))
            {
                cc->finish_pageblock = true;
