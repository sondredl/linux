...
{
    return object->min_count != KMEMLEAK_BLACK && object->count >= object->min_count;
}
                               ...
                               unsigned int objflags)
{
    ...
    if ((ptr > start) && !__link_object(object_l, start, ptr - start, object->min_count, objflags))
        object_l = NULL;
    if ((ptr + size < end) && !__link_object(object_r, ptr + size, end - ptr - size, object->min_count, objflags))
        object_r = NULL;
...
static void paint_ptr(unsigned long ptr, int color, unsigned int objflags)
{
    ...
    if (!object)
    {
        kmemleak_warn("Trying to color unknown object at 0x%08lx as %s\n",
                      ptr,
                      (color == KMEMLEAK_GREY) ? "Grey" : (color == KMEMLEAK_BLACK) ? "Black"
                                                                                    : "Unknown");
        return;
    }
...
static void scan_object(struct kmemleak_object *object)
{
    ...
    else if (hlist_empty(&object->area_list) || object->flags & OBJECT_FULL_SCAN)
    {
        void *start = object->flags & OBJECT_PHYS ? __va((phys_addr_t)object->pointer) : (void *)object->pointer;
        void *end   = start + object->size;
        void *next;
...
static void kmemleak_scan(void)
{
    ...
    list_for_each_entry_rcu(object, &object_list, object_list)
    {
        ...
        if ((object->flags & OBJECT_PHYS) && !(object->flags & OBJECT_NO_SCAN))
        {
            unsigned long phys = object->pointer;

            if (PHYS_PFN(phys) < min_low_pfn || PHYS_PFN(phys + object->size) >= max_low_pfn)
