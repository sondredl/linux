...
static struct page_ext *lookup_page_ext(const struct page *page)
{
    ...
    if (unlikely(!base))
        return NULL;
    index = pfn - round_down(node_start_pfn(page_to_nid(page)), MAX_ORDER_NR_PAGES);
...
static int __init alloc_node_page_ext(int nid)
{
     ...
     */
    if (!IS_ALIGNED(node_start_pfn(nid), MAX_ORDER_NR_PAGES) || !IS_ALIGNED(node_end_pfn(nid), MAX_ORDER_NR_PAGES))
