                                             ...
                                             unsigned long npages)
{
    ...
    for (; npages; npages--, pages++)
    {
        ...
        if (!folio_test_large(folio) || folio_test_hugetlb(folio))
            ...
            VM_BUG_ON_PAGE(!PageAnonExclusive(&folio->page) && !PageAnonExclusive(page), page);
    }
}
                                    ...
                                    int flags, struct follow_page_context *ctx)
{
    ...
    if (IS_ENABLED(CONFIG_HAVE_ARCH_TRANSPARENT_HUGEPAGE_PUD) && pud_devmap(pud))
    {
        /*
    ...
    if (!pud_devmap(pud) && !pud_write(pud) && gup_must_unshare(vma, flags, page))
                                    ...
                                    struct follow_page_context *ctx)
{
    ...
    if ((flags & FOLL_WRITE) && !can_follow_write_pmd(pmdval, page, vma, flags))
        return NULL;
    ...
    VM_BUG_ON_PAGE((flags & FOLL_PIN) && PageAnon(page) && !PageAnonExclusive(page), page);
                        ...
                        unsigned long end, bool write, int *locked)
{
     ...
     */
    gup_flags = FOLL_TOUCH | FOLL_HWPOISON | FOLL_UNLOCKABLE | FOLL_MADV_POPULATE;
    if (write)
        gup_flags |= FOLL_WRITE;
                              ...
                              unsigned int *gup_flags_p, unsigned int to_set)
{
    ...
    /* FOLL_GET and FOLL_PIN are mutually exclusive. */
    if (WARN_ON_ONCE((gup_flags & (FOLL_PIN | FOLL_GET)) == (FOLL_PIN | FOLL_GET)))
        return false;
    ...
    if (WARN_ON_ONCE((gup_flags & FOLL_LONGTERM) && (gup_flags & FOLL_PCI_P2PDMA)))
        return false;
...
static bool gup_fast_folio_allowed(struct folio *folio, unsigned int flags)
{
     ...
     */
    if ((flags & (FOLL_PIN | FOLL_LONGTERM | FOLL_WRITE)) == (FOLL_PIN | FOLL_LONGTERM | FOLL_WRITE))
                              ...
                              unsigned int gup_flags, struct page **pages)
{
    ...
    if (!IS_ENABLED(CONFIG_HAVE_GUP_FAST) || !gup_fast_permitted(start, end))
        return 0;
                             ...
                             unsigned int gup_flags, struct page **pages)
{
    ...
    if (WARN_ON_ONCE(gup_flags & ~(FOLL_WRITE | FOLL_LONGTERM | FOLL_FORCE | FOLL_PIN | FOLL_GET | FOLL_FAST_ONLY | FOLL_NOFAULT | FOLL_PCI_P2PDMA | FOLL_HONOR_NUMA_FAULT)))
        return -EINVAL;
