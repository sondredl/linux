...
{
    return (kmem_cache_debug_flags(s, SLAB_STORE_USER) && (s->flags & SLAB_KMALLOC));
}
 ...
 */
__no_kmsan_checks static inline void *get_freepointer_safe(struct kmem_cache *s, void *object)
{
                                      ...
                                      struct slab *slab, void *object)
{
    ...
    if (object < base || object >= base + slab->objects * s->size || (object - base) % s->size)
    {
        return 0;
    }
...
{
    if ((s->flags & SLAB_CONSISTENCY_CHECKS) && !check_valid_pointer(s, slab, nextfree) && freelist)
    {
                        ...
                        void *object, u8 val)
{
    ...
    if (s->flags & SLAB_RED_ZONE)
    {
        ...
        if (slub_debug_orig_size(s) && val == SLUB_RED_ACTIVE)
        {
            ...
            if (s->object_size > orig_size && !check_bytes_and_report(s, slab, object, "kmalloc Redzone", p + orig_size, val, s->object_size - orig_size))
            {
                return 0;
            }
        }
    ...
    if (s->flags & SLAB_POISON)
    {
        if (val != SLUB_RED_ACTIVE && (s->flags & __OBJECT_POISON))
        {
             ...
             */
            kasan_meta_size = kasan_metadata_size(s, true);
            if (kasan_meta_size < s->object_size - 1 && !check_bytes_and_report(s, slab, p, "Poison", p + kasan_meta_size, POISON_FREE, s->object_size - kasan_meta_size - 1))
                return 0;
            if (kasan_meta_size < s->object_size && !check_bytes_and_report(s, slab, p, "End Poison", p + s->object_size - 1, POISON_END, 1))
                return 0;
        }
        /*
...
static void setup_slab_debug(struct kmem_cache *s, struct slab *slab, void *addr)
{
...
static inline void setup_slab_debug(struct kmem_cache *s, struct slab *slab, void *addr)
{
}
                                               ...
                                               struct slabobj_ext *vec, unsigned int objects)
{
     ...
     */
    #define OBJCGS_CLEAR_MASK (__GFP_DMA | __GFP_RECLAIMABLE | __GFP_ACCOUNT | __GFP_NOFAIL)
    ...
    prepare_slab_obj_exts_hook(struct kmem_cache *s, gfp_t flags, void *p)
{
    ...
    slab = virt_to_slab(p);
    if (!slab_obj_exts(slab) && WARN(alloc_slab_obj_exts(slab, s, flags, false), "%s, %s: Failed to create slab extension vector!\n", __func__, s->name))
        return NULL;
...
static __fastpath_inline bool memcg_slab_post_alloc_hook(struct kmem_cache *s, struct list_lru *lru,
...
static __fastpath_inline void memcg_slab_free_hook(struct kmem_cache *s, struct slab *slab, void **p,
                                                   int objects)
{
    struct slabobj_ext *obj_exts;
 ...
 */
static __always_inline bool slab_free_hook(struct kmem_cache *s, void *x, bool init)
{
...
static __fastpath_inline bool slab_free_freelist_hook(struct kmem_cache *s, void **head, void **tail,
                                                      int *cnt)
{
                                    ...
                                    struct partial_context *pc)
{
    ...
    do
    {
        ...
        for_each_zone_zonelist(zone, z, zonelist, highest_zoneidx)
        {
            ...
            if (n && cpuset_zone_allowed(zone, pc->flags) && n->nr_partial > s->min_partial)
            {
...
new_slab:
    ...
    while (slub_percpu_partial(c))
    {
        ...
        if (likely(node_match(slab, node) && pfmemalloc_match(slab, gfpflags)))
        {
            c->slab  = slab;
            freelist = get_freelist(s, slab);
            VM_BUG_ON(!freelist);
...
static __fastpath_inline struct kmem_cache *slab_pre_alloc_hook(struct kmem_cache *s, gfp_t flags)
{
    flags &= gfp_allowed_mask;
...
static __fastpath_inline bool slab_post_alloc_hook(struct kmem_cache *s, struct list_lru *lru,
...
static __always_inline void *__do_kmalloc_node(size_t size, gfp_t flags, int node,
...
static __fastpath_inline void slab_free(struct kmem_cache *s, struct slab *slab, void *object,
                                        unsigned long addr)
{
...
static noinline void memcg_alloc_abort_single(struct kmem_cache *s, void *object)
{
    if (likely(slab_free_hook(s, object, slab_want_init_on_free(s))))
...
static __fastpath_inline void slab_free_bulk(struct kmem_cache *s, struct slab *slab, void *head,
...
static inline struct kmem_cache *cache_from_obj(struct kmem_cache *s, void *x)
{
    ...
    if (!IS_ENABLED(CONFIG_SLAB_FREELIST_HARDENED) && !kmem_cache_debug_flags(s, SLAB_CONSISTENCY_CHECKS))
        return s;
...
#ifndef CONFIG_SLUB_TINY
static inline int __kmem_cache_alloc_bulk(struct kmem_cache *s, gfp_t flags, size_t size,
                                          void **p)
{
    struct kmem_cache_cpu *c;
...
{
    BUILD_BUG_ON(PERCPU_DYNAMIC_EARLY_SIZE < NR_KMALLOC_TYPES * KMALLOC_SHIFT_HIGH * sizeof(struct kmem_cache_cpu));
...
static int calculate_sizes(struct kmem_cache *s)
{
    ...
    if (slub_debug_orig_size(s) || (flags & (SLAB_TYPESAFE_BY_RCU | SLAB_POISON)) || ((flags & SLAB_RED_ZONE) && s->object_size < sizeof(void *)) || s->ctor)
    {
        /*
...
bool __kmem_cache_empty(struct kmem_cache *s)
{
    ...
    for_each_kmem_cache_node(s, node, n) if (n->nr_partial || node_nr_slabs(n)) return false;
    return true;
}
                          ...
                          unsigned long *obj_map)
{
    ...
    for_each_object(p, s, addr, slab->objects)
    {
        u8 val = test_bit(__obj_to_index(s, addr, p), obj_map) ? SLUB_RED_INACTIVE : SLUB_RED_ACTIVE;
                        ...
                        unsigned int        orig_size)
{
    ...
    for (;;)
    {
        ...
        chandle = l->handle;
        cwaste  = l->waste;
        if ((track->addr == caddr) && (handle == chandle) && (waste == cwaste))
        {
        ...
        else if (track->addr == caddr && handle == chandle && waste < cwaste)
            end = pos;
        else
            start = pos;
    }
                         ...
                         unsigned long *obj_map)
{
    ...
    for_each_object(p, s, addr, slab->objects) if (!test_bit(__obj_to_index(s, addr, p), obj_map))
...
static int __init slab_debugfs_init(void)
{
    ...
    list_for_each_entry(s, &slab_caches, list) if (s->flags & SLAB_STORE_USER)
        debugfs_slab_add(s);
