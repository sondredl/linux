                           ...
                           void *old_entry, void *new_entry, bool need_rmap_locks)
{
    ...
    switch (entry)
    {
        ...
        case NORMAL_PUD:
            ...
            break;
        case HPAGE_PMD:
            moved = IS_ENABLED(CONFIG_TRANSPARENT_HUGEPAGE) && move_huge_pmd(vma, old_addr, new_addr, old_entry, new_entry);
            break;
        case HPAGE_PUD:
            moved = IS_ENABLED(CONFIG_TRANSPARENT_HUGEPAGE) && move_huge_pud(vma, old_addr, new_addr, old_entry, new_entry);
            break;
                             ...
                             unsigned long mask, bool for_stack)
{
    ...
    if (!can_align_down(old_vma, *old_addr, mask, for_stack) || !can_align_down(new_vma, *new_addr, mask, for_stack))
        return;
                               ...
                               bool need_rmap_locks, bool for_stack)
{
    ...
    again:
        ...
        {
            if (extent == HPAGE_PMD_SIZE && move_pgt_entry(HPAGE_PMD, vma, old_addr, new_addr, old_pmd, new_pmd, need_rmap_locks))
                continue;
                              ...
                              struct vm_userfaultfd_ctx *uf, struct list_head *uf_unmap)
{
    ...
    if (unlikely(!err && (flags & MREMAP_DONTUNMAP)))
    {
         ...
         */
        if (new_vma != vma && vma->vm_start == old_addr && vma->vm_end == (old_addr + old_len))
                               ...
                               struct list_head *uf_unmap)
{
    ...
    if (flags & MREMAP_DONTUNMAP && !may_expand_vm(mm, vma->vm_flags, old_len >> PAGE_SHIFT))
    {
        ret = -ENOMEM;
        goto out;
    }
    ...
    ret = get_unmapped_area(vma->vm_file, new_addr, new_len, vma->vm_pgoff + ((addr - vma->vm_start) >> PAGE_SHIFT),
                            map_flags);
    if (IS_ERR_VALUE(ret))
        goto out;
                ...
                unsigned long, new_addr)
{
     ...
     * in the process.
     */
    if (flags & MREMAP_DONTUNMAP && (!(flags & MREMAP_MAYMOVE) || old_len != new_len))
        return ret;
