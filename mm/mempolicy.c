                                  ...
                                  nodemask_t *nodes)
{
    ...
    {
        if (!nodes_empty(*nodes) || (flags & MPOL_F_STATIC_NODES) || (flags & MPOL_F_RELATIVE_NODES))
            return ERR_PTR(-EINVAL);
    }
...
static bool strictly_unmovable(unsigned long flags)
{
     ...
     */
    return (flags & (MPOL_MF_STRICT | MPOL_MF_MOVE | MPOL_MF_MOVE_ALL)) == MPOL_MF_STRICT;
}
...
static void queue_folios_pmd(pmd_t *pmd, struct mm_walk *walk)
{
    ...
    if (!queue_folio_required(folio, qp))
        return;
    if (!(qp->flags & (MPOL_MF_MOVE | MPOL_MF_MOVE_ALL)) || !vma_migratable(walk->vma) || !migrate_folio_add(folio, qp->pagelist, qp->flags))
        qp->nr_failed++;
}
                                  ...
                                  unsigned long end, struct mm_walk *walk)
{
    ...
    for (; addr != end; pte++, addr += PAGE_SIZE)
    {
        ...
        if (!(flags & (MPOL_MF_MOVE | MPOL_MF_MOVE_ALL)) || !vma_migratable(vma) || !migrate_folio_add(folio, qp->pagelist, flags))
        {
            qp->nr_failed++;
                                ...
                                struct mm_walk *walk)
{
    ...
    if (!queue_folio_required(folio, qp))
        goto unlock;
    if (!(flags & (MPOL_MF_MOVE | MPOL_MF_MOVE_ALL)) || !vma_migratable(walk->vma))
    {
        qp->nr_failed++;
        goto unlock;
    }
     ...
     */
    if ((flags & MPOL_MF_MOVE_ALL) || (!folio_likely_mapped_shared(folio) && !hugetlb_pmd_shared(pte)))
                                 ...
                                 struct mm_walk *walk)
{
    ...
    {
        qp->first = vma;
        if (!(flags & MPOL_MF_DISCONTIG_OK) && (qp->start < vma->vm_start))
            /* hole at head side of range */
            return -EFAULT;
    }
    next = find_vma(vma->vm_mm, vma->vm_end);
    if (!(flags & MPOL_MF_DISCONTIG_OK) && ((vma->vm_end < qp->end) && (!next || vma->vm_end < next->vm_start)))
                      ...
                      struct list_head *pagelist)
{
    ...
    const struct mm_walk_ops *ops = (flags & MPOL_MF_WRLOCK) ? &queue_pages_lock_vma_walk_ops : &queue_pages_walk_ops;
                             ...
                             nodemask_t *nodes)
{
    ...
    if (new && (new->mode == MPOL_INTERLEAVE || new->mode == MPOL_WEIGHTED_INTERLEAVE))
    {
                             ...
                             unsigned long addr, unsigned long flags)
{
    ...
    if (flags & MPOL_F_NODE)
    {
        ...
        else if (pol == current->mempolicy && pol->mode == MPOL_WEIGHTED_INTERLEAVE)
        {
                     ...
                     nodemask_t *nmask, unsigned long flags)
{
    ...
    if (!err && !list_empty(&pagelist))
    {
         ...
         */
        if (new->mode == MPOL_INTERLEAVE || new->mode == MPOL_WEIGHTED_INTERLEAVE)
        {
            struct folio *folio;
            unsigned int  order;
...
bool vma_migratable(struct vm_area_struct *vma)
{
    ...
    if (is_vm_hugetlb_page(vma) && !hugepage_migration_supported(hstate_vma(vma)))
        return false;
     ...
     * possible.
     */
    if (vma->vm_file && gfp_zone(mapping_gfp_mask(vma->vm_file->f_mapping)) < policy_zone)
        return false;
    return true;
}
...
{
    *ilx = 0;
    return (vma->vm_ops && vma->vm_ops->get_policy) ? vma->vm_ops->get_policy(vma, addr, ilx) : vma->vm_policy;
}
                                 ...
                                 unsigned long addr, int order, pgoff_t *ilx)
{
    ...
    if (pol->mode == MPOL_INTERLEAVE || pol->mode == MPOL_WEIGHTED_INTERLEAVE)
    {
