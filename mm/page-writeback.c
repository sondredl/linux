...
unsigned long cgwb_calc_thresh(struct bdi_writeback *wb)
{
    ...
    gdtc.avail = global_dirtyable_memory();
    gdtc.dirty = global_node_page_state(NR_FILE_DIRTY) + global_node_page_state(NR_WRITEBACK);
                               ...
                               unsigned long pages_dirtied, unsigned int flags)
{
    ...
    for (;;)
    {
         ...
         */
        if (!laptop_mode && nr_dirty > gdtc->bg_thresh && !writeback_in_progress(wb))
        ...
        if (!strictlimit)
        {
            ...
            if ((current->flags & PF_LOCAL_THROTTLE) && gdtc->wb_dirty < dirty_freerun_ceiling(gdtc->wb_thresh, gdtc->wb_bg_thresh))
                /*
        ...
        if (mdtc)
        {
            ...
            if (!strictlimit)
            {
                ...
                if ((current->flags & PF_LOCAL_THROTTLE) && mdtc->wb_dirty < dirty_freerun_ceiling(mdtc->wb_thresh, mdtc->wb_bg_thresh))
                    /*
