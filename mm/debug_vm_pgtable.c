...
static void __init pmd_huge_tests(struct pgtable_debug_args *args)
{
    ...
    if (!arch_vmap_pmd_supported(args->page_prot) || args->fixed_alignment < PMD_SIZE)
        return;
...
static void __init pud_huge_tests(struct pgtable_debug_args *args)
{
    ...
    if (!arch_vmap_pud_supported(args->page_prot) || args->fixed_alignment < PUD_SIZE)
        return;
...
static void __init pmd_swap_soft_dirty_tests(struct pgtable_debug_args *args)
{
    ...
    if (!IS_ENABLED(CONFIG_MEM_SOFT_DIRTY) || !IS_ENABLED(CONFIG_ARCH_ENABLE_THP_MIGRATION))
        return;
...
static void __init destroy_args(struct pgtable_debug_args *args)
{
    ...
    /* Free (huge) page */
    if (IS_ENABLED(CONFIG_TRANSPARENT_HUGEPAGE) && has_transparent_pud_hugepage() && args->pud_pfn != ULONG_MAX)
    {
    ...
    if (IS_ENABLED(CONFIG_TRANSPARENT_HUGEPAGE) && has_transparent_hugepage() && args->pmd_pfn != ULONG_MAX)
    {
        if (args->is_contiguous_page)
        {
...
static int __init init_args(struct pgtable_debug_args *args)
{
     ...
     */
    if (IS_ENABLED(CONFIG_TRANSPARENT_HUGEPAGE) && has_transparent_pud_hugepage())
    {
    ...
    if (IS_ENABLED(CONFIG_TRANSPARENT_HUGEPAGE) && has_transparent_hugepage())
    {
