...
static void mn_itree_inv_end(struct mmu_notifier_subscriptions *subscriptions)
{
    ...
    spin_lock(&subscriptions->lock);
    if (--subscriptions->active_invalidate_ranges || !mn_itree_is_invalidating(subscriptions))
    {
        spin_unlock(&subscriptions->lock);
        return;
    }
                            ...
                            struct mm_struct    *mm)
{
     ...
     */
    if (WARN_ON_ONCE(subscription && (subscription->ops->arch_invalidate_secondary_tlbs && (subscription->ops->invalidate_range_start || subscription->ops->invalidate_range_end))))
        return -EINVAL;
    ...
    unsigned long length, const struct mmu_interval_notifier_ops *ops)
{
     ...
     */
    if (length == 0 || check_add_overflow(start, length - 1, &interval_sub->interval_tree.last))
        return -EOVERFLOW;
