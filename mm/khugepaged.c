...
{
    return hpage_collapse_test_exit(mm) || test_bit(MMF_DISABLE_THP, &mm->flags);
}
...
{
    if (!test_bit(MMF_VM_HUGEPAGE, &vma->vm_mm->flags) && hugepage_flags_enabled())
    {
                                        ...
                                        struct list_head        *compound_pagelist)
{
         ...
         _pte++, address += PAGE_SIZE)
    {
        ...
        {
            ++none_or_zero;
            if (!userfaultfd_armed(vma) && (!cc->is_khugepaged || none_or_zero <= khugepaged_max_ptes_none))
            {
                continue;
            }
            else
    ...
    next:
         ...
         */
        if (cc->is_khugepaged && (pte_young(pteval) || folio_test_young(folio) || folio_test_referenced(folio) || mmu_notifier_test_young(vma->vm_mm, address)))
            referenced++;
                                                ...
                                                struct list_head      *compound_pagelist)
{
    ...
    {
        list_del(&src->lru);
        node_stat_sub_folio(src, NR_ISOLATED_ANON + folio_is_file_lru(src));
        folio_unlock(src);
        free_swap_cache(src);
...
#define khugepaged_defrag() \
    (transparent_hugepage_flags & (1 << TRANSPARENT_HUGEPAGE_DEFRAG_KHUGEPAGED_FLAG))
...
static int alloc_charge_folio(struct folio **foliop, struct mm_struct *mm,
                              struct collapse_control *cc)
{
    gfp_t         gfp  = (cc->is_khugepaged ? alloc_hugepage_khugepaged_gfpmask() : GFP_TRANSHUGE);
                                   ...
                                   struct collapse_control *cc)
{
         ...
         _pte++, _address += PAGE_SIZE)
    {
        ...
        {
            ++unmapped;
            if (!cc->is_khugepaged || unmapped <= khugepaged_max_ptes_swap)
            {
                /*
    ...
    else if (cc->is_khugepaged && (!referenced || (unmapped && referenced < HPAGE_PMD_NR / 2)))
    {
        result = SCAN_LACK_REFERENCED_PAGE;
    }
    else
                            ...
                            bool install_pmd)
{
    ...
    if (!vma || !vma->vm_file || !range_in_vma(vma, haddr, haddr + HPAGE_PMD_SIZE))
        return SCAN_VMA_CHECK;
                                    ...
                                    struct collapse_control *cc)
{
    ...
    xas_for_each(&xas, folio, start + HPAGE_PMD_NR - 1)
    {
        ...
        if (folio_ref_count(folio) != 1 + folio_mapcount(folio) + folio_test_private(folio))
        {
            result = SCAN_PAGE_COUNT;
            break;
        }
    ...
    *result = SCAN_FAIL;
    ...
    for_each_vma(vmi, vma)
    {
        ...
        while (khugepaged_scan.address < hend)
        {
            ...
            VM_BUG_ON(khugepaged_scan.address < hstart || khugepaged_scan.address + HPAGE_PMD_SIZE > hend);
...
{
    return !list_empty(&khugepaged_scan.mm_head) && hugepage_flags_enabled();
}
