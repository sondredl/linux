...
again:
    ...
    for (; addr < end; addr += PAGE_SIZE, ptep++)
    {
        ...
        {
            pfn = pte_pfn(pte);
            if (is_zero_pfn(pfn) && (migrate->flags & MIGRATE_VMA_SELECT_SYSTEM))
            {
                mpfn = MIGRATE_PFN_MIGRATE;
                migrate->cpages++;
                goto next;
            ...
            if (page && !is_zone_device_page(page) && !(migrate->flags & MIGRATE_VMA_SELECT_SYSTEM))
                goto next;
            else if (page && is_device_coherent_page(page) && (!(migrate->flags & MIGRATE_VMA_SELECT_DEVICE_COHERENT) || page->pgmap->owner != migrate->pgmap_owner))
                goto next;
                                          ...
                                          struct page   *fault_page)
{
    ...
    for (i = 0; i < npages; i++)
    {
        ...
        if (page_mapped(page) || !migrate_vma_check_page(page, fault_page))
        {
            if (!is_zone_device_page(page))
            {
                get_page(page);
                                    ...
                                    unsigned long      *src)
{
    ...
    {
        if (folio_is_zone_device(folio) && !folio_is_device_coherent(folio))
        {
                                   ...
                                   struct migrate_vma *migrate)
{
    ...
    for (i = 0; i < npages; i++)
    {
        ...
        if (is_device_private_page(newpage) || is_device_coherent_page(newpage))
        {
            if (mapping)
            {
                /*
