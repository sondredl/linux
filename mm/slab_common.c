 ...
 */
#define SLAB_NEVER_MERGE (SLAB_RED_ZONE | SLAB_POISON | SLAB_STORE_USER | SLAB_TRACE | SLAB_TYPESAFE_BY_RCU | SLAB_NOLEAKTRACE | SLAB_FAILSLAB | SLAB_NO_MERGE)
                               ...
                               void (*ctor)(void *))
{
    ...
    if (!IS_ENABLED(CONFIG_HARDENED_USERCOPY) || WARN_ON(!usersize && useroffset) || WARN_ON(size < usersize || size - usersize < useroffset))
...
static unsigned int __kmalloc_minalign(void)
{
    ...
    if (IS_ENABLED(CONFIG_DMA_BOUNCE_UNALIGNED_KMALLOC) && is_swiotlb_allocated())
...
static __always_inline __realloc_size(2) void *__do_krealloc(const void *p, size_t new_size, gfp_t flags)
{
    void  *ret;
    size_t ks;
