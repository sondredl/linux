...
static void zero_zone_numa_counters(struct zone *zone)
{
    ...
    for (item = 0; item < NR_VM_NUMA_EVENT_ITEMS; item++)
    {
        ...
        {
            per_cpu_ptr(zone->per_cpu_zonestats, cpu)->vm_numa_event[item] = 0;
        }
    }
}
...
static int refresh_cpu_vm_stats(bool do_pagesets)
{
    ...
    for_each_populated_zone(zone)
    {
        ...
        if (do_pagesets)
        {
             ...
             */
            if (!__this_cpu_read(pcp->expire) || !__this_cpu_read(pcp->count))
                continue;
...
unsigned int extfrag_for_order(struct zone *zone, unsigned int order)
{
    ...
    return div_u64((info.free_pages - (info.free_blocks_suitable << order)) * 100,
                   info.free_pages);
}
...
#if (defined(CONFIG_DEBUG_FS) && defined(CONFIG_COMPACTION)) || defined(CONFIG_PROC_FS)
...
static const struct seq_operations zoneinfo_op = {
    ...
    #define NR_VMSTAT_ITEMS (NR_VM_ZONE_STAT_ITEMS + NR_VM_NUMA_EVENT_ITEMS + NR_VM_NODE_STAT_ITEMS + NR_VM_WRITEBACK_STAT_ITEMS + (IS_ENABLED(CONFIG_VM_EVENT_COUNTERS) ? NR_VM_EVENT_ITEMS : 0))
