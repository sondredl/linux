...
{
    vm_fault_t                ret = 0;
    const struct mm_walk_ops *ops = lock_vma ? &break_ksm_lock_vma_ops : &break_ksm_ops;
...
{
    if (vma->vm_flags & (VM_SHARED | VM_MAYSHARE | VM_PFNMAP | VM_IO | VM_DONTEXPAND | VM_HUGETLB | VM_MIXEDMAP))
                              ...
                              pte_t *orig_pte)
{
    ...
    if (pte_write(entry) || pte_dirty(entry) || anon_exclusive || mm_tlb_flush_pending(mm))
    {
        ...
        if (anon_exclusive && folio_try_share_anon_rmap_pte(folio, &folio->page))
        {
            set_pte_at(mm, pvmw.address, pvmw.pte, entry);
            goto out_unlock;
        }
...
static __always_inline bool is_page_sharing_candidate(struct ksm_stable_node *stable_node)
{
                                     ...
                                     struct vm_area_struct *vma, unsigned long addr)
{
    ...
    else if (folio->index == linear_page_index(vma, addr) && anon_vma->root == vma->anon_vma->root)
    {
...
{
    return (long)(mm->ksm_merging_pages + mm_ksm_zero_pages(mm)) * PAGE_SIZE - mm->ksm_rmap_items * sizeof(struct ksm_rmap_item);
}
                                   ...
                                   struct kobj_attribute *attr, char *buf)
{
    ...
    general_profit = (ksm_pages_sharing + atomic_long_read(&ksm_zero_pages)) * PAGE_SIZE - ksm_rmap_items * sizeof(struct ksm_rmap_item);
