...
int anon_vma_clone(struct vm_area_struct *dst, struct vm_area_struct *src)
{
    ...
    list_for_each_entry_reverse(pavc, &src->anon_vma_chain, same_vma)
    {
         ...
         */
        if (!dst->anon_vma && src->anon_vma && anon_vma->num_children < 2 && anon_vma->num_active_vmas == 0)
...
retry:
    ...
    if (down_read_trylock(&root_anon_vma->rwsem))
    {
         ...
         */
        if (unlikely((unsigned long)READ_ONCE(folio->mapping) != anon_mapping))
        {
                                 ...
                                 struct vm_area_struct *vma, unsigned long address, void *arg)
{
    ...
    if ((vma->vm_flags & VM_LOCKED) && folio_test_large(folio) && folio_within_vma(folio, vma))
    {
                                                  ...
                                                  unsigned long address, rmap_t flags, enum rmap_level level)
{
    ...
    if (unlikely(!folio_test_anon(folio)))
    {
         ...
         * page.
         */
        VM_WARN_ON_FOLIO(folio_test_large(folio) && level != RMAP_LEVEL_PMD, folio);
    ...
    for (i = 0; i < nr_pages; i++)
    {
        ...
        VM_WARN_ON_FOLIO((atomic_read(&cur_page->_mapcount) > 0 || (folio_test_large(folio) && folio_entire_mapcount(folio) > 1)) && PageAnonExclusive(cur_page), folio);
    }
                                                  ...
                                                  enum rmap_level level)
{
    ...
    nr = __folio_add_rmap(folio, page, nr_pages, level, &nr_pmdmapped);
    if (nr_pmdmapped)
        __mod_node_page_state(pgdat, folio_test_swapbacked(folio) ? NR_SHMEM_PMDMAPPED : NR_FILE_PMDMAPPED, nr_pmdmapped);
    if (nr)
    ...
    if (nr_pmdmapped)
    {
        ...
        if (folio_test_anon(folio))
            ...
            __mod_node_page_state(pgdat,
                                  folio_test_swapbacked(folio) ? NR_SHMEM_PMDMAPPED : NR_FILE_PMDMAPPED,
                                  -nr_pmdmapped);
    }
    if (nr)
    {
         ...
         */
        if (folio_test_anon(folio) && partially_mapped && list_empty(&folio->_deferred_list))
                               ...
                               unsigned long address, void *arg)
{
    ...
    while (page_vma_mapped_walk(&pvmw))
    {
        ...
        else
        {
            ...
            else if (anon_exclusive && folio_try_share_anon_rmap_pte(folio, subpage))
            {
                set_pte_at(mm, address, pvmw.pte, pteval);
                ret = false;
                page_vma_mapped_walk_done(&pvmw);
                                           ...
                                           struct vm_area_struct *vma, unsigned long address, void *priv)
{
    ...
    mmu_notifier_range_init_owner(&range, MMU_NOTIFY_EXCLUSIVE, 0,
                                  vma->vm_mm, address, min(vma->vm_end, address + folio_size(folio)),
                                  args->owner);
    mmu_notifier_invalidate_range_start(&range);
                           ...
                           unsigned long address, rmap_t flags)
{
    ...
    if (flags & RMAP_EXCLUSIVE)
        SetPageAnonExclusive(&folio->page);
    VM_WARN_ON_FOLIO(folio_entire_mapcount(folio) > 1 && PageAnonExclusive(&folio->page), folio);
}
