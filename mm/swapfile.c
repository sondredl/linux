                                 ...
                                 unsigned long offset, unsigned long flags)
{
    ...
    {
        if ((flags & TTRS_ANYWAY) || ((flags & TTRS_UNMAPPED) && !folio_mapped(folio)) || ((flags & TTRS_FULL) && mem_cgroup_swap_full(folio)))
...
static void free_cluster(struct swap_info_struct *si, unsigned long idx)
{
     ...
     * after discard.
     */
    if ((si->flags & (SWP_WRITEOK | SWP_PAGE_DISCARD)) == (SWP_WRITEOK | SWP_PAGE_DISCARD))
    {
                                       ...
                                       unsigned long offset, int order)
{
    ...
    conflict = !cluster_list_empty(&si->free_clusters) && offset != cluster_list_first(&si->free_clusters) && cluster_is_free(&si->cluster_info[offset]);
...
new_cluster:
    ...
    if (tmp == SWAP_NEXT_INVALID)
    {
        ...
        {
            tmp = cluster_next(&si->free_clusters.head) * SWAPFILE_CLUSTER;
        }
                               ...
                               swp_entry_t slots[], int order)
{
    ...
    if (order > 0)
    {
         ...
         */
        if (!IS_ENABLED(CONFIG_THP_SWAP) || nr_pages > SWAPFILE_CLUSTER)
        {
            VM_WARN_ON_ONCE(1);
            return 0;
        }
                     ...
                     unsigned long addr, swp_entry_t entry, struct folio *folio)
{
    ...
    if (unlikely(!pte || !pte_same_as_swp(ptep_get(pte), swp_entry_to_pte(entry))))
    {
        ret = 0;
        goto out;
    }
...
retry:
    ...
    p = &init_mm.mmlist;
    while (READ_ONCE(si->inuse_pages) && !signal_pending(current) && (p = p->next) != &init_mm.mmlist)
    {
...
SYSCALL_DEFINE2(swapon, const char __user *, specialfile, int, swap_flags)
{
    ...
    if ((swap_flags & SWAP_FLAG_DISCARD) && p->bdev && bdev_max_discard_sectors(p->bdev))
    {
        /*
