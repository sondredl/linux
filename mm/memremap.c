...
{
    return (pfn_end(pgmap, range_id) - pfn_first(pgmap, range_id)) >> pgmap->vmemmap_shift;
}
...
{
    if (WARN_ON_ONCE(!folio->page.pgmap->ops || !folio->page.pgmap->ops->page_free))
        return;
    ...
    if (folio->page.pgmap->type != MEMORY_DEVICE_PRIVATE && folio->page.pgmap->type != MEMORY_DEVICE_COHERENT)
        /*
