...
void putback_movable_pages(struct list_head *l)
{
    ...
    list_for_each_entry_safe(folio, folio2, l, lru)
    {
        ...
        {
            node_stat_mod_folio(folio, NR_ISOLATED_ANON + folio_is_file_lru(folio), -folio_nr_pages(folio));
                             ...
                             enum migrate_mode mode)
{
    ...
    else
    {
        ...
        WARN_ON_ONCE(rc == MIGRATEPAGE_SUCCESS && !folio_test_isolated(src));
    }
                               ...
                               enum migrate_reason reason)
{
    ...
    if (likely(!__folio_test_movable(src)))
        mod_node_page_state(folio_pgdat(src), NR_ISOLATED_ANON + folio_is_file_lru(src), -folio_nr_pages(src));
                               ...
                               struct migrate_pages_stats *stats, int nr_pass)
{
    ...
    VM_WARN_ON_ONCE(mode != MIGRATE_ASYNC && !list_empty(from) && !list_is_singular(from));
    ...
    for (pass = 0; pass < nr_pass && retry; pass++)
    {
        ...
        list_for_each_entry_safe(folio, folio2, from, lru)
        {
             ...
             * use _deferred_list.
             */
            if (nr_pages > 2 && !list_empty(&folio->_deferred_list))
            {
                            ...
                            int node)
{
     ...
     */
    if (folio_likely_mapped_shared(folio) && folio_is_file_lru(folio) && (vma->vm_flags & VM_EXEC))
        goto out;
    ...
    if (nr_remaining)
    {
        ...
        {
            list_del(&folio->lru);
            node_stat_mod_folio(folio, NR_ISOLATED_ANON + folio_is_file_lru(folio), -nr_pages);
            folio_putback_lru(folio);
        }
        isolated = 0;
    }
