...
{
    return atomic64_read(&vmstats->stats_updates) > MEMCG_CHARGE_BATCH * num_online_cpus();
}
...
static inline unsigned long memcg_page_state_output(struct mem_cgroup *memcg,
                                                    int                item)
{
    return memcg_page_state(memcg, item) * memcg_page_state_output_unit(item);
}

static inline unsigned long memcg_page_state_local_output(
    struct mem_cgroup *memcg, int item)
{
    return memcg_page_state_local(memcg, item) * memcg_page_state_output_unit(item);
}
...
static void memcg_stat_format(struct mem_cgroup *memcg, struct seq_buf *s)
{
    ...
    seq_buf_printf(s, "pgscan %lu\n",
                   memcg_events(memcg, PGSCAN_KSWAPD) + memcg_events(memcg, PGSCAN_DIRECT) + memcg_events(memcg, PGSCAN_KHUGEPAGED));
    seq_buf_printf(s, "pgsteal %lu\n",
                   memcg_events(memcg, PGSTEAL_KSWAPD) + memcg_events(memcg, PGSTEAL_DIRECT) + memcg_events(memcg, PGSTEAL_KHUGEPAGED));
...
static void drain_all_stock(struct mem_cgroup *root_memcg)
{
    ...
    for_each_online_cpu(cpu)
    {
        ...
        if (memcg && READ_ONCE(stock->nr_pages) && mem_cgroup_is_descendant(memcg, root_memcg))
            flush = true;
...
done_restock:
    ...
    do
    {
        ...
        mem_high  = page_counter_read(&memcg->memory) > READ_ONCE(memcg->memory.high);
        swap_high = page_counter_read(&memcg->swap) > READ_ONCE(memcg->swap.high);
     ...
     */
    if (current->memcg_nr_pages_over_high > MEMCG_CHARGE_BATCH && !(current->flags & PF_MEMALLOC) && gfpflags_allow_blocking(gfp_mask))
...
static struct obj_cgroup *current_objcg_update(void)
{
    ...
    do
    {
        ...
        if (old)
        {
            old = (struct obj_cgroup *)((unsigned long)old & ~CURRENT_OBJCG_UPDATE_FLAG);
            obj_cgroup_put(old);
                                 ...
                                 unsigned long max, bool memsw)
{
    ...
    do
    {
         ...
         */
        limits_invariant = memsw ? max >= READ_ONCE(memcg->memory.max) : max <= memcg->memsw.max;
        if (!limits_invariant)
        {
...
static unsigned long mem_cgroup_usage(struct mem_cgroup *memcg, bool swap)
{
    ...
    if (mem_cgroup_is_root(memcg))
    {
         ...
         */
        val = global_node_page_state(NR_FILE_PAGES) + global_node_page_state(NR_ANON_MAPPED);
        if (swap)
...
static void memcg1_stat_format(struct mem_cgroup *memcg, struct seq_buf *s)
{
    ...
    for (i = 0; i < NR_LRU_LISTS; i++)
        seq_buf_printf(s, "%s %lu\n", lru_list_name(i),
                       memcg_page_state_local(memcg, NR_LRU_BASE + i) * PAGE_SIZE);
    ...
    for (i = 0; i < NR_LRU_LISTS; i++)
        seq_buf_printf(s, "total_%s %llu\n", lru_list_name(i),
                       (u64)memcg_page_state(memcg, NR_LRU_BASE + i) * PAGE_SIZE);
                         ...
                         unsigned long *pwriteback)
{
    ...
    *pfilepages = memcg_page_state(memcg, NR_INACTIVE_FILE) + memcg_page_state(memcg, NR_ACTIVE_FILE);
...
static struct mem_cgroup *mem_cgroup_alloc(struct mem_cgroup *parent)
{
    ...
    for_each_node(node) if (!alloc_mem_cgroup_per_node_info(memcg, node)) goto fail;
...
static void mem_cgroup_exit(struct task_struct *task)
{
    ...
    objcg = (struct obj_cgroup *)((unsigned long)objcg & ~CURRENT_OBJCG_UPDATE_FLAG);
    obj_cgroup_put(objcg);
...
static void mem_cgroup_lru_gen_attach(struct cgroup_taskset *tset)
{
    ...
    cgroup_taskset_for_each_leader(task, css, tset) break;
...
static inline unsigned long lruvec_page_state_output(struct lruvec *lruvec,
                                                     int            item)
{
    return lruvec_page_state(lruvec, item) * memcg_page_state_output_unit(item);
}
                                          ...
                                          unsigned long siblings_protected)
{
    ...
    if (!(cgrp_dfl_root.flags & CGRP_ROOT_MEMORY_RECURSIVE_PROT))
        return ep;
    if (parent_effective > siblings_protected && parent_usage > siblings_protected && usage > protected)
    {
                                  ...
                                  long nr_pages)
{
     ...
     */
    if (mem_cgroup_disabled() || !memcg || !cgroup_subsys_on_dfl(memory_cgrp_subsys) || !(cgrp_dfl_root.flags & CGRP_ROOT_MEMORY_HUGETLB_ACCOUNTING))
        return -EOPNOTSUPP;
...
static void uncharge_folio(struct folio *folio, struct uncharge_gather *ug)
{
    ...
    VM_BUG_ON_FOLIO(folio_test_lru(folio), folio);
    VM_BUG_ON_FOLIO(folio_order(folio) > 1 && !folio_test_hugetlb(folio) && !list_empty(&folio->_deferred_list), folio);
...
long mem_cgroup_get_nr_swap_pages(struct mem_cgroup *memcg)
{
    ...
    for (; !mem_cgroup_is_root(memcg); memcg = parent_mem_cgroup(memcg))
        nr_swap_pages = min_t(long, nr_swap_pages,
                              READ_ONCE(memcg->swap.max) - page_counter_read(&memcg->swap));
    return nr_swap_pages;
}
...
bool mem_cgroup_swap_full(struct folio *folio)
{
    ...
    for (; !mem_cgroup_is_root(memcg); memcg = parent_mem_cgroup(memcg))
    {
        ...
        if (usage * 2 >= READ_ONCE(memcg->swap.high) || usage * 2 >= READ_ONCE(memcg->swap.max))
            return true;
    }
...
static int __init setup_swap_account(char *s)
{
    ...
    if (!kstrtobool(s, &res) && !res)
        pr_warn_once(
            "The swapaccount=0 commandline option is deprecated "
