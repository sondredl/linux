...
{
    BUILD_BUG_ON(PGSTEAL_DIRECT - PGSTEAL_KSWAPD != PGDEMOTE_DIRECT - PGDEMOTE_KSWAPD);
    BUILD_BUG_ON(PGSTEAL_KHUGEPAGED - PGSTEAL_KSWAPD != PGDEMOTE_KHUGEPAGED - PGDEMOTE_KSWAPD);
    BUILD_BUG_ON(PGSTEAL_DIRECT - PGSTEAL_KSWAPD != PGSCAN_DIRECT - PGSCAN_KSWAPD);
    BUILD_BUG_ON(PGSTEAL_KHUGEPAGED - PGSTEAL_KSWAPD != PGSCAN_KHUGEPAGED - PGSCAN_KSWAPD);
...
static inline int is_page_cache_freeable(struct folio *folio)
{
     ...
     */
    return folio_ref_count(folio) - folio_test_private(folio) == 1 + folio_nr_pages(folio);
}
...
void reclaim_throttle(pg_data_t *pgdat, enum vmscan_throttle_state reason)
{
     ...
     */
    if (!current_is_kswapd() && current->flags & (PF_USER_WORKER | PF_KTHREAD))
    {
        cond_resched();
        return;
    }
                              ...
                              int nr_throttled)
{
     ...
     */
    nr_written = node_page_state(pgdat, NR_THROTTLED_WRITTEN) - READ_ONCE(pgdat->nr_reclaim_start);
                            ...
                            bool reclaimed, struct mem_cgroup *target_memcg)
{
    ...
    else
    {
         ...
         * same address_space.
         */
        if (reclaimed && folio_is_file_lru(folio) && !mapping_exiting(mapping) && !dax_mapping(mapping))
            shadow = workingset_eviction(folio, target_memcg);
                                        ...
                                        bool *dirty, bool *writeback)
{
     ...
     * test is needed.
     */
    if (!folio_is_file_lru(folio) || (folio_test_anon(folio) && !folio_test_swapbacked(folio)))
    {
        *dirty     = false;
        *writeback = false;
        return;
                                      ...
                                      struct pglist_data *pgdat)
{
    ...
    struct migration_target_control mtc = {
         ...
         * instead of migrated.
         */
        .gfp_mask = (GFP_HIGHUSER_MOVABLE & ~__GFP_RECLAIM) | __GFP_NOWARN | __GFP_NOMEMALLOC | GFP_NOWAIT,
        .nid      = target_nid,
        .nmask    = &allowed_mask,
        .reason   = MR_DEMOTION,
    };
...
retry:
    while (!list_empty(folio_list))
    {
        ...
        if (lru_gen_enabled() && !ignore_references && folio_mapped(folio) && folio_test_referenced(folio))
            goto keep_locked;
        ...
        {
            /* Case 1 above */
            if (current_is_kswapd() && folio_test_reclaim(folio) && test_bit(PGDAT_WRITEBACK, &pgdat->flags))
            {
            ...
            else if (writeback_throttling_sane(sc) || !folio_test_reclaim(folio) || !may_enter_fs(folio, sc->gfp_mask))
            {
                /*
         ...
         */
        if (do_demote_pass && (thp_migration_supported() || !folio_test_large(folio)))
        {
        ...
        if (folio_test_anon(folio) && folio_test_swapbacked(folio))
        {
            if (!folio_test_swapcache(folio))
            {
                ...
                if (folio_test_large(folio))
                {
                     ...
                     */
                    if (data_race(!list_empty(&folio->_deferred_list)) && split_folio_to_list(folio, folio_list))
                        goto activate_locked;
                }
        ...
        if (folio_test_dirty(folio))
        {
             ...
             * flag set).
             */
            if (folio_is_file_lru(folio) && (!current_is_kswapd() || !folio_test_reclaim(folio) || !test_bit(PGDAT_DIRTY, &pgdat->flags)))
            {
                /*
    ...
    activate_locked:
        ...
        if (folio_test_swapcache(folio) && (mem_cgroup_swap_full(folio) || folio_test_mlocked(folio)))
                                           ...
                                           struct list_head *folio_list)
{
    ...
    {
        if (!folio_test_hugetlb(folio) && folio_is_file_lru(folio) && !folio_test_dirty(folio) && !__folio_test_movable(folio) && !folio_test_unevictable(folio))
        {
...
{
    return !current_is_kswapd() && gfp_migratetype(sc->gfp_mask) != MIGRATE_MOVABLE && folio_migratetype(folio) == MIGRATE_CMA;
}
#else
static bool skip_cma(struct folio *folio, struct scan_control *sc)
{
...
static void prepare_scan_control(pg_data_t *pgdat, struct scan_control *sc)
{
    ...
    if (!sc->force_deactivate)
    {
        ...
        if (refaults != target_lruvec->refaults[WORKINGSET_ANON] || inactive_is_low(target_lruvec, LRU_INACTIVE_ANON))
        ...
        if (refaults != target_lruvec->refaults[WORKINGSET_FILE] || inactive_is_low(target_lruvec, LRU_INACTIVE_FILE))
    ...
    if (file >> sc->priority && !(sc->may_deactivate & DEACTIVATE_FILE) && !sc->no_cache_trim_mode)
    ...
    if (!cgroup_reclaim(sc))
    {
        ...
        sc->file_is_tiny =
            file + free <= total_high_wmark && !(sc->may_deactivate & DEACTIVATE_ANON) && anon >> sc->priority;
    }
}
...
static int get_swappiness(struct lruvec *lruvec, struct scan_control *sc)
{
    ...
    if (!can_demote(pgdat->node_id, sc) && mem_cgroup_get_nr_swap_pages(memcg) < MIN_LRU_BATCH)
        return 0;
...
static bool __maybe_unused seq_is_valid(struct lruvec *lruvec)
{
    ...
    return get_nr_gens(lruvec, LRU_GEN_FILE) >= MIN_NR_GENS && get_nr_gens(lruvec, LRU_GEN_FILE) <= get_nr_gens(lruvec, LRU_GEN_ANON) && get_nr_gens(lruvec, LRU_GEN_ANON) <= MAX_NR_GENS;
}
                          ...
                          struct ctrl_pos *pos)
{
    ...
    pos->refaulted = lrugen->avg_refaulted[type][tier] + atomic_long_read(&lrugen->refaulted[hist][type][tier]);
    pos->total     = lrugen->avg_total[type][tier] + atomic_long_read(&lrugen->evicted[hist][type][tier]);
    if (tier)
...
static void reset_ctrl_pos(struct lruvec *lruvec, int type, bool carryover)
{
    ...
    for (tier = 0; tier < MAX_NR_TIERS; tier++)
    {
        if (carryover)
        {
            ...
            sum = lrugen->avg_refaulted[type][tier] + atomic_long_read(&lrugen->refaulted[hist][type][tier]);
            WRITE_ONCE(lrugen->avg_refaulted[type][tier], sum / 2);

            sum = lrugen->avg_total[type][tier] + atomic_long_read(&lrugen->evicted[hist][type][tier]);
            if (tier)
...
static bool positive_ctrl_err(struct ctrl_pos *sp, struct ctrl_pos *pv)
{
     ...
     */
    return pv->refaulted < MIN_LRU_BATCH || pv->refaulted * (sp->total + MIN_LRU_BATCH) * sp->gain <= (sp->refaulted + 1) * pv->total * pv->gain;
}
...
restart:
    for (i = pte_index(start), addr = start; addr != end; i++, addr += PAGE_SIZE)
    {
        ...
        if (pte_dirty(ptent) && !folio_test_dirty(folio) && !(folio_test_anon(folio) && folio_test_swapbacked(folio) && !folio_test_swapcache(folio)))
                                  ...
                                  struct mm_walk *args, unsigned long *bitmap, unsigned long *first)
{
    ...
    do
    {
        ...
        if (pmd_dirty(pmd[i]) && !folio_test_dirty(folio) && !(folio_test_anon(folio) && folio_test_swapbacked(folio) && !folio_test_swapcache(folio)))
            folio_mark_dirty(folio);
...
restart:
    ...
    for (type = 0; type < ANON_AND_FILE; type++)
    {
        ...
        {
            enum lru_list lru   = type * LRU_INACTIVE_FILE;
            long          delta = lrugen->nr_pages[prev][type][zone] - lrugen->nr_pages[next][type][zone];
                       ...
                       int tier_idx)
{
    ...
    if (folio_test_locked(folio) || folio_test_writeback(folio) || (type == LRU_GEN_FILE && folio_test_dirty(folio)))
    {
...
static bool isolate_folio(struct lruvec *lruvec, struct folio *folio, struct scan_control *sc)
{
    ...
    /* swap constrained */
    if (!(sc->gfp_mask & __GFP_IO) && (folio_test_dirty(folio) || (folio_test_anon(folio) && !folio_test_swapcache(folio))))
        return false;
...
retry:
    ...
    list_for_each_entry_safe_reverse(folio, next, &list, lru)
    {
        ...
        if (skip_retry || folio_test_active(folio) || folio_test_referenced(folio) || folio_mapped(folio) || folio_test_locked(folio) || folio_test_dirty(folio) || folio_test_writeback(folio))
        {
...
static int shrink_one(struct lruvec *lruvec, struct scan_control *sc)
{
    ...
    return READ_ONCE(lruvec->lrugen.seg) != MEMCG_LRU_TAIL ? MEMCG_LRU_TAIL : MEMCG_LRU_YOUNG;
}
...
static void shrink_lruvec(struct lruvec *lruvec, struct scan_control *sc)
{
     ...
     */
    proportional_reclaim = (!cgroup_reclaim(sc) && !current_is_kswapd() && sc->priority == DEF_PRIORITY);
...
again:
     ...
     * the LRU too quickly.
     */
    if (!current_is_kswapd() && current_may_throttle() && !sc->hibernation_mode && (test_bit(LRUVEC_CGROUP_CONGESTED, &target_lruvec->flags) || test_bit(LRUVEC_NODE_CONGESTED, &target_lruvec->flags)))
...
restart:
     ...
     */
    if (!sc.nr_reclaimed && sc.priority < 1 && !sc.no_cache_trim_mode && sc.cache_trim_mode_failed)
    {
...
static int __node_reclaim(struct pglist_data *pgdat, gfp_t gfp_mask, unsigned int order)
{
    ...
    if (node_pagecache_reclaimable(pgdat) > pgdat->min_unmapped_pages || node_page_state_pages(pgdat, NR_SLAB_RECLAIMABLE_B) > pgdat->min_slab_pages)
    {
        /*
