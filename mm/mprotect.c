                             ...
                             unsigned long end, pgprot_t newprot, unsigned long cp_flags)
{
    ...
    if (prot_numa && !(vma->vm_flags & VM_SHARED) && atomic_read(&vma->vm_mm->mm_users) == 1)
    ...
    do
    {
        ...
        if (pte_present(oldpte))
        {
            ...
            if (prot_numa)
            {
                ...
                if (is_cow_mapping(vma->vm_flags) && (folio_maybe_dma_pinned(folio) || folio_likely_mapped_shared(folio)))
                    continue;
             ...
             */
            if ((cp_flags & MM_CP_TRY_CHANGE_WRITABLE) && !pte_write(ptent) && can_change_pte_writable(vma, addr, ptent))
                                    ...
                                    unsigned long end, pgprot_t newprot, unsigned long cp_flags)
{
    ...
    again:
        ...
        {
            if ((next - addr != HPAGE_PMD_SIZE) || pgtable_split_needed(vma, cp_flags))
            {
                   ...
                   unsigned long start, unsigned long end, unsigned long newflags)
{
     ...
     */
    if (arch_has_pfn_modify_check() && (vma->vm_flags & (VM_PFNMAP | VM_MIXEDMAP)) && (newflags & VM_ACCESS_FLAGS) == 0)
    {
    ...
    if (newflags & VM_WRITE)
    {
        ...
        if (!may_expand_vm(mm, newflags, nrpages) && may_expand_vm(mm, oldflags, nrpages))
            return -ENOMEM;
        if (!(oldflags & (VM_ACCOUNT | VM_WRITE | VM_HUGETLB | VM_SHARED | VM_NORESERVE)))
        {
            charged = nrpages;
    ...
    else if ((oldflags & VM_ACCOUNT) && vma_is_anonymous(vma) && !vma->anon_vma)
    {
        newflags &= ~VM_ACCOUNT;
    }
    ...
    *pprev = vma;
     ...
     * fault on access.
     */
    if ((oldflags & (VM_WRITE | VM_SHARED | VM_LOCKED)) == VM_LOCKED && (newflags & VM_WRITE))
    {
