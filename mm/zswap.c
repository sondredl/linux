...
static struct zswap_pool *__zswap_pool_create_fallback(void)
{
    ...
    if (!has_comp && strcmp(zswap_compressor, CONFIG_ZSWAP_COMPRESSOR_DEFAULT))
    {
...
static void zswap_decompress(struct zswap_entry *entry, struct page *page)
{
     ...
     */
    if ((acomp_ctx->is_sleepable && !zpool_can_sleep_mapped(zpool)) || !virt_addr_valid(src))
    {
                                         ...
                                         struct shrink_control *sc)
{
    ...
    if (!zswap_shrinker_enabled || !mem_cgroup_zswap_writeback_enabled(sc->memcg))
    {
        sc->nr_scanned = 0;
        return SHRINK_STOP;
    }
