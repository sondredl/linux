...
static void shmem_recalc_inode(struct inode *inode, long alloced, long swapped)
{
    ...
    freed = info->alloced - info->swapped - READ_ONCE(inode->i_mapping->nrpages);
    /*
...
void shmem_unlock_mapping(struct address_space *mapping)
{
     ...
     */
    while (!mapping_unevictable(mapping) && filemap_get_folios(mapping, &index, ~0UL, &fbatch))
    {
                         ...
                         u32 request_mask, unsigned int query_flags)
{
    ...
    stat->attributes_mask |= (STATX_ATTR_APPEND | STATX_ATTR_IMMUTABLE | STATX_ATTR_NODUMP);
                                               ...
                                               struct mm_struct *fault_mm, bool huge)
{
    ...
    if (error)
    {
         ...
         */
        spin_lock(&info->lock);
        freed = pages + info->alloced - info->swapped - READ_ONCE(mapping->nrpages);
        if (freed > 0)
            info->alloced -= freed;
        spin_unlock(&info->lock);
        if (freed > 0)
                              ...
                              vm_fault_t *fault_type)
{
    ...
    *foliop = NULL;
    ...
    folio_lock(folio);
    if (!folio_test_swapcache(folio) || folio->swap.val != swap.val || !shmem_confirm_swap(mapping, index, swap))
    {
        error = -EEXIST;
        goto unlock;
    }
...
alloced:
    alloced = true;
    if (folio_test_pmd_mappable(folio) && DIV_ROUND_UP(i_size_read(inode), PAGE_SIZE) < folio_next_index(folio) - 1)
    {
        struct shmem_sb_info    *sbinfo = SHMEM_SB(inode->i_sb);
        struct shmem_inode_info *info   = SHMEM_I(inode);
        /*
                                      ...
                                      size_t len, unsigned int flags)
{
    ...
    do
    {
        ...
        if (folio)
        {
            ...
            if (folio_test_hwpoison(folio) || (folio_test_large(folio) && folio_test_has_hwpoisoned(folio)))
            {
                error = -EIO;
                break;
            }
...
static int shmem_statfs(struct dentry *dentry, struct kstatfs *buf)
{
    ...
    if (sbinfo->max_blocks)
    {
        ...
        buf->f_bavail =
            buf->f_bfree = sbinfo->max_blocks - percpu_counter_sum(&sbinfo->used_blocks);
    }
    if (sbinfo->max_inodes)
    {
                                  ...
                                  struct delayed_call *done)
{
    ...
    if (!dentry)
    {
        ...
        if (PageHWPoison(folio_page(folio, 0)) || !folio_test_uptodate(folio))
        {
            folio_put(folio);
            return ERR_PTR(-ECHILD);
        }
                              ...
                              struct dentry *dentry, struct fileattr *fa)
{
    ...
    info->fsflags = (info->fsflags & ~SHMEM_FL_USER_MODIFIABLE) | (fa->flags & SHMEM_FL_USER_MODIFIABLE);
...
static int shmem_parse_one(struct fs_context *fc, struct fs_parameter *param)
{
    ...
    switch (opt)
    {
        ...
        case Opt_huge:
            ...
            if (ctx->huge != SHMEM_HUGE_NEVER && !(IS_ENABLED(CONFIG_TRANSPARENT_HUGEPAGE) && has_transparent_hugepage()))
...
static int shmem_reconfigure(struct fs_context *fc)
{
    ...
    if ((ctx->seen & SHMEM_SEEN_INUMS) && !ctx->full_inums && sbinfo->next_ino > UINT_MAX)
    {
    ...
    if (ctx->seen & SHMEM_SEEN_QUOTA && !sb_any_quota_loaded(fc->root->d_sb))
    {
        ...
        goto out;
            ...
            #define CHANGED_LIMIT(name) \
                (ctx->qlimits.name##hardlimit && (ctx->qlimits.name##hardlimit != sbinfo->qlimits.name##hardlimit))

    if (CHANGED_LIMIT(usrquota_b) || CHANGED_LIMIT(usrquota_i) || CHANGED_LIMIT(grpquota_b) || CHANGED_LIMIT(grpquota_i))
    {
