...
static inline struct hugetlb_cgroup *hugetlb_cgroup_from_task(struct task_struct *task)
{
                                 ...
                                 enum hugetlb_memory_event event)
{
    ...
    } while ((hugetlb = parent_hugetlb_cgroup(hugetlb)) && !hugetlb_cgroup_is_root(hugetlb));
}
...
void hugetlb_cgroup_uncharge_counter(struct resv_map *resv, unsigned long start,
                                     unsigned long end)
{
    if (hugetlb_cgroup_disabled() || !resv || !resv->reservation_counter || !resv->css)
        return;
                                         ...
                                         bool                region_del)
{
    ...
    if (rg->reservation_counter && resv->pages_per_hpage && !resv->reservation_counter)
    {
    ...
    if (legacy)
    {
        ...
        for_each_node_state(nid, N_MEMORY)
            seq_printf(seq, " N%d=%lu", nid,
                       READ_ONCE(h_cg->nodeinfo[nid]->usage[idx]) * PAGE_SIZE);
        seq_putc(seq, '\n');
    }
