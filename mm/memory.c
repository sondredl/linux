                                  ...
                                  pte_t *ptep)
{
    ...
    VM_BUG_ON_FOLIO(pte_write(pte) && (!folio_test_anon(folio) && PageAnonExclusive(page)), folio);
                                   ...
                                   struct zap_details *details)
{
    ...
    do
    {
        ...
        else if (pte_marker_entry_uffd_wp(entry))
        {
             ...
             */
            if (!vma_is_anonymous(vma) && !zap_drop_file_uffd_wp(details))
                continue;
        }
                                          ...
                                          struct zap_details *details)
{
    ...
    do
    {
        ...
        else if (details && details->single_folio && folio_test_pmd_mappable(details->single_folio) && next - addr == HPAGE_PMD_SIZE && pmd_none(*pmd))
        {
...
void zap_vma_ptes(struct vm_area_struct *vma, unsigned long address,
                  unsigned long size)
{
    if (!range_in_vma(vma, address, address + size) || !(vma->vm_flags & VM_PFNMAP))
        return;
...
static int validate_page_before_insert(struct page *page)
{
    ...
    if (folio_test_anon(folio) || folio_test_slab(folio) || page_has_type(page))
        return -EINVAL;
                              ...
                              pgtbl_mod_mask *mask)
{
    ...
    if (create)
    {
        mapped_pte = pte = (mm == &init_mm) ? pte_alloc_kernel_track(pmd, addr, mask) : pte_alloc_map_lock(mm, pmd, addr, &ptl);
        if (!pte)
            return -ENOMEM;
    }
    else
    {
        mapped_pte = pte = (mm == &init_mm) ? pte_offset_kernel(pmd, addr) : pte_offset_map_lock(mm, pmd, addr, &ptl);
        if (!pte)
            return -EINVAL;
    }
                                           ...
                                           unsigned int           fault_flags)
{
    if (!folio_test_swapcache(folio))
        return false;
    if (mem_cgroup_swap_full(folio) || (vma->vm_flags & VM_LOCKED) || folio_test_mlocked(folio))
        return true;
    /*
...
vm_fault_t do_swap_page(struct vm_fault *vmf)
{
    ...
    if (swapcache)
    {
         ...
         * caches if required.
         */
        if ((vmf->flags & FAULT_FLAG_WRITE) && folio == swapcache && !folio_test_ksm(folio) && !folio_test_lru(folio))
            lru_add_drain();
    }
    ...
    if (!folio_test_ksm(folio))
    {
        ...
        else if (exclusive && folio_test_writeback(folio) && data_race(si->flags & SWP_STABLE_WRITES))
        {
            /*
    ...
    VM_BUG_ON(!folio_test_anon(folio) || (pte_write(pte) && !PageAnonExclusive(page)));
    set_pte_at(vma->vm_mm, vmf->address, vmf->pte, pte);
                                       ...
                                       bool ignore_writable, bool pte_write_upgrade)
{
    ...
    for (addr = start; addr != end; start_ptep++, addr += PAGE_SIZE)
    {
        ...
        if (!ignore_writable)
        {
            ...
            if (!writable && pte_write_upgrade && can_change_pte_writable(vma, addr, ptent))
                writable = true;
        }
...
static inline vm_fault_t wp_huge_pmd(struct vm_fault *vmf)
{
    ...
    if (vma_is_anonymous(vma))
    {
        if (likely(!unshare) && userfaultfd_huge_pmd_wp(vma, vmf->orig_pmd))
        {
                                    ...
                                    unsigned long address, unsigned int flags)
{
    ...
    if (!vmf.pud)
        return VM_FAULT_OOM;
retry_pud:
    if (pud_none(*vmf.pud) && thp_vma_allowable_order(vma, vm_flags, TVA_IN_PF | TVA_ENFORCE_SYSFS, PUD_ORDER))
    {
    ...
    if (pmd_none(*vmf.pmd) && thp_vma_allowable_order(vma, vm_flags, TVA_IN_PF | TVA_ENFORCE_SYSFS, PMD_ORDER))
    {
    ...
    else
    {
        ...
        {
            VM_BUG_ON(thp_migration_supported() && !is_pmd_migration_entry(vmf.orig_pmd));
                                       ...
                                       unsigned int          *flags)
{
    ...
    else if (*flags & FAULT_FLAG_WRITE)
    {
        ...
        if (WARN_ON_ONCE(!(vma->vm_flags & VM_WRITE) && !is_cow_mapping(vma->vm_flags)))
            return VM_FAULT_SIGSEGV;
    }
