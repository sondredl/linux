    ...
    switch (cmd)
    {
        ...
        case PIN_LONGTERM_BENCHMARK:
            for (i = 0; i < nr_pages; i++)
            {
                ...
                else if (cmd == PIN_LONGTERM_BENCHMARK && WARN(!folio_is_longterm_pinnable(folio), "pages[%lu] is NOT pinnable but pinned\n", i))
                {
                    dump_page(&folio->page, "gup_test failure");
                    break;
                }
...
static inline int pin_longterm_test_start(unsigned long arg)
{
    ...
    if (args.flags & ~(PIN_LONGTERM_TEST_FLAG_USE_WRITE | PIN_LONGTERM_TEST_FLAG_USE_FAST))
        return -EINVAL;
