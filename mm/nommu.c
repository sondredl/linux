                                 ...
                                 unsigned long *_capabilities)
{
    ...
    if (file)
    {
        ...
        if (capabilities & NOMMU_MAP_DIRECT)
        {
            if (((prot & PROT_READ) && !(capabilities & NOMMU_MAP_READ)) || ((prot & PROT_WRITE) && !(capabilities & NOMMU_MAP_WRITE)) || ((prot & PROT_EXEC) && !(capabilities & NOMMU_MAP_EXEC)))
            {
                capabilities &= ~NOMMU_MAP_DIRECT;
                if (flags & MAP_SHARED)
                {
                                        ...
                                        unsigned long capabilities)
{
    ...
    else
    {
        ...
        vm_flags |= VM_SHARED | VM_MAYSHARE | (capabilities & NOMMU_VMFLAGS);
    }
