...
try_again:
    ...
    if (!parent)
    {
        ...
        {
            pr_warn_once(
                "Inconsistency in POSIX message queue, "
                                      ...
                                      struct mq_attr *attr)
{
    ...
    if (S_ISREG(mode))
    {
        ...
        {
            if (info->attr.mq_maxmsg > HARD_MSGMAX || info->attr.mq_msgsize > HARD_MSGSIZEMAX)
                goto out_inode;
        }
        else
        {
            if (info->attr.mq_maxmsg > ipc_ns->mq_msg_max || info->attr.mq_msgsize > ipc_ns->mq_msgsize_max)
                goto out_inode;
        }
        ret = -EOVERFLOW;
        /* check for overflow */
...
static int mqueue_create_attr(struct dentry *dentry, umode_t mode, void *arg)
{
    ...
    if (ipc_ns->mq_queues_count >= ipc_ns->mq_queues_max && !capable(CAP_SYS_RESOURCE))
    {
        error = -ENOSPC;
        goto out_unlock;
    }
                                ...
                                size_t count, loff_t *off)
{
    ...
    snprintf(buffer, sizeof(buffer),
             ...
             (info->notify_owner && info->notify.sigev_notify == SIGEV_SIGNAL) ? info->notify.sigev_signo : 0,
...
static void remove_notification(struct mqueue_inode_info *info)
{
    if (info->notify_owner != NULL && info->notify.sigev_notify == SIGEV_THREAD)
    {
                              ...
                              struct timespec64 *ts)
{
    ...
    if (ret == 0)
    {
        ...
        if ((u_msg_prio && put_user(msg_ptr->m_type, u_msg_prio)) || store_msg(u_msg_ptr, msg_ptr, msg_ptr->m_ts))
        {
            ret = -EFAULT;
        }
        free_msg(msg_ptr);
...
static int do_mq_notify(mqd_t mqdes, const struct sigevent *notification)
{
    ...
    {
        if (unlikely(notification->sigev_notify != SIGEV_NONE && notification->sigev_notify != SIGEV_SIGNAL && notification->sigev_notify != SIGEV_THREAD))
            return -EINVAL;
