...
static int MIPS_is_fake_mcount(Elf_Rel const *rp)
{
    ...
    is_fake      = (old_r_offset != ~(Elf_Addr)0) && (current_r_offset - old_r_offset == MIPS_FAKEMCOUNT_OFFSET);
                              ...
                              char const *const    str0)
{
    ...
    if (symname[0] == '.')
        ++symname; /* ppc64 hack */
    if (strcmp(mcount, symname) == 0 || (altmcount && strcmp(altmcount, symname) == 0) || (strcmp(fentry, symname) == 0))
                               ...
                               unsigned const        reltype)
{
    ...
    for (t = nrel; t; --t)
    {
        ...
        if (mcountsym && mcountsym == Elf_r_sym(relp) && !is_fake_mcount(relp))
        {
            uint_t const addend =
                                    ...
                                    char const *const     fname)
{
    ...
    if (w(txthdr->sh_type) != SHT_PROGBITS || !(_w(txthdr->sh_flags) & SHF_EXECINSTR))
        return NULL;
    return txtname;
}
