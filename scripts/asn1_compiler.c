...
static void build_type_list(void)
{
    ...
    for (n = 0; n < nr_tokens - 1; n++)
        if (token_list[n + 0].token_type == TOKEN_TYPE_NAME && token_list[n + 1].token_type == TOKEN_ASSIGNMENT)
            nr++;
...
static void parse(void)
{
    ...
    do
    {
        ...
        if (cursor[0].token_type != TOKEN_TYPE_NAME || cursor[1].token_type != TOKEN_ASSIGNMENT)
            abort();
        cursor += 2;
                                  ...
                                  struct token *name)
{
    ...
    if (cursor < end && (cursor->token_type == DIRECTIVE_OPTIONAL || cursor->token_type == DIRECTIVE_DEFAULT))
    {
        cursor++;
...
static void render_element(FILE *out, struct element *e, struct element *tag)
{
    ...
    if (e->flags & ELEMENT_SKIPPABLE || (tag && tag->flags & ELEMENT_SKIPPABLE))
        skippable = 1;
    ...
    cond = (e->flags & ELEMENT_CONDITIONAL || (tag && tag->flags & ELEMENT_CONDITIONAL)) ? "COND_" : "";
    act  = e->action ? "_ACT" : "";
    switch (e->compound)
    {
        case ANY:
    ...
    if (tag->class == ASN1_UNIV && tag->tag != 14 && tag->tag != 15 && tag->tag != 31)
